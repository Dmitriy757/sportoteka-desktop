...
class ActionTrackerBleService {
  ...
  Future<bool> forceReconnectCommandChannel({
    Duration settleDelay = const Duration(milliseconds: 1400),
  }) async {
    ...
    await disconnect(clearInfo: false);
    connectedInfo = info;
    lastKnownInfo = info;
  ...
  Future<void> connect(
    ...
    lastKnownInfo = info;
    _log(
        'Подключение к ${info.name} / ${info.id}...${info.name.contains('paired') ? ' (Android paired/bonded)' : ''}');
  ...
  Future<ActionTrackerDevice?> _scanForReconnect(
    ...
    for (final item in candidates) {
      ...
      if (targetName.isNotEmpty &&
          name.contains(targetName.replaceAll(' ', ''))) return item;
      if (name.contains('ATP') || name.contains('ACT') || name.contains('GPS'))
        return item;
    }
    return null;
  }
  ...
  Future<ActionTrackerDevice?> _nativeAndroidBondedByTarget(
    ...
    try {
      ...
      for (final entry in raw) {
        ...
        final idKey = _normalizeId(id);
        final nameKey = _normalizeName(name);
        final idMatch = targetId.isNotEmpty &&
            (idKey == targetId ||
                idKey.endsWith(targetId) ||
                targetId.endsWith(idKey));
        final nameMatch = targetName.isNotEmpty &&
            nameKey.isNotEmpty &&
            (nameKey == targetName ||
                nameKey.contains(targetName) ||
                targetName.contains(nameKey));
