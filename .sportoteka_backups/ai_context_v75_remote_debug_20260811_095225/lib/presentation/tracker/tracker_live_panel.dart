...
// а зависший PHP отвечал TimeoutException спустя 20 секунд.
  ...
  Future<void> _stopLive() async {
    if (widget.onStopTeamLive != null && widget.teamTrackerCount > 0) {
      _setLiveRunning(false);
      _paused = false;
      _phoneTimer?.cancel();
      _trackerTimer?.cancel();
...
// ignore: avoid_print
    ...
    return DefaultTextStyle.merge(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          if (width < 720) {
            base = _phoneOpenFieldLayout(online, active);
          } else if (width < 1120) {
                  ...
                  children: [
                    ...
                    if (_monitorPlayersExpanded)
                      Positioned.fill(
                        child: Material(
                          color: Colors.white,
                          child: _expandedMonitorDetailPlayer() == null
                              ? _tabletExpandedMonitorPlayersGrid(
                                  compact: width < 1120 ||
                                      constraints.maxHeight < 680,
                                )
                              : _tabletExpandedMonitorPlayerDetails(
                                  _expandedMonitorDetailPlayer()!,
                                  compact: width < 1120 ||
                                      constraints.maxHeight < 680,
                                ),
                        ),
  ...
  Widget _desktopOpenFieldLayout(int online, int active) {
    return LayoutBuilder(
      builder: (context, c) {
        ...
        final playersHeight = _monitorPlayersSectionHeight(
          ...
          child: Column(
            children: [
              ...
              SizedBox(
                ...
                child: _tabletMonitorPlayersSection(compact: compactHeight),
              ),
              _ofHorizontalDivider(),
              Expanded(
                child: _selectedPlayerLiveDashboard(
                  compact: compactHeight,
                  vertical: false,
                ),
              ),
        ...
        return Container(
          ...
          child: Column(
            children: [
              ...
              SizedBox(
                height: playersHeight,
                child: _tabletMonitorPlayersSection(compact: false),
              ),
              _ofHorizontalDivider(),
              Expanded(
                child: _selectedPlayerLiveDashboard(
        ...
        return Container(
          ...
          child: Column(
            children: [
              ...
              SizedBox(
                height: playersHeight,
                child: _tabletMonitorPlayersSection(compact: true),
              ),
              _ofHorizontalDivider(),
              Expanded(
                child: _selectedPlayerLiveDashboard(
  ...
  String _signalLabelForPlayer(TrackerPlayerOption player) {
    if (widget.connectedTeamPlayerIds.contains(player.id)) return 'BLE готов';
  ...
  void _openExpandedMonitorPlayerDetails(TrackerPlayerOption player) {
    ...
    setState(() => _expandedMonitorDetailPlayerId = player.id);
  }

  void _closeExpandedMonitorPlayerDetails() {
    if (_expandedMonitorDetailPlayerId == null) return;
    setState(() => _expandedMonitorDetailPlayerId = null);
  }
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${online ? 'Live online' : 'Live ожидает'} · '
  ...
  Widget _tabletExpandedMonitorPlayersGrid({required bool compact}) {
    final players = _monitorCandidatePlayers()
        ...
        .toList(growable: false);
    final monitoredIds =
        _monitorPlayersForGrid().map((player) => player.id).toSet();
    ...
    if (players.isEmpty) {
      return Container(
        key: const ValueKey<String>('monitor_players_expanded_empty'),
        color: Colors.white,
        padding: const EdgeInsets.symmetric(horizontal: 10),
        alignment: Alignment.centerLeft,
        child: const Text(
          'Подключите трекеры или Polar — здесь появятся карточки команды.',
          style: TextStyle(
            color: _OF.muted2,
            fontSize: 11,
            fontWeight: FontWeight.w700,
    ...
    return Container(
      key: const ValueKey<String>('monitor_players_expanded'),
      color: Colors.white,
      padding: EdgeInsets.fromLTRB(
        compact ? 8 : 10,
        6,
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          final columns = _expandedMonitorColumns(constraints.maxWidth);
          return Column(
            children: [
              SizedBox(
                height: 36,
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${players.length} карточек · $connectedCount с данными',
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            style: const TextStyle(
                              color: _OF.green,
              ...
              Expanded(
                child: GridView.builder(
                  ...
                  itemBuilder: (_, index) => _tabletMonitorPlayerChip(
                    players[index],
                    index,
                    monitored: monitoredIds.contains(players[index].id),
                    focused: widget.selectedPlayer?.id == players[index].id,
                    compact: compact,
                    expandedCard: true,
                  ),
                ),
  ...
  Widget _tabletMonitorPlayersStrip({required bool compact}) {
    final players = _monitorCandidatePlayers();
    final monitored = _monitorPlayersForGrid();
    final monitoredIds = monitored.map((p) => p.id).toSet();
    final connectedCount = players
        .where((p) =>
            _isPlayerOnlineLive(p.id) ||
            _heartRateForPlayerId(p.id) != null ||
    ...
    if (players.isEmpty) {
      return Container(
        ...
        alignment: Alignment.centerLeft,
        child: const Text(
            'Подключите трекеры или Polar — здесь появится лента игроков для Live-мониторинга.',
            style: TextStyle(
                color: _OF.muted2, fontSize: 11, fontWeight: FontWeight.w700)),
      );
    }
    return Container(
      color: Colors.white,
      padding: EdgeInsets.fromLTRB(
          compact ? 8 : 10, compact ? 7 : 8, compact ? 8 : 10, compact ? 7 : 8),
      child: Row(
        children: [
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${monitored.length} в окнах · $connectedCount online',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(
                    color: _OF.green,
          ...
          const SizedBox(width: 8),
          _monitorCardsToggleButton(compact: compact),
          const SizedBox(width: 5),
          _NoHoverTap(
            onTap: _clearMonitorPlayers,
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 5),
            Text(
              _monitorPlayersExpanded ? 'Свернуть' : '12 карточек',
              style: TextStyle(
                color: _OF.green,
...
// отмеченные игроки не переключаются и не останавливаются.
  ...
  Widget _tabletMonitorPlayerChip(
    ...
    final hasTrack = local?.points.isNotEmpty ?? false;
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          _compactPlayerName(player.name),
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                            color:
                                teamSelected || focused ? _OF.green : _OF.text,
              ...
              Row(children: [
                Expanded(
                    child: _monitorStripMetric(
                        ...
                        distance > 0 ? _OF.text : _OF.muted2)),
                Expanded(
                    child: _monitorStripMetric(
                        'Скор.',
                        speed.toStringAsFixed(1),
                        speed > 0 ? _OF.text : _OF.muted2)),
                Expanded(
                    child: _monitorStripMetric(
                        'Пульс',
                        heartRate == null ? '—' : '${heartRate.bpm}',
                        heartRate == null ? _OF.muted2 : _OF.red)),
                Expanded(
                    child: _monitorStripMetric(
                        'Нагрузка',
                        load.toStringAsFixed(0),
                        load > 0 ? loadColor : _OF.muted2)),
              ]),
              if (expandedCard) ...[
                const SizedBox(height: 7),
                _NoHoverTap(
  ...
  List<_RuntimeTrack> _focusedFieldTracks(TrackerPlayerOption player) {
    final focused = _displayTrackForPlayer(player);
    if (!_showTeamOnFocusedField) {
      return focused == null
          ? const <_RuntimeTrack>[]
    ...
    for (final candidate in _focusedTeamFieldPlayers(player)) {
      final track = _displayTrackForPlayer(candidate);
  ...
  Widget _focusedPlayerFieldPanel(
    ...
    final focusedTrack = _displayTrackForPlayer(player);
    ...
    return _ofPanel(
      title: teamMode
          ...
          : (_activity.requiresField
              ? 'Поле · ${_compactPlayerName(player.name)}'
              : 'Маршрут · ${_compactPlayerName(player.name)}'),
      subtitle: teamMode
  ...
  Widget _focusedTeamDataPanel(
    ...
    for (var i = 0; i < players.length; i++) {
      ...
      if (history.isNotEmpty) {
        heartSeries.add(
          _LiveMetricSeries(
            label: _compactPlayerName(player.name),
            color: _chartColor(i),
            points: history
                .map(
                ...
                child: DecoratedBox(
                  ...
                  child: CustomPaint(
                    painter: _LiveLineChartPainter(
                      series: series,
                      minY: heartMode ? 80 : 0,
                      maxY: heartMode ? 205 : math.max(22.0, maxSpeedValue + 3),
  ...
  Widget _focusedPlayerDataPanel(
    ...
    final track = _displayTrackForPlayer(player);
    final heartRate = _heartRateForPlayerId(player.id);
    final history = _focusedHeartRateHistory(player, heartRate);
    final loadTimeline = _focusedLoadTimeline(track);
    ...
    return _ofPanel(
      ...
      subtitle: online
          ? '${_compactPlayerName(player.name)} · online'
          : '${_compactPlayerName(player.name)} · ждём данные',
      actions: [
        Container(
          height: 22,
  ...
  Widget _tabletMonitoringGridPanel(
    ...
    final subtitle = players.isEmpty
        ...
        : '${players.length}/$_maxMonitorPlayers окон · свайп влево/вправо для остальных';
    return _ofPanel(
      title: title,
      subtitle: subtitle,
      actions: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(_compactPlayerName(player.name),
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            style: TextStyle(
                                color: _OF.text,
  ...
  Widget _phoneLiveHeroCard(int online, int active) {
    ...
    final statusText =
        _paused ? 'Пауза' : (_running ? 'Live идёт' : 'Готово к старту');
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 7),
              Expanded(
                child: Text('$statusText · ${widget.teamName}',
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: const TextStyle(
                        color: _OF.text,
              ...
              _phoneIconOnlyButton(Icons.more_horiz_rounded,
                  () => _openPhoneMoreActionsSheet(online, active)),
            ],
          ),
          const SizedBox(height: 8),
          Row(
                  ...
                  child: FilledButton.icon(
                    ...
                    style: FilledButton.styleFrom(
                      ...
                      disabledBackgroundColor:
                          canStart ? _OF.green.withOpacity(.45) : _OF.header,
          ...
          Row(
            children: [
              Expanded(
                  child: _phoneHeroMiniStat(Icons.timer_rounded,
                      _durationText(), _running && !_paused)),
              const SizedBox(width: 7),
              Expanded(
                  child: _phoneHeroMiniStat(
                      Icons.groups_rounded, '$online/$active', online > 0)),
              const SizedBox(width: 7),
              Expanded(
                  child: _phoneHeroMiniStat(Icons.gps_fixed_rounded,
                      gpsReady ? 'GPS' : 'GPS —', gpsReady)),
              const SizedBox(width: 7),
              Expanded(
                  child: _phoneHeroMiniStat(
                      Icons.monitor_heart_rounded,
                      hasPolar
                          ? 'HR ${widget.heartRateConnectedCount}'
                          : 'HR —',
                      hasPolar)),
            ],
          ),
  ...
  Widget _phoneSelectedPlayerLiveCard(int online, int active) {
    ...
    final maxSpeed = math.max(_safeLiveSpeed(session?.maxSpeedKmh ?? 0.0),
        _safeLiveSpeed(local?.maxSpeedKmh ?? 0.0));
    final load = math.max(session?.loadScore ?? 0.0, local?.loadScore ?? 0.0);
    final onlinePlayer = player != null &&
        (_isPlayerOnlineLive(player.id) ||
            heartRate != null ||
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              _playerAvatarCircle(
                  player: player, radius: 24, online: onlinePlayer),
              const SizedBox(width: 10),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(player?.name ?? 'Выберите игрока',
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: const TextStyle(
                            color: _OF.text,
                    ...
                    Text(
                        player == null
                            ? 'откройте список игроков для контроля Live'
                            : '№${player.number ?? '—'} · $fieldLabel · ${onlinePlayer ? 'online' : (_running ? 'ждём данные' : 'готов к старту')}',
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: TextStyle(
                            color: onlinePlayer ? _OF.green : _OF.muted2,
                            fontSize: 11.0,
                            fontWeight: FontWeight.w600)),
                  ],
                ),
              ...
              _phoneStatusPill(
                  onlinePlayer ? 'ON' : (_running ? 'WAIT' : 'OFF'),
                  onlinePlayer
                      ? _OF.green
              ...
              _phoneIconOnlyButton(
                  ...
                  player == null
                      ...
                      : () => _selectPlayerForAnalytics(player)),
            ],
          ),
          ...
          Row(
            children: [
              ...
              Expanded(
                  child: _phoneMetricBox(
                      'Пульс', heartRate == null ? '—' : '${heartRate.bpm}')),
              const SizedBox(width: 7),
              Expanded(
                  child: _phoneMetricBox('Нагрузка', load.toStringAsFixed(0))),
            ],
          ),
          const SizedBox(height: 8),
          Row(
            children: [
              Expanded(
                  child: _phoneSmallInfoLine(Icons.trending_up_rounded,
                      'Max ${maxSpeed.toStringAsFixed(1)} км/ч')),
              const SizedBox(width: 8),
              Expanded(
                  child: _phoneSmallInfoLine(
                Icons.sensors_rounded,
                _teamTrackerReady
                    ? (widget.teamTrackerCount > 0
                        ? 'GPS online · ${widget.teamTrackerCount}'
                        : 'Трекер online')
                    : 'Трекеры offline',
              )),
            ],
            ...
            SizedBox(
              ...
              child: hasData
                  ...
                  : Center(
                      child: Text(
                          _running
                              ? 'Ждём первые значения нагрузки'
                              : 'После старта появится график нагрузки',
                          textAlign: TextAlign.center,
                          style: const TextStyle(
                              color: _OF.muted2,
                              fontSize: 11.0,
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 2),
                  Text(
                      '№$number · ${_activity.shortTitle} · ${heart == null ? (online ? 'online' : 'ждём данные') : '${heart.bpm} bpm'}',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: TextStyle(
                          color: online ? _OF.green : _OF.muted2,
            ...
            Text(load > 0 ? load.toStringAsFixed(load < 10 ? 1 : 0) : '—',
                style: TextStyle(
                    color: load > 0 ? _OF.green : _OF.muted2,
                    fontSize: 13.4,
                    fontWeight: FontWeight.w700)),
            const SizedBox(width: 6),
            const Icon(Icons.chevron_right_rounded,
                size: 20, color: _OF.muted2),
          ],
        ),
      ),
    );
  ...
  Widget _liveControlStrip(int online, int active, {required bool compact}) {
    ...
    Widget primaryLiveButton({required bool phone}) {
      ...
      final surface = _running ? _OF.redSoft : _OF.greenSoftStrong;
      return SizedBox(
        height: 32,
        child: FilledButton.icon(
          onPressed: _starting
    ...
    if (compact) {
      return Container(
        ...
        decoration: const BoxDecoration(
          color: Colors.white,
          border: Border(bottom: BorderSide(color: _OF.line, width: .7)),
        ),
        child: Row(
          children: [
  ...
  Widget _activePlayersPanel(int online, int active) {
    ...
    return _ofPanel(
      ...
      actions: _renderingFloatingLiveContent
          ? const []
          : [
              _livePanelWindowButton(
                tooltip: 'Открыть подключённых игроков отдельным окном',
                onTap: () => _openExpandedLiveBlock(
                    'Подключённые игроки',
                    Icons.groups_rounded,
                    () => _activePlayersPanel(online, active)),
              ),
            ],
          ...
          child: _NoHoverTap(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: selected
                    ? _OF.blueSoft
                    : (online ? _OF.greenSoft : Colors.white),
                border: Border(
                    left: BorderSide(
                        color: online ? _OF.green : _OF.line, width: 4),
                    bottom: const BorderSide(color: _OF.line)),
              ),
              child: Row(children: [
                SizedBox(
                    width: 32,
                ...
                if (showStats) ...[
                  ...
                  SizedBox(
                      width: 30,
                      child: Text(heartRateBpm == null ? '—' : '$heartRateBpm',
                          textAlign: TextAlign.right,
                          style: TextStyle(
                              color:
                                  heartRateBpm == null ? _OF.muted2 : _OF.red,
                              fontSize: 10.4,
                              fontWeight: FontWeight.w700))),
                  const SizedBox(width: 6),
                  SizedBox(
                      width: 24,
                      child: Text(load.toStringAsFixed(0),
                          textAlign: TextAlign.right,
                          style: TextStyle(
                              color: load > 70 ? _OF.red : _OF.muted,
                              fontSize: 9.6,
                              fontWeight: FontWeight.w700))),
                  const SizedBox(width: 6),
                ] else
    ...
    return _ofPanel(
      ...
      actions: [
        ...
        if (!_renderingFloatingLiveContent)
          _livePanelWindowButton(
            label: 'На весь экран',
            tooltip: _liveSidePanelMode == 'map'
                ? 'Развернуть live-карту по выбранным игрокам'
  ...
  Widget _wholeTeamTablePanel({bool compact = false, bool phoneCards = false}) {
    ...
    return _ofPanel(
      ...
      subtitle: _running
          ...
          : 'Командная сводка · Анализ',
      actions: (_renderingFloatingLiveContent || phoneCards || hideWindowAction)
          ? const []
          : [
              _livePanelWindowButton(
                label: 'На весь экран',
                ...
                onTap: () => _openExpandedLiveBlock(
                    ...
                    () => _wholeTeamTablePanel(compact: false)),
              ),
            ],
      child: phoneCards
  ...
  Widget _phonePlayerCardFromPlayer(TrackerPlayerOption p, int i) {
    ...
    return _phonePlayerCard(
      ...
      distance: math.max(
          session?.totalDistanceM ?? 0.0, local?.totalDistanceM ?? 0.0),
      metersPerMin: math.max(
          session?.metersPerMinute ?? 0.0, local?.metersPerMinute ?? 0.0),
      load: math.max(session?.loadScore ?? 0.0, local?.loadScore ?? 0.0),
      max: math.max(_safeLiveSpeed(session?.maxSpeedKmh ?? 0.0),
          _safeLiveSpeed(local?.maxSpeedKmh ?? 0.0)),
      sprint: math.max(
          session?.sprintDistanceM ?? 0.0, local?.sprintDistanceM ?? 0.0),
      heartRateBpm: _heartRateForPlayerId(p.id)?.bpm,
  ...
  Widget _phonePlayerCardFromSession(TrackerLiveSessionModel s, int i) {
    ...
    return _phonePlayerCard(
      ...
      max: math.max(_safeLiveSpeed(s.maxSpeedKmh),
          _safeLiveSpeed(local?.maxSpeedKmh ?? 0.0)),
    ...
    return Container(
      ...
      child: Row(children: [
        ...cells.map((c) => Expanded(
            flex: c == 'ИГРОК' ? (compact ? 5 : 4) : (c == 'МЕТКА' ? 2 : 1),
            child: Center(
                child: Text(c,
                    style: const TextStyle(
                        color: _OF.text,
  ...
  Widget _teamTableRowFromPlayer(TrackerPlayerOption p, int i,
    ...
    return _teamTableRow(
      ...
      cod: math.max(session?.changeOfDirectionCount ?? 0,
  ...
  Widget _teamTableRowFromSession(TrackerLiveSessionModel s, int i,
    ...
    return _teamTableRow(
      ...
      cod: math.max(
          s.changeOfDirectionCount, local?.changeOfDirectionCount ?? 0),
      heartRateBpm: _heartRateForPlayerId(option.id)?.bpm,
      compact: compact,
      highlight: widget.selectedPlayer?.id == option.id,
      monitoring: _isPlayerInMonitorGrid(option.id),
        ...
        for (var i = 0; i < values.length; i++)
          Expanded(
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(
                  border: Border(
                      bottom: BorderSide(color: _OF.line),
                      right: BorderSide(color: _OF.line))),
              child: Text(
                values[i],
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
          ...
          child: _NoHoverTap(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  border: Border.all(
                      color: highlighted ? _OF.green : _OF.greenBorder)),
              child: Icon(highlighted ? Icons.check_rounded : Icons.add_rounded,
                  size: 15, color: highlighted ? Colors.white : _OF.green),
            ),
          ),
        ),
      );
    ...
    return Container(
      ...
      child: Column(children: [
        Container(
          height: 38,
          padding: const EdgeInsets.symmetric(horizontal: 10),
          decoration: const BoxDecoration(color: _OF.soft),
          child: Row(children: [
            Expanded(
              child: Text(
                title,
    ...
    for (var i = 0; i < players.length; i++) {
      ...
      if (history.isNotEmpty) {
        series.add(_LiveMetricSeries(
            label: _compactPlayerName(p.name),
            color: _chartColor(i),
            points: history));
      }
    }
...
class _LiveLineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double yOf(_LiveMetricPoint p) =>
        chart.bottom -
        ((p.value.clamp(yMin, yMax).toDouble() - yMin) / (yMax - yMin)) *
            chart.height;
...
// Светлая CMR-подложка без чёрного фона вокруг поля.
  ...
  void _drawRealPitch(Canvas canvas, Rect pitch) {
    ...
    canvas.drawLine(
        Offset(centerX, inner.top), Offset(centerX, inner.bottom), thin);
    canvas.drawCircle(inner.center, inner.height * .135, thin);
    canvas.drawCircle(inner.center, math.max(2.3, inner.width * .004),
    ...
    final leftPenalty = Rect.fromLTWH(
        inner.left, inner.center.dy - penaltyH / 2, penaltyW, penaltyH);
    final rightPenalty = Rect.fromLTWH(inner.right - penaltyW,
        inner.center.dy - penaltyH / 2, penaltyW, penaltyH);
    final leftBox =
    ...
    canvas.drawCircle(Offset(inner.left + spotOffset, inner.center.dy), 2.2,
        Paint()..color = Colors.white.withOpacity(.82));
    canvas.drawCircle(Offset(inner.right - spotOffset, inner.center.dy), 2.2,
        Paint()..color = Colors.white.withOpacity(.82));
    ...
    final goalPaint = Paint()
      ...
      ..style = PaintingStyle.stroke;
    canvas.drawRect(
        Rect.fromLTWH(inner.left - goalDepth, inner.center.dy - goalH / 2,
            goalDepth, goalH),
        goalPaint);
    canvas.drawRect(
        Rect.fromLTWH(
            inner.right, inner.center.dy - goalH / 2, goalDepth, goalH),
        goalPaint);

    final cornerR = inner.height * .035;
    final cornerPaint = thin;
    canvas.drawArc(Rect.fromCircle(center: inner.topLeft, radius: cornerR), 0,
        math.pi / 2, false, cornerPaint);
    canvas.drawArc(Rect.fromCircle(center: inner.topRight, radius: cornerR),
        math.pi / 2, math.pi / 2, false, cornerPaint);
    canvas.drawArc(Rect.fromCircle(center: inner.bottomRight, radius: cornerR),
        math.pi, math.pi / 2, false, cornerPaint);
    canvas.drawArc(Rect.fromCircle(center: inner.bottomLeft, radius: cornerR),
        -math.pi / 2, math.pi / 2, false, cornerPaint);
...
// Боковые технические зоны — визуально похоже на реальное поле в аналитических программах.
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(
            inner.left + inner.width * .31,
            inner.top - pitch.height * .035,
            inner.width * .14,
            pitch.height * .025),
...
class _OF {
  ...
  static const Color greenSoftStrong = Color(0xFFE7F6ED);
