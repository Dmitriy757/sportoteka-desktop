...
class _TrackerMatchWorkspaceScreenState
    ...
    switch (section) {
      ...
      case TrackerWorkspaceSection.analytics:
        return 'Проанализируй выбранную тренировку $playerScope как единый контекст. '
            'Сначала разреши все session_ids как одну логическую тренировку и проверь качество GPS/Polar. '
            'Сопоставь дистанцию, скорость, спринты, ускорения/торможения, нагрузку и пульс. '
            'Если одного канала нет, продолжай по доступным данным и честно обозначь ограничение. '
            'Покажи: качество данных, что происходит, почему, кому уделить внимание и подробный план следующей тренировки. '
            'После анализа продолжай диалог в этом же контексте и отвечай на уточнения тренера.';
  ...
  Map<String, dynamic> _trackerContextAiPayload(
    ...
    return <String, dynamic>{
      ...
      if (analytics) 'response_depth': 'deep_coach_dialogue',
    };
  }
    ...
    if (widget.embeddedInClubWorkspace) {
      return Theme(
        ...
        child: WillPopScope(
          ...
          child: SafeArea(
            // На части Android-планшетов Club Workspace сохраняет верхний
            // системный inset. Без SafeArea белая шапка аналитики заходила
            // под него и визуально обрезалась.
            bottom: false,
            child: _withTrackerContextAiLayer(
  ...
  Widget _devices() {
    ...
    Future<void> startMobileAssignment(int tab) async {
      if (_selectedPlayer == null) {
        await openMobilePlayerPickerSheet();
      }
      if (!mounted || _selectedPlayer == null) return;
      await openMobileDevicePickerSheet(tab: tab);
    }
        ...
        child: Container(
          ...
          child: Row(children: [
            connectChip(
                ...
                caption: 'трекер',
                primary: true,
                onTap: () => unawaited(startMobileAssignment(0))),
            const SizedBox(width: 8),
            connectChip(
                icon: Icons.favorite_rounded,
                title: 'Polar',
                caption: 'пульс',
                primary: false,
                onTap: () => unawaited(startMobileAssignment(1))),
            const SizedBox(width: 8),
            iconChip(
                icon: Icons.tune_rounded,
                label: 'Управление',
    ...
    Widget mobileSetupProgress() {
      final player = _selectedPlayer;
      final playerReady = player != null;
      final gpsReady =
          player != null && _gpsDevicesForPlayer(player.id).isNotEmpty;
      final polarReady = player != null &&
          (_heartDevicePlayerIds.values.contains(player.id) ||
              _savedDevices.any((device) =>
                  device.playerId == player.id &&
                  _isHeartRateDeviceModel(device)));
      ...
      Widget step({
        ...
        final color = done || current ? _TD.green : _TD.muted;
        return Expanded(
          child: Container(
            height: 54,
            ...
            decoration: BoxDecoration(
              color: done
                  ? _TD.greenSoft
                  : (current ? Colors.white : _TD.soft),
              borderRadius: BorderRadius.circular(_TD.interactiveRadius),
              border: Border.all(
                color: done || current ? _TD.greenBorder : _TD.border,
                width: .8,
              ),
            ),
            child: Row(children: [
              Container(
                ...
                alignment: Alignment.center,
                decoration: BoxDecoration(
                  color: done ? _TD.green : Colors.white,
                  shape: BoxShape.circle,
                  border: done ? null : Border.all(color: color, width: 1.2),
                ),
                child: done
      ...
      final complete = playerReady && gpsReady && polarReady;
      return Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Row(children: [
            ...
            step(
                ...
                current: playerReady && !gpsReady),
            const SizedBox(width: 6),
            step(
                number: 3,
                label: 'Polar',
                done: polarReady,
                current: playerReady && gpsReady && !polarReady),
          ]),
        ]),
      );
    }

    Widget desktopSetupProgress({required bool compact}) {
      final player = _selectedPlayer;
      final playerReady = player != null;
      final gpsReady =
          player != null && _gpsDevicesForPlayer(player.id).isNotEmpty;
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(value,
                          ...
                          overflow: TextOverflow.ellipsis,
                          style: TextStyle(
                              color: done ? _TD.greenDark : _TD.muted,
                              fontSize: compact ? 9.3 : 10.0,
                              fontWeight: FontWeight.w600)),
                    ],
                  ),
                ),
      ...
      return Container(
        ...
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(_TD.tabletCardRadius),
          border: Border.all(color: _TD.border.withOpacity(.5), width: .7),
        ),
        child: Row(children: [
          step(
            number: 1,
            title: 'Игрок',
            value: player?.name ?? 'выберите в списке',
            done: playerReady,
            current: !playerReady,
            onTap: null,
          ),
          ...
          step(
            number: 2,
            title: 'GPS-трекер',
            value: gpsReady ? 'назначен' : 'найдите и назначьте',
            done: gpsReady,
            current: playerReady && !gpsReady,
            onTap: () {
              setState(() => _deviceWorkspaceTab = 0);
              if (!_scanning) _scan(universalMode: true);
            },
          ),
          const SizedBox(width: 7),
          step(
            number: 3,
            title: 'Polar H10',
            value: polarReady ? 'назначен' : 'найдите и назначьте',
            done: polarReady,
            current: playerReady && gpsReady && !polarReady,
            onTap: () {
              setState(() => _deviceWorkspaceTab = 1);
              if (!_scanningHeart) _scanHeartRate();
            },
          ),
          const SizedBox(width: 7),
          Expanded(
            child: _NoHoverTap(
              ...
              child: Container(
                ...
                padding: const EdgeInsets.symmetric(horizontal: 10),
                decoration: BoxDecoration(
                    color: complete ? _TD.green : _TD.greenSoft,
                    borderRadius:
                        BorderRadius.circular(_TD.interactiveRadius),
                    border: Border.all(
                        color: complete ? _TD.green : _TD.greenBorder,
                        width: .8)),
                child: Row(children: [
                  Icon(
                      complete
                      ...
                      color: complete ? Colors.white : _TD.green,
                      size: 19),
                  const SizedBox(width: 7),
                  Expanded(
                    child: Column(
                      ...
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(complete ? 'Открыть Live' : 'Следующий шаг',
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                            style: TextStyle(
                                color:
                                    complete ? Colors.white : _TD.greenDark,
                                fontSize: compact ? 10.7 : 11.3,
                                fontWeight: FontWeight.w700)),
                        const SizedBox(height: 2),
                        Text(
    ...
    return _DarkPage(
      ...
      child: LayoutBuilder(builder: (context, constraints) {
        ...
        return Column(children: [
          desktopSetupProgress(compact: tablet),
  ...
  Widget _field() {
    ...
    Widget fieldSetupGuide({required bool compact}) {
      final fieldReady = _selectedField != null;
      final gpsReady = _hasAnyGpsCommandChannel;
      final cornersReady = visibleCorners.length >= 4;
      final savedReady = showingSavedCalibration ||
          (_selectedField?.hasCalibration == true && savedCorners.length >= 4);
      ...
      IconData nextIcon;
      if (!fieldReady) {
        nextAction = _createNewFieldDraft;
        actionLabel = 'Создать поле';
        nextIcon = Icons.add_location_alt_rounded;
      } else if (!gpsReady) {
        nextAction =
            () => _openTrackerSection(TrackerWorkspaceSection.devices);
        actionLabel = 'Подключить GPS';
        nextIcon = Icons.sensors_rounded;
      } else if (savedReady) {
        nextAction = () => _openTrackerSection(TrackerWorkspaceSection.live);
        actionLabel = 'Открыть Live';
        nextIcon = Icons.play_arrow_rounded;
      } else if (cornersReady) {
        nextAction = _saveCapturedField;
        actionLabel = 'Сохранить поле';
        nextIcon = Icons.save_rounded;
      } else {
        nextAction = _captureCalibrationPoint;
        actionLabel = _calibrationCapturing
            ? 'Ожидаю GPS'
            : 'Записать точку $nextLabel';
        nextIcon = Icons.gps_fixed_rounded;
      }
      ...
      return Container(
        ...
        child: Row(children: [
          Expanded(
            child: SingleChildScrollView(
              ...
              step(
                  number: 1,
                  title: 'Выберите поле',
                  value: _selectedField?.title ?? 'не выбрано',
                  done: fieldReady,
                  current: !fieldReady,
                  onTap: fieldReady ? null : _createNewFieldDraft),
              const SizedBox(width: 7),
              step(
                  number: 2,
                  title: 'Подключите GPS',
                  value: gpsReady
                      ? '${_teamBlePool.connectedCount} подключено'
                      : 'перейти к датчикам',
                  done: gpsReady,
                  current: fieldReady && !gpsReady,
                  onTap: () =>
                      _openTrackerSection(TrackerWorkspaceSection.devices)),
              const SizedBox(width: 7),
              step(
                  number: 3,
                  title: 'Пройдите A/B/C/D',
                  value: '${visibleCorners.length}/4 точек',
                  done: cornersReady,
                  current: fieldReady && gpsReady && !cornersReady,
                  onTap: fieldReady && gpsReady && !cornersReady
                      ? _captureCalibrationPoint
                      : null),
              const SizedBox(width: 7),
              step(
                  number: 4,
                  title: 'Сохраните',
                  value: savedReady ? 'готово к Live' : 'после 4 точек',
                  done: savedReady,
                  current: cornersReady && !savedReady,
                  onTap: cornersReady && !savedReady
                      ? _saveCapturedField
                      : null),
              ]),
            ),
          ...
          _DarkActionButton(
            ...
            label: actionLabel,
            primary: true,
            onTap: _calibrationCapturing && !savedReady ? null : nextAction,
          ),
        ]),
      );
    }
    ...
    return _DarkPage(
      title: 'Поле',
      subtitle:
          'Подготовка карты для Live: A верхний левый → B верхний правый → C нижний правый → D нижний левый',
      icon: Icons.map_rounded,
      trailing: _TrackerToolbarScroller(children: [
        _DarkActionButton(
            icon: Icons.add_rounded,
      ...
      child: LayoutBuilder(builder: (context, constraints) {
        ...
        return Column(children: [
          fieldSetupGuide(compact: tablet),
          const SizedBox(height: 8),
          Expanded(
            child: Row(children: [
              Expanded(flex: tablet ? 5 : 4, child: fieldsCard()),
              const _WorkspacePaneDivider.vertical(),
              Expanded(
                  flex: tablet ? 7 : 8,
                  child: calibrationCard(compact: tablet)),
            ]),
          ),
        ]);
      }),
    ...
    showModalBottomSheet<void>(
      ...
      builder: (sheetContext) {
        ...
        final maxHeight = math
            .min(MediaQuery.of(sheetContext).size.height * .94, 900.0)
            .toDouble();
    ...
    if (isMenu) {
      return Tooltip(
        ...
        child: GestureDetector(
          ...
          onTap: onTap,
          child: Container(
            constraints: BoxConstraints(minWidth: label == null ? 36 : 72),
            height: 36,
            alignment: Alignment.center,
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(icon, color: _TD.text, size: 18),
                if (label != null) ...[
                  const SizedBox(width: 6),
                  Text(
                    label!,
