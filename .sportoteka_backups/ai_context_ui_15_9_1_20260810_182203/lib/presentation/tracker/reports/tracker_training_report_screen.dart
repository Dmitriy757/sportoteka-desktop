...
class _TrackerTrainingReportScreenState extends State<TrackerTrainingReportScreen> {
  ...
  bool _sameRequestedSessions(TrackerTrainingReportScreen other) {
    ...
    final previous = <int>{other.sessionId, ...other.sessionIds.where((id) => id > 0)};
    return current.length == previous.length && current.containsAll(previous);
  }

  TrackerTrainingReport _withExpectedParticipants(TrackerTrainingReport report) {
    final expectedIds = widget.expectedParticipantIds.where((id) => id > 0).toSet();
          ...
          if (_show('summary'))
            Wrap(
              ...
              children: [
                ...
                metric(Icons.monitor_heart_rounded, 'Нагрузка', summary.playerLoad.toStringAsFixed(0), 'нагрузка'),
              ],
            ),
...
''');
  ...
  static String _playersTable(List<TrackerTrainingPlayerRow> players) {
    ...
    const headers = ['Игрок', 'Дист.', 'м/мин', 'Avg', 'Max', 'V3', 'V4', 'V5', 'Спр.', 'Уск.', 'Торм.', 'Нагрузка', 'Avg HR', 'Max HR', 'GPS'];
...
class _PerformanceMatrix extends StatelessWidget {
  ...
  TableRow _matrixHeaderRow() => _tableRow([
    'Игрок','Время','Дист. м','м/мин','Макс.','Нагрузка',
