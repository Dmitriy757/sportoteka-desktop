...
class _CmrClubAiAssistantPanelState extends State<CmrClubAiAssistantPanel> {
  ...
  final GlobalKey _latestAnswerAnchor = GlobalKey();
  ...
  Future<void> _ask([String? forced]) async {
    ...
    // Клавиатура появляется только после явного нажатия в поле ввода.
    // После отправки освобождаем место под ответ и не возвращаем фокус сами.
    _focus.unfocus();
    _scrollToBottom();
  ...
  void _scrollLatestAnswerToTop() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      final answerContext = _latestAnswerAnchor.currentContext;
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isLatestAnswer = !_sending &&
              index == _messages.length - 1 &&
              msg.role == _AiRole.assistant;
          return isLatestAnswer
              ? KeyedSubtree(key: _latestAnswerAnchor, child: bubble)
              : bubble;
        },
      ),
    );
