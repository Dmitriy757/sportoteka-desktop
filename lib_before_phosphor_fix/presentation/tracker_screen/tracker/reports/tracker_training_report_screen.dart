...
class _ReportExportSection {
  const _ReportExportSection({required this.id, required this.title, required this.note, required this.icon});
  final String id;
  final String title;
  final String note;
  final IconData icon;
...
const List<_ReportExportSection> _reportExportSections = [
  _ReportExportSection(id: 'summary', title: 'Сводка', note: 'KPI, статус, данные сессии', icon: Icons.dashboard_rounded),
  _ReportExportSection(id: 'players', title: 'Игроки', note: 'состав, аватары, таблицы', icon: Icons.groups_rounded),
  _ReportExportSection(id: 'locomotor', title: 'Локомоторика', note: 'дистанция, скорость, зоны', icon: Icons.directions_run_rounded),
  _ReportExportSection(id: 'maps', title: 'Карта и heatmap', note: 'траектория, тепловая карта', icon: Icons.map_rounded),
  _ReportExportSection(id: 'speed', title: 'Скорость', note: 'график, HIR, спринты', icon: Icons.speed_rounded),
  _ReportExportSection(id: 'mechanics', title: 'Механика', note: 'ускорения и торможения', icon: Icons.compare_arrows_rounded),
  _ReportExportSection(id: 'internal', title: 'Пульс / нагрузка', note: 'Polar H10, зоны ЧСС', icon: Icons.favorite_rounded),
  _ReportExportSection(id: 'microcycle', title: 'Микроцикл', note: 'динамика и сравнение', icon: Icons.calendar_month_rounded),
  _ReportExportSection(id: 'player_pages', title: 'Листы игроков', note: 'карта + график по каждому', icon: Icons.badge_rounded),
  _ReportExportSection(id: 'ai', title: 'ИИ анализ', note: 'выводы для тренера', icon: Icons.auto_awesome_rounded),
];

Set<String> _defaultReportExportSectionIds() => _reportExportSections.map((e) => e.id).toSet();
List<String> _normalizeReportExportSections(Set<String> selected) {
  final ids = selected.isEmpty ? _defaultReportExportSectionIds() : Set<String>.from(selected);
...
String _exportSectionsLabel(Set<String> selected) {
  final count = selected.isEmpty ? _reportExportSections.length : selected.length;
  if (count >= _reportExportSections.length) return 'Все разделы';
  if (count == 1) {
    final id = selected.first;
    return _reportExportSections.firstWhere((s) => s.id == id, orElse: () => _reportExportSections.first).title;
  }
...
class _TrackerTrainingReportScreenState extends State<TrackerTrainingReportScreen> {
  ...
  final Set<String> _selectedExportSectionIds = _defaultReportExportSectionIds();
  bool _autoSelectionOpened = false;

  List<String> _activeExportSections() => _normalizeReportExportSections(_selectedExportSectionIds);

  String _activeExportSectionsLabel() => _exportSectionsLabel(_selectedExportSectionIds);
  ...
  List<int> _selectedRealPlayerIds() => _selectedReportPlayerIds.where((id) => id > 0).toList(growable: false);

  List<TrackerTrainingPlayerRow> _reportFilterSourcePlayers(TrackerTrainingReport report) {
    final base = report.players.isNotEmpty ? report.players : report.diagnosticPlayers;
    final sessionPlayers = base.where(_isReportSessionPlayer).toList(growable: false);
    return sessionPlayers.isNotEmpty ? sessionPlayers : base.toList(growable: false);
  }
  ...
  void _toggleReportPlayerKey(int key, List<int> allKeys) {
    setState(() {
      ...
      if (_selectedReportPlayerIds.length >= allKeys.length) _selectedReportPlayerIds.clear();
    });
  }
  ...
  TrackerReportSummary _summaryFromFilteredPlayers(List<TrackerTrainingPlayerRow> rows, TrackerReportSummary fallback) {
    if (rows.isEmpty) return fallback;
    final moving = rows.where((p) => p.distanceM > 0 || p.maxSpeedKmh > 0 || p.pointsCount > 0 || p.heartRateSamplesCount > 0).toList(growable: false);
    ...
    final hrAvg = hrWeight <= 0 ? 0.0 : hrPlayers.fold<double>(0, (a, p) => a + p.heartRateAvgBpm * (p.heartRateSamplesCount > 0 ? p.heartRateSamplesCount : 1)) / hrWeight;
    return TrackerReportSummary(
      ...
      heartRateMaxBpm: hrPlayers.fold<double>(0, (m, p) => math.max(m, p.heartRateMaxBpm)),
      heartRateSamplesCount: hrPlayers.fold<int>(0, (a, p) => a + p.heartRateSamplesCount),
    );
  }
  ...
  void initState() {
    super.initState();
    _selectedReportPlayerIds.addAll(widget.initialSelectedPlayerIds.where((id) => id > 0));
    _api = widget.api ?? TrackerTrainingReportApi();
    _future = _api.loadTrainingReport(sessionId: widget.sessionId, teamId: widget.teamId, rosterPlayers: widget.rosterPlayers);
  }

  @override
  void didUpdateWidget(covariant TrackerTrainingReportScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    final selectionChanged = oldWidget.initialSelectedPlayerIds.length != widget.initialSelectedPlayerIds.length ||
        !oldWidget.initialSelectedPlayerIds.containsAll(widget.initialSelectedPlayerIds);
    if (oldWidget.sessionId != widget.sessionId || selectionChanged) {
  ...
  Uri _pdfUriForReport(TrackerTrainingReport report, {List<String>? sections}) {
    ...
    if (kIsWeb) return _PrintableTrainingReportHtml.buildUri(report, sections: selectedSections);
    return _api.pdfExportUri(
      ...
      includeHeatmap: selectedSections.contains('heatmap') || selectedSections.contains('maps'),
      includeCharts: selectedSections.any((s) => s == 'speed' || s == 'internal' || s == 'hr' || s == 'mechanics' || s == 'microcycle'),
  ...
  TrackerTrainingReport _filteredReport(TrackerTrainingReport report) {
    ...
    bool sameName(String value) => selectedNames.isNotEmpty && selectedNames.contains(value.trim().toLowerCase());
    ...
    return TrackerTrainingReport(
      ...
      summary: _summaryFromFilteredPlayers(players.isNotEmpty ? players : diagnostic, report.summary),
  ...
  Future<void> _openPlayerFilter(TrackerTrainingReport sourceReport) async {
    ...
    Widget pickerBody(BuildContext modalContext, StateSetter modalSetState, {required bool desktop}) {
      return SafeArea(
        child: SizedBox(
          width: desktop ? 390 : double.infinity,
          height: desktop ? double.infinity : MediaQuery.sizeOf(modalContext).height * .78,
          child: Material(
            color: Colors.white,
            child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              Container(
                ...
                child: Row(children: [
                  const Icon(Icons.groups_rounded, color: _R.green, size: 22),
                  const SizedBox(width: 8),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                    Text('Выбор игроков', style: _ReportType.title(16)),
                    const SizedBox(height: 3),
                    Text(temp.isEmpty ? 'В отчёт включены все игроки' : 'Выбрано игроков: ${temp.length}', style: const TextStyle(color: _R.muted, fontSize: 9.5, fontWeight: FontWeight.w700)),
                  ])),
              ...
              Padding(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: _R.soft, borderRadius: BorderRadius.circular(12)),
                  child: const Row(children: [
                    Icon(Icons.search_rounded, color: _R.muted, size: 15),
                    SizedBox(width: 8),
                    Expanded(child: Text('Выберите одного или нескольких игроков', style: TextStyle(color: _R.muted, fontSize: 11.2, fontWeight: FontWeight.w700))),
                  ]),
                ),
              ),
              Expanded(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return _NoHoverTap(
                      onTap: () => modalSetState(() {
                        ...
                        if (temp.length == players.length || temp.isEmpty) temp.clear();
                      }),
                      borderRadius: BorderRadius.circular(12),
                      child: Container(
                        constraints: const BoxConstraints(minHeight: 62),
                        padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8),
                        decoration: BoxDecoration(color: selected ? _R.softGreen : Colors.white, borderRadius: BorderRadius.circular(12)),
                        child: Row(children: [
                          _PlayerAvatar(player: player, size: 38),
                          const SizedBox(width: 8),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                            Text(player.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 13, fontWeight: FontWeight.w900)),
                            const SizedBox(height: 3),
                            Text('GPS ${player.pointsCount} · ЧСС ${player.heartRateSamplesCount} · ${player.distanceM.toStringAsFixed(0)} м', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 10, fontWeight: FontWeight.w700)),
                          ])),
                          const SizedBox(width: 8),
                          Icon(selected ? Icons.check_circle_rounded : Icons.radio_button_unchecked_rounded, color: selected ? _R.green : _R.muted),
                        ]),
                      ),
                    );
                  },
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(backgroundColor: _R.green, foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                    icon: const Icon(Icons.check_rounded),
                    label: const Text('Применить', style: TextStyle(fontWeight: FontWeight.w900)),
                  ),
                ),
              ),
            ]),
    ...
    if (desktop) {
      await showGeneralDialog<void>(
        ...
        transitionBuilder: (context, animation, secondaryAnimation, child) {
          final offset = Tween<Offset>(begin: const Offset(1, 0), end: Offset.zero).animate(CurvedAnimation(parent: animation, curve: Curves.easeOutCubic));
  ...
  Future<void> _openExportOptions(TrackerTrainingReport sourceReport, TrackerTrainingReport visibleReport) async {
    final tempSections = Set<String>.from(_selectedExportSectionIds);

    Widget buildSelector(BuildContext modalContext, StateSetter modalSetState, {required bool desktop}) {
      final sectionsLabel = _exportSectionsLabel(tempSections);
      final playersLabel = _selectedReportPlayerIds.isEmpty ? 'все игроки' : '${_selectedReportPlayerIds.length} игрок(ов)';
            ...
            child: ConstrainedBox(
              ...
              child: Column(children: [
                Container(
                  ...
                  child: Row(children: [
                    const Icon(Icons.description_rounded, color: _R.green, size: 19),
                    const SizedBox(width: 7),
                    Expanded(child: Text(title, style: _ReportType.title(14))),
                    _RoundIconButton(icon: Icons.close_rounded, onTap: () => Navigator.of(exportContext).pop()),
                  ]),
                ),
      ...
      Widget controls() => Column(
            children: [
              Container(
                ...
                child: Row(children: [
                  Container(width: 34, height: 34, decoration: BoxDecoration(color: _R.softGreen, borderRadius: BorderRadius.circular(9), border: Border.all(color: _R.greenLine)), child: const Icon(Icons.calendar_month_rounded, color: _R.green, size: 15)),
                  const SizedBox(width: 8),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                    Text('Выбор отчёта', maxLines: 1, overflow: TextOverflow.ellipsis, style: _ReportType.title(13.4).copyWith(color: _R.text)),
                    const SizedBox(height: 2),
                    Text('${visibleReport.dateLabel} · $sectionsLabel · $playersLabel', maxLines: 1, overflow: TextOverflow.ellipsis, style: _ReportType.caption(10.4).copyWith(color: _R.muted)),
                  ])),
                  _RoundIconButton(icon: Icons.close_rounded, onTap: () => Navigator.of(modalContext).pop()),
                ]),
              ),
              Container(
                ...
                decoration: const BoxDecoration(color: Colors.white, border: Border(bottom: BorderSide(color: _R.border))),
                child: Row(children: [
                  const Expanded(child: _ReportSelectorTab(icon: Icons.calendar_month_rounded, label: 'Дата и сессия', selected: true)),
                  const SizedBox(width: 6),
                  Expanded(child: _ReportSelectorTab(icon: Icons.groups_rounded, label: 'Игроки', selected: false, onTap: () async {
                    Navigator.of(modalContext).pop();
                    await _openPlayerFilter(sourceReport);
                    if (mounted) _openExportOptions(sourceReport, _filteredReport(sourceReport));
                  })),
                  const SizedBox(width: 6),
                  const Expanded(child: _ReportSelectorTab(icon: Icons.layers_rounded, label: 'Блоки отчёта', selected: false)),
                ]),
              ),
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    Row(children: [
                      const Expanded(child: Text('Что показать в предпросмотре', style: TextStyle(color: _R.text, fontSize: 12.5, fontWeight: FontWeight.w900))),
                      TextButton(onPressed: () => modalSetState(() => tempSections..clear()..addAll(_defaultReportExportSectionIds())), child: const Text('Выбрать всё')),
                    ]),
                    for (final section in _reportExportSections) ...[
                      _ReportExportOptionTile(section: section, selected: tempSections.contains(section.id), onTap: () => toggleSection(section.id)),
              ...
              Container(
                ...
                decoration: const BoxDecoration(color: Colors.white, border: Border(top: BorderSide(color: _R.border))),
                child: Row(children: [
                  Expanded(child: _ReportExportActionButton(icon: Icons.picture_as_pdf_rounded, label: 'PDF', primary: false, onTap: () {
                    openExportInModal(
                      _pdfUriForReport(_filteredReport(sourceReport), sections: _normalizeReportExportSections(tempSections)),
                      'PDF / печать',
                    );
                  })),
                  const SizedBox(width: 7),
                  Expanded(child: _ReportExportActionButton(icon: Icons.table_chart_rounded, label: 'Excel', primary: false, onTap: () {
                    openExportInModal(
                      _csvUriForReport(_filteredReport(sourceReport), sections: _normalizeReportExportSections(tempSections)),
                      'Excel / CSV отчёт',
                    );
                  })),
                  const SizedBox(width: 7),
                  Expanded(flex: 2, child: _ReportExportActionButton(icon: Icons.check_rounded, label: 'Применить', primary: true, onTap: () {
                    saveSelection();
                    ScaffoldMessenger.maybeOf(modalContext)?.showSnackBar(
                      const SnackBar(content: Text('Состав отчёта применён. Можно открыть PDF или Excel.')),
                    );
                  })),
                ]),
              ),
      ...
      if (!desktop) {
        return SafeArea(top: false, child: Container(decoration: const BoxDecoration(color: _R.bg, borderRadius: BorderRadius.vertical(top: Radius.circular(18))), child: controls()));
      }
        ...
        child: SizedBox(
          ...
          child: ClipRRect(
            ...
            child: Material(
              color: _R.bg,
              child: Row(children: [
                SizedBox(width: math.min(390.0, math.max(320.0, viewport.width * .34)), child: controls()),
                ...
                Expanded(
                  child: Column(children: [
                    Container(
                      ...
                      decoration: const BoxDecoration(color: Colors.white, border: Border(bottom: BorderSide(color: _R.border))),
                      child: Row(children: [
                        Container(width: 3, height: 20, decoration: BoxDecoration(color: _R.green, borderRadius: BorderRadius.circular(99))),
                        const SizedBox(width: 9),
                        Text('Предпросмотр отчёта', style: _ReportType.section()),
                        const Spacer(),
                        Text('Изменения применяются сразу', style: _ReportType.caption()),
                      ]),
                    ),
                    Expanded(child: _ReportModalLivePreview(report: _filteredReport(sourceReport), sections: tempSections)),
                  ]),
                ),
              ]),
            ),
    ...
    if (desktop) {
      await showDialog<void>(
        ...
        builder: (modalContext) => StatefulBuilder(builder: (context, modalSetState) => buildSelector(modalContext, modalSetState, desktop: true)),
      );
    } else {
      await showModalBottomSheet<void>(
        context: context,
        isScrollControlled: true,
        backgroundColor: Colors.transparent,
        builder: (modalContext) => StatefulBuilder(builder: (context, modalSetState) => SizedBox(height: MediaQuery.sizeOf(context).height * .94, child: buildSelector(modalContext, modalSetState, desktop: false))),
      );
    }
  }
    ...
    final content = ScrollConfiguration(
      ...
      child: FutureBuilder<TrackerTrainingReport>(
        ...
        builder: (context, snapshot) {
          ...
          if (widget.autoOpenSelection && snapshot.hasData && !_autoSelectionOpened) {
          ...
          return LayoutBuilder(builder: (context, constraints) {
            ...
            if (widget.analyticsSection != null) {
              ...
              switch (widget.analyticsSection) {
                ...
                case 'microcycle':
                  ...
                  break;
                case 'ai':
                  section = _AiAnalysisTab(report: report);
                  break;
                default:
                  section = _SummaryTab(report: report, sections: _selectedExportSectionIds);
              }
              if (snapshot.hasError) return _ErrorView(error: '${snapshot.error}', onRetry: _reload);
              if (loading && snapshot.data == null) return const Center(child: CircularProgressIndicator());
              return ColoredBox(
                color: _R.bg,
                child: Padding(
            ...
            if (widget.selectionOnly && widget.inlineAnalyticsReport) {
              ...
              if (loading && snapshot.data == null) {
                return const Center(child: CircularProgressIndicator(color: _R.green));
              }
              return Stack(
                children: [
                  _InlineAnalyticsReportWorkspace(
                    ...
                    onToggleSection: (id) {
                      setState(() {
                        if (_selectedExportSectionIds.contains(id) && _selectedExportSectionIds.length > 1) {
                    ...
                    onPdf: () => _openExport(
                      _pdfUriForReport(
                        ...
                        sections: _normalizeReportExportSections(_selectedExportSectionIds),
                      ),
                      title: 'PDF / печать',
                    ),
                    onExcel: () => _openExport(
                    ...
                    children: [
                      if (widget.inlineAnalyticsReport)
                        Container(
                          ...
                          decoration: const BoxDecoration(
                            color: Colors.white,
                            border: Border(bottom: BorderSide(color: _R.border, width: .7)),
                          ),
                          child: Row(
                            children: [
                              Expanded(
                              ...
                              TextButton(
                                onPressed: () => _openPlayerFilter(sourceReport),
                                child: const Text('Игроки'),
                              ),
                              const SizedBox(width: 4),
                              OutlinedButton(
                                onPressed: () => _openExport(_csvUriForReport(report), title: 'Excel / CSV отчёт'),
                                child: const Text('Excel'),
                              ),
                              const SizedBox(width: 6),
                              FilledButton(
                                onPressed: () => _openExport(_pdfUriForReport(report), title: 'PDF / печать'),
                                style: FilledButton.styleFrom(backgroundColor: _R.green, foregroundColor: Colors.white),
                        ...
                        _Header(
                          ...
                          onRefresh: _reload,
                          onPdf: () => _openExport(_pdfUriForReport(report), title: 'PDF / печать'),
                          onExcel: () => _openExport(_csvUriForReport(report), title: 'Excel / CSV отчёт'),
                          onPlayers: () => _openPlayerFilter(sourceReport),
                          onSettings: () => _openExportOptions(sourceReport, report),
                      ...
                      if (snapshot.hasError)
                        ...
                        Expanded(child: _SummaryTab(report: report, sections: _selectedExportSectionIds)),
                    ],
                  ),
                  if (_exportPreviewUri != null)
                    Positioned.fill(
              ...
              children: [
                if (mobile)
                  _MobileReportShell(
                    ...
                    onPdf: () => _openExport(_pdfUriForReport(report), title: 'PDF / печать'),
                    onExcel: () => _openExport(_csvUriForReport(report), title: 'Excel / CSV отчёт'),
                    onExportSettings: () => _openExportOptions(sourceReport, report),
  ...
  Widget _buildTab(TrackerTrainingReport report) {
    switch (_tab) {
      ...
      case 5:
        return _AiAnalysisTab(report: report);
      default:
        return _SummaryTab(report: report, sections: _selectedExportSectionIds);
    }
  }
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final sidebarWidth = math.min(390.0, math.max(310.0, constraints.maxWidth * .34));
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Предпросмотр отчёта',
                      style: _ReportType.section().copyWith(fontWeight: FontWeight.w600),
                    ),
                  ),
                  Text(
        ...
        return ColoredBox(
          ...
          child: mobile
              ? Column(
                  children: [
                    SizedBox(height: math.min(420, constraints.maxHeight * .48), child: selector),
              ...
              : Row(
                  ...
                  children: [
                    SizedBox(width: compact ? math.min(340, sidebarWidth) : sidebarWidth, child: selector),
                    Expanded(child: preview),
                  ],
                ),
        );
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              Text(
                'Отчёт',
                style: _ReportType.title(15).copyWith(fontWeight: FontWeight.w600),
              ),
              const SizedBox(height: 3),
              Text(
                playersLabel,
            ...
            children: [
              Row(
                children: [
                  ...
                  TextButton(onPressed: onPlayers, child: const Text('Выбрать')),
                ],
              ),
              playerFilter,
              const SizedBox(height: 8),
              Row(
                children: [
                  ...
                  TextButton(onPressed: onSelectAllSections, child: const Text('Все')),
                ],
              ),
...
class _ReportSelectorTab extends StatelessWidget {
  const _ReportSelectorTab({required this.icon, required this.label, required this.selected, this.onTap});
  final IconData icon;
  final String label;
  final bool selected;
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(9), border: Border.all(color: selected ? _R.greenLine : _R.border)),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(icon, size: 15, color: selected ? _R.greenDark : _R.graphite),
            const SizedBox(width: 6),
            Flexible(child: Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: _ReportType.action().copyWith(color: selected ? _R.greenDark : _R.text, fontSize: 10.6))),
          ]),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Container(width: 3, height: 16, decoration: BoxDecoration(color: _R.green, borderRadius: BorderRadius.circular(99))), const SizedBox(width: 8), Text('Выбранная тренировка', style: _ReportType.section())]),
        const SizedBox(height: 9),
        Text(report.title.isEmpty ? 'Тренировка' : report.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: _ReportType.title(14)),
        const SizedBox(height: 4),
        Text('${report.dateLabel} · ${report.durationLabel} · ${report.teamName}', maxLines: 2, overflow: TextOverflow.ellipsis, style: _ReportType.caption(10.5)),
        const SizedBox(height: 9),
        Wrap(spacing: 6, runSpacing: 6, children: [
          _ReportMiniPill(icon: Icons.groups_rounded, text: '${report.playersCount} игроков'),
          _ReportMiniPill(icon: Icons.gps_fixed_rounded, text: '${report.pointsCount} GPS'),
          _ReportMiniPill(icon: Icons.favorite_rounded, text: '${report.summary.heartRateSamplesCount} HR'),
        ]),
      ]),
    );
  }
...
class _ReportMiniPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(color: _R.softGreen, borderRadius: BorderRadius.circular(999), border: Border.all(color: _R.greenLine)),
        child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 13, color: _R.greenDark), const SizedBox(width: 5), Text(text, style: const TextStyle(color: _R.greenDark, fontSize: 9.0, fontWeight: FontWeight.w500))]),
      );
}
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, c) {
        ...
        final width = tablet ? c.maxWidth - margin * 2 : math.min(1220.0, c.maxWidth - margin * 2);
          ...
          child: SizedBox(
            ...
            child: Container(
              decoration: BoxDecoration(
                ...
                boxShadow: const [BoxShadow(color: Color(0x16000000), blurRadius: 18, offset: Offset(0, 18))],
              ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const Text('PDF/CSV открывается внутри окна или во внешнем приложении', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.muted, fontSize: 10.4, fontWeight: FontWeight.w600)),
                            ],
                          ),
                        ),
                      ],
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: _R.bg,
          borderRadius: BorderRadius.vertical(top: Radius.circular(_R.sheetRadius), bottom: Radius.circular(18)),
        ),
        child: Column(
          children: [
            Container(
            ...
            Container(
              ...
              decoration: const BoxDecoration(color: Colors.white, border: Border(bottom: BorderSide(color: _R.border))),
              child: Row(children: [
                Expanded(child: Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: _ReportType.title(14))),
                const SizedBox(width: 7),
  ...
  Widget build(BuildContext context) {
    if (error != null) {
      return Container(
        color: _R.bg,
        child: ListView(
          padding: EdgeInsets.fromLTRB(_R.mobilePagePadding, 8, _R.mobilePagePadding, 22),
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          _DzenReportPlayersCard(report: report, selectedPlayersCount: selectedPlayersCount, onPlayers: onPlayers),
          const SizedBox(height: 8),
          _DzenReportSummaryCard(
            report: report,
            onOpenMap: () => onOpenDetail('Карта активности / тепловая карта', _ActivityMapPanel(report: report)),
            onOpenHr: () => onOpenDetail('Пульс команды', SizedBox(height: 460, child: _HeartRateTimelineChart(report: report))),
            onOpenPlayers: () => onOpenDetail('Игроки и локомоторика', _MobilePlayersDetails(report: report)),
          ),
          const SizedBox(height: 8),
          _DzenReportModulesCard(report: report, onOpenDetail: onOpenDetail),
          const SizedBox(height: 8),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Отчёты', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.text, fontSize: 14.8, fontWeight: FontWeight.w900, letterSpacing: -.1)),
                const SizedBox(height: 2),
                Text('$team · ${_mobileReportSessionLine(report)}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 9.5, fontWeight: FontWeight.w700)),
              ],
            ),
          ),
          if (loading) ...[
            const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(color: _R.green, strokeWidth: 2)),
            const SizedBox(width: 7),
          ],
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, c) {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Отчёт по тренировке', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.text, fontSize: 15, fontWeight: FontWeight.w900, letterSpacing: -.2)),
                const SizedBox(height: 5),
                Text(_mobileReportSessionLine(report), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.graphite, fontSize: 11.5, fontWeight: FontWeight.w700)),
              ],
            ),
          ),
        ],
      ...
      return _DzenEdgeCard(
        ...
        child: compact
            ? Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [title, const SizedBox(height: 8), Align(alignment: Alignment.centerRight, child: _DzenPdfButton(onTap: onPdf))])
            : Row(children: [Expanded(child: title), const SizedBox(width: 7), _DzenPdfButton(onTap: onPdf)]),
      );
    });
  }
}
...
class _DzenSelectedSessionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final metrics = [
      _DzenMetric(Icons.route_rounded, _mobileReportMeters(_mobileReportDistance(report)), 'Дистанция'),
      _DzenMetric(Icons.speed_rounded, _mobileReportSpeed(report), 'Макс. скорость'),
    ...
    return _DzenEdgeCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          const _DzenCardTitle(icon: Icons.calendar_month_rounded, title: 'Выбранная сессия'),
          const SizedBox(height: 4),
          SizedBox(
            height: 62,
            child: Row(
              children: [
                for (var i = 0; i < metrics.length; i++) ...[
                  ...
                  if (i != metrics.length - 1) Container(width: 1, height: 34, color: _R.border),
                ],
              ],
            ),
          ),
...
class _DzenReportPlayersCard extends StatelessWidget {
  const _DzenReportPlayersCard({required this.report, required this.selectedPlayersCount, required this.onPlayers});
  final TrackerTrainingReport report;
  final int selectedPlayersCount;
  final VoidCallback onPlayers;
  ...
  Widget build(BuildContext context) {
    ...
    final label = selectedPlayersCount > 0 ? 'Выбрано $selectedPlayersCount' : 'Все игроки';
    return _DzenEdgeCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Игроки для выгрузки', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.text, fontSize: 15.2, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 2),
                  Text(label, style: TextStyle(color: _R.muted, fontSize: 10.4, fontWeight: FontWeight.w800)),
                ]),
              ),
          ...
          const SizedBox(height: 8),
          if (visible.isEmpty)
            const Text('Игроки появятся после загрузки сессии.', style: TextStyle(color: _R.muted, fontSize: 10.2, fontWeight: FontWeight.w500))
          else
            Row(
              children: [
                ...
                if (players.length > visible.length)
                  Container(
                    ...
                    height: 46,
                    alignment: Alignment.center,
                    decoration: BoxDecoration(color: _R.soft, shape: BoxShape.circle, border: Border.all(color: _R.border)),
                    child: Text('+${players.length - visible.length}', style: const TextStyle(color: _R.graphite, fontSize: 12, fontWeight: FontWeight.w900)),
                  ),
              ],
            ),
        ],
...
class _DzenReportExportCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final playerLabel = selectedPlayersCount > 0 ? '$selectedPlayersCount игрок(ов)' : 'вся команда';
    return _DzenEdgeCard(
        ...
        children: [
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Выбор отчёта', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.text, fontSize: 15.2, fontWeight: FontWeight.w900)),
              const SizedBox(height: 2),
              Text('$sectionsLabel · $playerLabel', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.muted, fontSize: 10.4, fontWeight: FontWeight.w800)),
            ])),
            _DzenOutlineButton(label: 'Настроить', onTap: onSettings),
          ]),
          const SizedBox(height: 8),
          Row(children: [
            Expanded(child: _ReportExportActionButton(icon: Icons.picture_as_pdf_rounded, label: 'PDF', primary: true, onTap: onPdf)),
            const SizedBox(width: 7),
            Expanded(child: _ReportExportActionButton(icon: Icons.table_chart_rounded, label: 'Excel', primary: false, onTap: onExcel)),
            const SizedBox(width: 7),
            Expanded(child: _ReportExportActionButton(icon: Icons.groups_rounded, label: 'Игроки', primary: false, onTap: onPlayers)),
          ]),
        ],
      ),
    );
...
class _ReportExportActionButton extends StatelessWidget {
  const _ReportExportActionButton({required this.icon, required this.label, required this.primary, required this.onTap});
  final IconData icon;
  final String label;
  final bool primary;
  final VoidCallback onTap;
    ...
    return _NoHoverTap(
      ...
      child: Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.center, mainAxisSize: MainAxisSize.min, children: [
          ...
          Flexible(child: Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: _ReportType.action(primary: primary))),
        ]),
      ),
    );
  }
...
class _ReportExportOptionTile extends StatelessWidget {
  const _ReportExportOptionTile({required this.section, required this.selected, required this.onTap});
            ...
            children: [
              ...
              AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: selected ? null : Border.all(color: _R.border, width: .8),
                ),
                alignment: Alignment.center,
                child: selected
...
class _DzenReportModulesCard extends StatelessWidget {
  const _DzenReportModulesCard({required this.report, required this.onOpenDetail});
  final TrackerTrainingReport report;
  final void Function(String title, Widget child) onOpenDetail;
  ...
  Widget build(BuildContext context) {
    final modules = [
      _DzenReportModule(Icons.dashboard_rounded, 'Сводка', 'KPI, карты, игроки', () => onOpenDetail('Сводка отчёта', _MobileSummaryDetails(report: report))),
      _DzenReportModule(Icons.directions_run_rounded, 'Локомоторика', 'таблица как на ПК', () => onOpenDetail('Локомоторика игроков', _MobilePlayersDetails(report: report))),
      _DzenReportModule(Icons.compare_arrows_rounded, 'Механика', 'ускорения / торможения', () => onOpenDetail('Механика', _MobileMechanicsDetails(report: report))),
      _DzenReportModule(Icons.favorite_rounded, 'Пульс', 'HR и зоны', () => onOpenDetail('Пульс и внутренняя нагрузка', _MobileInternalLoadDetails(report: report))),
      _DzenReportModule(Icons.calendar_month_rounded, 'Микроцикл', 'динамика нагрузки', () => onOpenDetail('Микроцикл', _MobileMicrocycleDetails(report: report))),
      _DzenReportModule(Icons.auto_awesome_rounded, 'ИИ анализ', 'выводы тренеру', () => onOpenDetail('ИИ анализ', _MobileAiDetails(report: report))),
    ];
    return _DzenEdgeCard(
      padding: const EdgeInsets.fromLTRB(16, 16, 16, 12),
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        const _DzenCardTitle(icon: Icons.apps_rounded, title: 'Все разделы как на ПК'),
        const SizedBox(height: 8),
        GridView.builder(
          shrinkWrap: true,
          physics: const NeverScrollableScrollPhysics(),
          itemCount: modules.length,
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, mainAxisExtent: 84, crossAxisSpacing: 8, mainAxisSpacing: 8),
          itemBuilder: (context, i) => _DzenReportModuleTile(module: modules[i]),
        ),
      ]),
    );
  }
...
class _DzenReportModuleTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _NoHoverTap(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 6),
        decoration: BoxDecoration(color: _R.soft, borderRadius: BorderRadius.circular(_R.mobileButtonRadius), border: Border.all(color: _R.border)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Container(width: 30, height: 30, decoration: BoxDecoration(color: _R.softGreen, borderRadius: BorderRadius.circular(9)), child: Icon(module.icon, color: _R.green, size: 16)),
            const Spacer(),
            const Icon(Icons.chevron_right_rounded, color: _R.muted, size: 20),
          ]),
          const Spacer(),
          Text(module.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 12.2, fontWeight: FontWeight.w900)),
          const SizedBox(height: 1),
          Text(module.note, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 9.3, fontWeight: FontWeight.w800)),
        ]),
      ),
    );
  }
...
class _MobileSummaryDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: 8),
      _Card(title: 'КАРТА АКТИВНОСТИ / ТЕПЛОВАЯ КАРТА', child: _ActivityMapPanel(report: report)),
      const SizedBox(height: 8),
      _Card(title: 'ГРАФИК СКОРОСТИ', child: SizedBox(height: 420, child: _ReportSpeedGraphPanel(report: report))),
      const SizedBox(height: 8),
...
class _MobileMechanicsDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final players = _reportPlayers(report);
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      _Card(title: 'УСКОРЕНИЯ И ТОРМОЖЕНИЯ', child: SizedBox(height: 420, child: _DualBarChart(players: players, first: (p) => p.accelerations.toDouble(), second: (p) => p.decelerations.toDouble(), firstLabel: 'Ускорения', secondLabel: 'Торможения'))),
      const SizedBox(height: 8),
      _Card(title: 'ВЗРЫВНЫЕ ДЕЙСТВИЯ / ПОВОРОТЫ', child: SizedBox(height: 420, child: _DualBarChart(players: players, first: (p) => p.explosiveActions.toDouble(), second: (p) => p.highSpeedActions.toDouble(), firstLabel: 'Взрывные', secondLabel: 'Повороты'))),
    ]);
  }
}
...
class _MobileMicrocycleDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      _Card(title: 'ДИНАМИКА МИКРОЦИКЛА', child: SizedBox(height: 440, child: _MicrocycleChart(points: report.microcycle))),
      const SizedBox(height: 8),
...
class _MobileAiDetails extends StatelessWidget {
  const _MobileAiDetails({required this.report});
  final TrackerTrainingReport report;

  @override
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      _Card(title: 'ИИ АНАЛИЗ / ДАННЫЕ ДЛЯ ТРЕНЕРА', child: _ReportAiPlaceholder(report: report, compact: false)),
      const SizedBox(height: 8),
      _Card(title: 'РЕЙТИНГ И СРАВНЕНИЕ ИГРОКА', child: SizedBox(height: 420, child: _RadarComparisonPanel(report: report))),
    ]);
  }
}

class _DzenReportSummaryCard extends StatelessWidget {
  const _DzenReportSummaryCard({required this.report, required this.onOpenMap, required this.onOpenHr, required this.onOpenPlayers});
  final TrackerTrainingReport report;
      ...
      child: Column(
        ...
        children: [
          const Padding(
            ...
            child: _DzenCardTitle(icon: Icons.insights_rounded, title: 'Сводка отчёта'),
          ),
          SizedBox(
            height: 244,
            child: ListView(
              ...
              children: [
                SizedBox(width: 166, child: _DzenTeamSummaryPanel(report: report, onTap: onOpenPlayers)),
                const SizedBox(width: 7),
                SizedBox(width: 278, child: _DzenMapSummaryPanel(report: report, onTap: onOpenMap)),
                const SizedBox(width: 7),
                SizedBox(width: 214, child: _DzenHrSummaryPanel(report: report, onTap: onOpenHr)),
              ],
            ),
          ),
        ],
...
class _DzenSprintCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final items = [
      ...
      _DzenPlainStat(value: _mobileReportMeters(s.sprintDistanceM), label: 'Общая дистанция'),
      _DzenPlainStat(value: _mobileReportShortDuration(report.durationLabel), label: 'Общая длительность'),
    ];
    return _DzenEdgeCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          const _DzenCardTitle(icon: Icons.directions_run_rounded, title: 'Спринты'),
          const SizedBox(height: 8),
          Row(
            children: [
              for (var i = 0; i < items.length; i++) ...[
                Expanded(child: items[i]),
                if (i != items.length - 1) Container(width: 1, height: 34, color: _R.border),
              ],
            ],
          ),
        ],
...
class _DzenRecentSessionsCard extends StatelessWidget {
  const _DzenRecentSessionsCard({required this.report, required this.onRefresh});
  final TrackerTrainingReport report;
  final VoidCallback onRefresh;
  ...
  Widget build(BuildContext context) {
    ...
    return _DzenEdgeCard(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          const _DzenCardTitle(icon: Icons.article_rounded, title: 'Последние сессии'),
          const SizedBox(height: 8),
          for (var i = 0; i < ids.length; i++)
            _DzenSessionRow(
              sessionId: ids[i],
              selected: ids[i] == report.sessionId,
              subtitle: i == 0 ? _mobileReportSessionRowSubtitle(report) : _mobileReportPreviousRowSubtitle(report, i),
              onTap: onRefresh,
            ),
        ],
      ),
...
class _DzenEdgeCard extends StatelessWidget {
  const _DzenEdgeCard({required this.child, this.padding = const EdgeInsets.all(10)});
  final Widget child;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _R.border),
        boxShadow: const [BoxShadow(color: Color(0x07000000), blurRadius: 20, offset: Offset(0, 10))],
      ),
      child: child,
    );
  }
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        const SizedBox(width: 7),
        Expanded(child: Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 14.8, fontWeight: FontWeight.w900))),
      ],
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return _NoHoverTap(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: _R.green, borderRadius: BorderRadius.circular(_R.mobileButtonRadius), boxShadow: const [BoxShadow(color: Color(0x1800A750), blurRadius: 18, offset: Offset(0, 8))]),
        child: const Row(
          mainAxisSize: MainAxisSize.min,
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            SizedBox(width: 8),
            Text('Выгрузить', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: Colors.white, fontSize: 12.2, fontWeight: FontWeight.w900)),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return _NoHoverTap(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.symmetric(horizontal: 10),
        alignment: Alignment.center,
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: _R.greenLine)),
        child: Text(label, style: const TextStyle(color: _R.greenDark, fontSize: 12.2, fontWeight: FontWeight.w900)),
      ),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return _NoHoverTap(
      ...
      child: Container(
        ...
        height: 42,
        alignment: Alignment.center,
        decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Color(0x0D000000), blurRadius: 18, offset: Offset(0, 8))]),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 5),
          Text(metric.value, maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(color: _R.text, fontSize: 12.8, fontWeight: FontWeight.w900)),
          const SizedBox(height: 1),
          Text(metric.label, maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(color: _R.graphite, fontSize: 9.3, fontWeight: FontWeight.w700)),
        ],
      ),
    );
  }
    ...
    return SizedBox(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 5),
          Text(_shortReportPlayerName(player.name), maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(color: _R.graphite, fontSize: 10, fontWeight: FontWeight.w800)),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return _NoHoverTap(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.all(8),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(_R.mobileButtonRadius), border: Border.all(color: _R.border)),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(color: _R.text, fontSize: 11.2, fontWeight: FontWeight.w900)),
            const SizedBox(height: 8),
  ...
  Widget build(BuildContext context) {
    return _DzenMiniPanel(
      ...
      child: Column(
        ...
        children: [
          _DzenSmallKpi(icon: Icons.route_rounded, value: _mobileReportMeters(_mobileReportDistance(report)), label: 'Дистанция'),
          _DzenSmallKpi(icon: Icons.speed_rounded, value: _mobileReportSpeed(report), label: 'Макс. скорость'),
          _DzenSmallKpi(icon: Icons.favorite_rounded, value: _mobileReportHr(report), label: 'Ср. пульс'),
          _DzenSmallKpi(icon: Icons.timer_rounded, value: _mobileReportShortDuration(report.durationLabel), label: 'Длительность'),
        ],
      ),
    );
  }
    ...
    return _DzenMiniPanel(
      ...
      child: Column(
        children: [
          ...
          Row(children: [
            const Text('Низкая', style: TextStyle(color: _R.muted, fontSize: 9.2, fontWeight: FontWeight.w800)),
            const SizedBox(width: 7),
            Expanded(
              child: Container(
                height: 6,
                decoration: BoxDecoration(
                  borderRadius: BorderRadius.circular(99),
                  gradient: const LinearGradient(colors: [Color(0xFF00A750), Color(0xFFF6D84B), Color(0xFFFF7A1A), Color(0xFFEF4444)]),
                ),
              ),
            ),
            const SizedBox(width: 7),
            const Text('Высокая', style: TextStyle(color: _R.muted, fontSize: 9.2, fontWeight: FontWeight.w800)),
          ]),
        ],
      ),
    );
...
class _DzenSmallKpi extends StatelessWidget {
  const _DzenSmallKpi({required this.icon, required this.value, required this.label});
  final IconData icon;
  final String value;
  final String label;
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      ...
      const SizedBox(width: 7),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 12.5, fontWeight: FontWeight.w900)),
        Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.graphite, fontSize: 9.4, fontWeight: FontWeight.w700)),
      ])),
    ]);
  }
}
...
class _DzenPlainStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 13.2, fontWeight: FontWeight.w600)),
        const SizedBox(height: 2),
        Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(color: _R.graphite, fontSize: 10, fontWeight: FontWeight.w700)),
      ],
    );
  }
}
...
class _DzenSessionRow extends StatelessWidget {
  const _DzenSessionRow({required this.sessionId, required this.subtitle, required this.selected, required this.onTap});
  final int sessionId;
  ...
  Widget build(BuildContext context) {
    return _NoHoverTap(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Container(width: 34, height: 34, decoration: BoxDecoration(color: selected ? _R.softGreen : _R.soft, borderRadius: BorderRadius.circular(10), border: Border.all(color: selected ? _R.greenLine : _R.border)), child: Icon(Icons.assignment_rounded, color: selected ? _R.green : _R.muted, size: 15)),
            const SizedBox(width: 7),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
              Text('Сессия #$sessionId', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 12.4, fontWeight: FontWeight.w900)),
              const SizedBox(height: 2),
              Text(subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.graphite, fontSize: 10, fontWeight: FontWeight.w700)),
            ])),
            Icon(Icons.chevron_right_rounded, color: selected ? _R.green : _R.muted, size: 24),
          ],
        ),
      ),
    );
...
class _DzenHrMiniChart extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final points = _reportHrTimeline(report).where((p) => p.bpm > 0).toList(growable: false);
    if (points.isEmpty) {
      return const Center(child: Text('Нет HR-данных', style: TextStyle(color: _R.muted, fontSize: 10.4, fontWeight: FontWeight.w500)));
    }
    return CustomPaint(painter: _DzenHrMiniPainter(points: points), child: const SizedBox.expand());
  }
}
...
class _DzenHrMiniPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final axis = Paint()..color = _R.border..strokeWidth = 1;
    ...
    for (final label in labels) {
      final y = bottom - ((label - 40) / 120.0).clamp(0.0, 1.0) * (bottom - top);
      canvas.drawLine(Offset(left, y), Offset(right, y), axis);
      textPainter.text = TextSpan(text: '$label', style: const TextStyle(color: _R.muted, fontSize: 8, fontWeight: FontWeight.w700));
      textPainter.layout();
      textPainter.paint(canvas, Offset(2, y - 5));
    }
    final sampled = _sampleHr(points, 80);
    if (sampled.length < 2) return;
    final minBpm = math.max(35, sampled.map((p) => p.bpm).reduce(math.min) - 8).toDouble();
    final maxBpm = math.max(minBpm + 25, sampled.map((p) => p.bpm).reduce(math.max) + 8).toDouble();
    final path = Path();
    for (var i = 0; i < sampled.length; i++) {
      final x = left + (sampled.length == 1 ? 0 : i / (sampled.length - 1)) * (right - left);
      final y = bottom - ((sampled[i].bpm - minBpm) / (maxBpm - minBpm)).clamp(0.0, 1.0) * (bottom - top);
      if (i == 0) {
        path.moveTo(x, y);
      } else {
        path.lineTo(x, y);
    ...
    textPainter.text = const TextSpan(text: 'мин', style: TextStyle(color: _R.muted, fontSize: 8, fontWeight: FontWeight.w800));
    textPainter.layout();
    textPainter.paint(canvas, Offset(right - textPainter.width, size.height - 12));
  }

  @override
  bool shouldRepaint(covariant _DzenHrMiniPainter oldDelegate) => oldDelegate.points != points;
}

List<TrackerHeartRatePoint> _sampleHr(List<TrackerHeartRatePoint> points, int maxCount) {
...
double _mobileReportDistance(TrackerTrainingReport report) {
  ...
  if (report.routePoints.isNotEmpty) {
    return report.routePoints.fold<double>(0, (a, p) => math.max(a, p.distanceM));
  }
  return 0;
}
...
String _mobileReportMeters(double value) {
  ...
  if (value >= 1000) return '${(value / 1000).toStringAsFixed(value >= 10000 ? 1 : 2)} км';
...
String _mobileReportPreviousRowSubtitle(TrackerTrainingReport report, int index) {
  final date = _mobileReportDateOnly(report.dateLabel);
  final player = report.playersCount > 0 ? '${math.max(1, report.playersCount - index)} игрока' : 'игроки не выбраны';
...
List<int> _mobileReportSessionIds(TrackerTrainingReport report) {
  ...
  if (report.sessionId > 0 && !ids.contains(report.sessionId)) ids.insert(0, report.sessionId);
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
            Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: _ReportType.title(14)),
            const SizedBox(height: 2),
...
class _ReportPlayerFilterStrip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final keys = <int>[for (var i = 0; i < players.length; i++) keyForPlayer(players[i], i)];
    return Container(
      height: 58,
      color: Colors.white,
      ...
      child: ListView(
        ...
        children: [
          _ReportPlayerQuickChip(
            ...
            leading: const Icon(Icons.groups_rounded, color: _R.green, size: 15),
            onTap: onAll,
          ),
          ...
          for (var i = 0; i < math.min(players.length, 3); i++) ...[
            _ReportPlayerQuickChip(
              ...
              note: '${players[i].distanceM.toStringAsFixed(0)} м · HR ${players[i].heartRateSamplesCount}',
          ...
          _ReportPlayerQuickChip(
            ...
            leading: const Icon(Icons.tune_rounded, color: _R.graphite, size: 15),
            onTap: onOpenFull,
          ),
        ],
      ),
...
class _ReportPlayerQuickChip extends StatelessWidget {
  const _ReportPlayerQuickChip({required this.label, required this.note, required this.active, required this.leading, required this.onTap});
  final String label;
  final String note;
  final bool active;
  ...
  Widget build(BuildContext context) {
    return _NoHoverTap(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: active ? _R.softGreen : Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: active ? _R.greenLine : _R.border)),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          leading,
          const SizedBox(width: 4),
          Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
            ConstrainedBox(constraints: const BoxConstraints(maxWidth: 150), child: Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: active ? _R.greenDark : _R.text, fontSize: 11.2, fontWeight: FontWeight.w900))),
            const SizedBox(height: 2),
            ConstrainedBox(constraints: const BoxConstraints(maxWidth: 150), child: Text(note, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 9.2, fontWeight: FontWeight.w700))),
          ]),
        ]),
      ),
    );
      ...
      child: _NoHoverTap(
        ...
        child: Container(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(width: 5),
            Flexible(child: Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: active ? _R.greenDark : _R.graphite, fontSize: 14.0, fontWeight: active ? FontWeight.w700 : FontWeight.w600))),
          ]),
        ),
      ),
    );
...
class _MobileHeroCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(8),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(_R.mobileCardRadius), border: Border.all(color: _R.border), boxShadow: const [BoxShadow(color: Color(0x07000000), blurRadius: 18, offset: Offset(0, 8))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          _TeamLogo(url: report.teamLogoUrl, title: report.teamName, size: 46),
          const SizedBox(width: 8),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Сессия #${report.sessionId}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 14, fontWeight: FontWeight.w900)),
            const SizedBox(height: 2),
            Text([report.teamName, report.dateLabel, report.durationLabel].where((v) => v.trim().isNotEmpty).join(' · '), maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 11.2, height: 1.2, fontWeight: FontWeight.w800)),
          ])),
...
class _MobileCoachCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(8),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(_R.tabletCardRadius), border: Border.all(color: _R.greenLine)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [
          Icon(Icons.tips_and_updates_rounded, color: _R.green, size: 15),
          SizedBox(width: 8),
          Text('Итог тренера', style: TextStyle(color: _R.text, fontSize: 11.8, fontWeight: FontWeight.w900)),
        ]),
        const SizedBox(height: 8),
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: _NoHoverTap(
        ...
        borderRadius: BorderRadius.circular(_R.mobileCardRadius),
        child: Container(
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(_R.mobileCardRadius), border: Border.all(color: _R.border), boxShadow: const [BoxShadow(color: Color(0x05000000), blurRadius: 14, offset: Offset(0, 7))]),
          clipBehavior: Clip.antiAlias,
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Padding(
              padding: const EdgeInsets.fromLTRB(12, 10, 8, 8),
              child: Row(children: [
                Container(width: 34, height: 34, decoration: BoxDecoration(color: _R.softGreen, borderRadius: BorderRadius.circular(12), border: Border.all(color: _R.greenLine)), child: Icon(icon, color: _R.green, size: 15)),
                const SizedBox(width: 8),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(width: 6),
                Container(width: 34, height: 34, decoration: BoxDecoration(color: _R.soft, borderRadius: BorderRadius.circular(_R.mobileInnerRadius), border: Border.all(color: _R.border)), child: const Icon(Icons.open_in_full_rounded, color: _R.graphite, size: 16)),
              ]),
            ),
        ...
        child: ClipRRect(
          ...
          child: CustomPaint(
            ...
            child: points.isEmpty
                ? const Center(child: Text('Нет GPS-точек', style: TextStyle(color: Colors.white70, fontSize: 9.5, fontWeight: FontWeight.w900)))
...
class _MobileSectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: const TextStyle(color: _R.text, fontSize: 13.2, fontWeight: FontWeight.w600)),
        const SizedBox(height: 2),
      ...
      return Container(
        ...
        child: Row(
          children: [
            ...
            _TeamLogo(url: report.teamLogoUrl, title: report.teamName, size: compact ? 28 : 32),
            const SizedBox(width: 7),
            Expanded(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                ...
                children: [
                  Text(
                    ...
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(color: _R.text, fontSize: compact ? 15.0 : 16.0, fontWeight: FontWeight.w600, letterSpacing: 0),
                  ),
                  const SizedBox(height: 1),
                  Text(
                    subtitle,
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(color: _R.muted, fontSize: compact ? 11.0 : 11.5, fontWeight: FontWeight.w500),
                  ),
                ],
              ),
            ),
            ...
            _HeaderButton(icon: Icons.tune_rounded, label: hideLabels ? 'Выбор' : 'Выбор отчёта', onTap: onSettings),
            const SizedBox(width: 7),
...
class _Tabs extends StatelessWidget {
  ...
  static const items = ['Сводка', 'Локомоторика', 'Механика', 'Пульс', 'Микроцикл', 'ИИ анализ'];
  static const icons = [Icons.dashboard_rounded, Icons.directions_run_rounded, Icons.compare_arrows_rounded, Icons.favorite_rounded, Icons.calendar_month_rounded, Icons.auto_awesome_rounded];
        ...
        itemBuilder: (context, i) {
          ...
          return _NoHoverTap(
            ...
            child: AnimatedContainer(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(icons[i], color: active ? _R.green : _R.graphite, size: 15),
                const SizedBox(width: 6),
                Text(
                  items[i],
                  style: TextStyle(
...
class _ReportModalLivePreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final players = _reportPlayers(report);
    final withHr = players.where((p) => p.heartRateSamplesCount > 0 || p.heartRateAvgBpm > 0).length;
      ...
      return Container(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            ...
            height: 28,
            alignment: Alignment.center,
            decoration: BoxDecoration(color: _R.softGreen, borderRadius: BorderRadius.circular(9)),
            child: Icon(icon, color: _R.greenDark, size: 15),
          ),
          const SizedBox(width: 7),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 9.0, fontWeight: FontWeight.w500)),
            const SizedBox(height: 2),
            Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 13.2, fontWeight: FontWeight.w600)),
            Text(note, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 8.2, fontWeight: FontWeight.w500)),
          ])),
        ]),
      );
    }
    ...
    Widget sectionCard({required String title, required IconData icon, required Widget child}) {
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(13),
          boxShadow: const [
            BoxShadow(color: Color(0x07111827), blurRadius: 14, offset: Offset(0, 3)),
          ],
        ),
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Row(children: [
            Container(
              ...
              height: 26,
              alignment: Alignment.center,
              decoration: BoxDecoration(color: _R.softGreen, borderRadius: BorderRadius.circular(8)),
              child: Icon(icon, size: 14, color: _R.green),
            ),
            const SizedBox(width: 8),
            Expanded(child: Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 12.0, fontWeight: FontWeight.w600))),
          ]),
          const SizedBox(height: 8),
          child,
        ]),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          if (_show('summary'))
            Wrap(
              ...
              runSpacing: 8,
              children: [
                metric(Icons.route_rounded, 'Дистанция', '${summary.totalDistanceM.toStringAsFixed(0)} м', 'общий объём'),
                metric(Icons.groups_rounded, 'Игроки', '${report.playersCount}', 'в отчёте'),
                metric(Icons.speed_rounded, 'Макс. скорость', '${summary.maxSpeedKmh.toStringAsFixed(1)} км/ч', 'пиковое значение'),
                metric(Icons.favorite_rounded, 'Пульс', '${summary.heartRateAvgBpm.toStringAsFixed(0)} уд/мин', '$withHr Polar'),
                metric(Icons.bolt_rounded, 'Спринты', '${summary.sprintCount}', 'за сессию'),
                metric(Icons.monitor_heart_rounded, 'Load', summary.playerLoad.toStringAsFixed(0), 'нагрузка'),
              ],
            ),
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              if (_show('speed'))
                Expanded(child: sectionCard(
                  title: 'Скорость и зоны',
                  icon: Icons.speed_rounded,
                  child: _ReportSpeedGraphPanel(report: report, constrainedHeight: true),
                )),
              if (_show('speed') && _show('internal')) SizedBox(width: gap),
              if (_show('internal'))
                Expanded(child: sectionCard(title: 'Пульс и нагрузка', icon: Icons.favorite_rounded, child: SizedBox(height: 300, child: _HrZoneSummaryBars(players: players)))),
            ])
          else ...[
            if (_show('speed')) sectionCard(
              title: 'Скорость и зоны',
              icon: Icons.speed_rounded,
              child: _ReportSpeedGraphPanel(report: report, constrainedHeight: true),
            ),
            if (_show('speed') && _show('internal')) SizedBox(height: gap),
            if (_show('internal')) sectionCard(title: 'Пульс и нагрузка', icon: Icons.favorite_rounded, child: SizedBox(height: 210, child: _HrZoneSummaryBars(players: players))),
          ],
          if (_show('speed') || _show('internal')) SizedBox(height: gap),
          if (_show('players'))
            sectionCard(title: 'Игроки сессии', icon: Icons.groups_rounded, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 6), child: _PlayersOverview(report: report))),
          if (_show('players')) SizedBox(height: gap),
          if (_show('locomotor'))
            sectionCard(title: 'Локомоторика', icon: Icons.directions_run_rounded, child: Padding(padding: const EdgeInsets.all(8), child: _LocomotorCards(players: players))),
          if (_show('locomotor')) SizedBox(height: gap),
          if (_show('mechanics'))
            sectionCard(title: 'Механика', icon: Icons.compare_arrows_rounded, child: SizedBox(height: 260, child: _DualBarChart(players: players, first: (p) => p.accelerations.toDouble(), second: (p) => p.decelerations.toDouble(), firstLabel: 'Ускорения', secondLabel: 'Торможения'))),
          if (_show('mechanics')) SizedBox(height: gap),
          if (_show('microcycle'))
            sectionCard(title: 'Микроцикл', icon: Icons.calendar_month_rounded, child: SizedBox(height: 210, child: _MicrocycleChart(points: report.microcycle))),
          if (_show('microcycle')) SizedBox(height: gap),
          if (_show('ai'))
            sectionCard(title: 'ИИ-анализ', icon: Icons.auto_awesome_rounded, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 6), child: _ReportAiPlaceholder(report: report, compact: false))),
          const SizedBox(height: 4),
        ]),
      );
    });
...
class _SummaryTab extends StatelessWidget {
  ...
  bool _show(String id) => sections == null || sections!.isEmpty || sections!.contains(id);
      ...
      children: [
        ...
        LayoutBuilder(builder: (context, c) {
          ...
          final left = Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            if (_show('summary')) _Card(title: 'ОБЩАЯ ИНФОРМАЦИЯ', child: _SummaryGrid(report: report)),
            SizedBox(height: gap),
            if (!hasData) ...[
              _NoMovementNotice(report: report),
              SizedBox(height: gap),
            ],
            if (_show('players')) _Card(title: 'ИГРОКИ И ТРЕКЕРЫ', child: _PlayersOverview(report: report)),
            SizedBox(height: gap),
            if (_show('players')) _Card(title: 'SPORTOTEKA PERFORMANCE MATRIX', child: _PerformanceMatrix(players: _reportPlayers(report))),
            SizedBox(height: gap),
            if (_show('player_pages')) _Card(title: 'ПО КАЖДОМУ ИГРОКУ: КАРТА + ГРАФИК ПО ВРЕМЕНИ', child: _PlayerDeepDiveCards(report: report)),
            SizedBox(height: gap),
            if (_show('locomotor')) _Card(title: 'ЛОКОМОТОРНЫЙ ПРОФИЛЬ ИГРОКОВ', child: _LocomotorCards(players: _reportPlayers(report))),
            SizedBox(height: gap),
            _Card(title: 'ПЕРИОДЫ / УПРАЖНЕНИЯ', child: _PeriodsTable(periods: report.periods)),
          ]);
          final right = Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            if (_show('speed')) _Card(title: 'ГРАФИК СКОРОСТИ', child: SizedBox(height: desktop ? 370 : 320, child: _ReportSpeedGraphPanel(report: report))),
            SizedBox(height: gap),
            if (!tablet) ...[
            ...
            SizedBox(height: gap),
            if (_show('mechanics')) _Card(title: 'МЕХАНИКА: УСКОРЕНИЯ И ТОРМОЖЕНИЯ', child: SizedBox(height: desktop ? 280 : 240, child: _DualBarChart(players: _reportPlayers(report), first: (p) => p.accelerations.toDouble(), second: (p) => p.decelerations.toDouble(), firstLabel: 'Ускорения', secondLabel: 'Торможения'))),
            SizedBox(height: gap),
            if (_show('microcycle')) _Card(title: 'ДИНАМИКА МИКРОЦИКЛА', child: SizedBox(height: desktop ? 260 : 230, child: _MicrocycleChart(points: report.microcycle))),
            SizedBox(height: gap),
            if (_show('ai')) _Card(title: 'ИИ АНАЛИЗ', child: _ReportAiPlaceholder(report: report, compact: true)),
            SizedBox(height: gap),
          ...
          if (tablet) {
            ...
            final tabletHeight = math.max(430.0, math.min(620.0, screenH - 118.0));
...
class _TabletReportQuickBlocks extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final metrics = <_Metric>[
      _Metric('Дистанция', '${s.averageDistanceM.toStringAsFixed(0)} м', 'объём'),
      _Metric('Игроки', '${report.playersCount}', 'в отчёте'),
      _Metric('Max', '${s.maxSpeedKmh.toStringAsFixed(1)} км/ч', 'скорость'),
      _Metric('Спринты', '${s.sprintCount}', 'кол-во'),
      _Metric('Load', s.playerLoad.toStringAsFixed(0), 'нагрузка'),
        ...
        GridView.count(
          ...
          children: [
            _TabletReportQuickBlock(
                icon: Icons.speed_rounded,
                title: 'Скорость',
                subtitle: '${report.pointsCount} GPS · max ${s.maxSpeedKmh.toStringAsFixed(1)} км/ч',
                height: blockHeight,
              ...
              _TabletReportQuickBlock(
                icon: Icons.favorite_rounded,
                title: 'Пульс / Load',
                subtitle: withHr > 0 ? '$withHr Polar · зоны ЧСС' : 'ожидаем Polar H10',
                height: blockHeight,
        ...
        Container(
          ...
          child: Row(children: [
            Container(
              ...
              height: 24,
              alignment: Alignment.center,
              decoration: BoxDecoration(color: _R.softGreen, borderRadius: BorderRadius.circular(8), border: Border.all(color: _R.greenLine)),
              child: Icon(icon, size: 14, color: _R.greenDark),
            ),
            const SizedBox(width: 7),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
              Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 10.8, fontWeight: FontWeight.w900)),
              Text(subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 8.7, fontWeight: FontWeight.w700)),
            ])),
          ]),
        ),
...
class _TabletReportCoachNotes extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final s = report.summary;
    final players = _reportPlayers(report);
    final maxPlayer = [...players]..sort((a, b) => b.maxSpeedKmh.compareTo(a.maxSpeedKmh));
    final topDistance = [...players]..sort((a, b) => b.distanceM.compareTo(a.distanceM));
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 6),
      decoration: BoxDecoration(color: _R.softGreen.withOpacity(.62), borderRadius: BorderRadius.circular(_R.mobileInnerRadius), border: Border.all(color: _R.greenLine)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Кратко для тренера', style: TextStyle(color: _R.text, fontSize: 9.8, fontWeight: FontWeight.w900)),
        const SizedBox(height: 6),
        _TabletReportNoteLine(icon: Icons.route_rounded, text: topDistance.isEmpty ? 'Нет данных по дистанции.' : 'Лидер объёма: ${topDistance.first.name} · ${topDistance.first.distanceM.toStringAsFixed(0)} м.'),
        const SizedBox(height: 5),
        _TabletReportNoteLine(icon: Icons.speed_rounded, text: maxPlayer.isEmpty ? 'Нет данных по скорости.' : 'Пик скорости: ${maxPlayer.first.name} · ${maxPlayer.first.maxSpeedKmh.toStringAsFixed(1)} км/ч.'),
        const SizedBox(height: 5),
        _TabletReportNoteLine(icon: Icons.local_fire_department_rounded, text: 'Командная нагрузка ${s.playerLoad.toStringAsFixed(0)} · HIR ${s.highSpeedDistanceM.toStringAsFixed(0)} м.'),
      ]),
    );
  }
}
...
class _TabletReportNoteLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(width: 6),
      Expanded(child: Text(text, style: const TextStyle(color: _R.graphite, fontSize: 8.1, fontWeight: FontWeight.w800, height: 1.08))),
    ]);
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Спортотека. Отчёт', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.text, fontSize: 13.2, fontWeight: FontWeight.w600)),
            const SizedBox(height: 3),
            Text(
              '${report.teamName.isEmpty ? 'Команда' : report.teamName} · ${report.dateLabel.isEmpty ? 'дата не указана' : report.dateLabel}',
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              style: const TextStyle(color: _R.muted, fontSize: 10.2, fontWeight: FontWeight.w500),
            ),
          ]),
        ),
        const SizedBox(width: 7),
...
class _PlayerDeepDiveCards extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final players = _reportPlayers(report).where((p) => p.distanceM > 0 || p.pointsCount > 0 || p.heartRateSamplesCount > 0).toList(growable: false);
    if (players.isEmpty) return const _EmptyBlock('Нет данных по игрокам для индивидуальных карт и графиков.');
    return LayoutBuilder(builder: (context, c) {
      ...
      return GridView.builder(
        ...
        itemBuilder: (context, index) => _PlayerDeepDiveCard(report: report, player: players[index]),
      );
    });
  }
}
...
class _PlayerDeepDiveCard extends StatelessWidget {
  ...
  bool _samePoint(TrackerReportPoint p) {
    ...
    return p.playerName.trim().isNotEmpty && p.playerName.trim().toLowerCase() == player.name.trim().toLowerCase();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFFFAFBFC), borderRadius: BorderRadius.circular(13), border: Border.all(color: _R.border)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('${player.duration} · ${player.distanceM.toStringAsFixed(0)} м · max ${player.maxSpeedKmh.toStringAsFixed(1)} · HR ${player.heartRateSamplesCount}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 9, fontWeight: FontWeight.w800)),
          ])),
        ]),
        const SizedBox(height: 8),
        Expanded(
          child: LayoutBuilder(builder: (context, c) {
            ...
            final pitch = ClipRRect(
              ...
              child: CustomPaint(
                painter: _ReportPitchPainter(points: points, hsrKmh: hsrKmh, sprintKmh: sprintKmh),
                child: points.isEmpty ? const Center(child: Text('нет GPS', style: TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.w900))) : const SizedBox.expand(),
              ),
            );
            final map = Center(
              child: AspectRatio(
            ...
            final timeline = Container(
              decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), border: Border.all(color: _R.border)),
              child: ClipRect(child: CustomPaint(painter: _PlayerMiniTimelinePainter(points: points, heartRate: hr), child: const SizedBox.expand())),
            );
            if (vertical) return Column(children: [Expanded(child: map), const SizedBox(height: 7), Expanded(child: timeline)]);
            return Row(children: [Expanded(child: map), const SizedBox(width: 7), Expanded(child: timeline)]);
          }),
        ),
      ]),
    );
...
class _PlayerMiniTimelinePainter extends _BaseChartPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    final rect = Rect.fromLTWH(28, 20, math.max(10.0, size.width - 42), math.max(10.0, size.height - 44));
    drawGrid(canvas, rect, lines: 3);
    text(canvas, 'скорость / ЧСС', Offset(rect.left, 3), size: 8.4, color: _R.text, weight: FontWeight.w900, maxWidth: rect.width);
    final speed = points.where((p) => p.speedKmh > 0).toList(growable: false)..sort((a, b) => a.timeMs.compareTo(b.timeMs));
    if (speed.isEmpty && heartRate.isEmpty) {
      text(canvas, 'нет таймлайна', Offset(rect.left + 12, rect.center.dy - 6), size: 9, color: _R.muted, weight: FontWeight.w900, maxWidth: rect.width - 24);
      return;
    }
    void drawLine<T>(List<T> list, int Function(T) key, double Function(T) value, double maxY, Color color) {
      if (list.isEmpty) return;
      final minX = list.map(key).fold<int>(key(list.first), (a, b) => a < b ? a : b).toDouble();
      var maxX = list.map(key).fold<int>(key(list.first), (a, b) => a > b ? a : b).toDouble();
      if ((maxX - minX).abs() < 1) maxX = minX + 60000;
      final path = Path();
      for (var i = 0; i < list.length; i++) {
        final item = list[i];
        final x = rect.left + rect.width * ((key(item) - minX) / (maxX - minX)).clamp(0.0, 1.0).toDouble();
        final y = rect.bottom - rect.height * (value(item).clamp(0.0, maxY) / maxY).toDouble();
        if (i == 0) path.moveTo(x, y); else path.lineTo(x, y);
      }
      canvas.drawPath(path, Paint()..color = color..style = PaintingStyle.stroke..strokeWidth = 2.0..strokeCap = StrokeCap.round..strokeJoin = StrokeJoin.round);
    }
    final maxSpeed = math.max(12.0, speed.fold<double>(0, (m, p) => math.max(m, p.speedKmh)) + 2);
    drawLine<TrackerReportPoint>(speed, (p) => p.timeMs, (p) => p.speedKmh, maxSpeed, _R.green);
    drawLine<TrackerHeartRatePoint>(heartRate, _hrPointSortKey, (p) => p.bpm.toDouble(), 210, _R.red);
    text(canvas, '0 мин', Offset(rect.left, rect.bottom + 7), size: 8, color: _R.muted, weight: FontWeight.w900, maxWidth: 40);
  }

  @override
  bool shouldRepaint(covariant _PlayerMiniTimelinePainter oldDelegate) => oldDelegate.points != points || oldDelegate.heartRate != heartRate;
}
...
class _PlayerReportCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(9),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(9), border: Border.all(color: _R.border)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          _PlayerAvatar(player: player, size: 38),
          const SizedBox(width: 7),
...
class _TinyPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text('$label $value', style: const TextStyle(color: _R.graphite, fontSize: 8.2, fontWeight: FontWeight.w900)),
    );
  }
}
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        if (compact) {
          return Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              ...
              const SizedBox(height: 5),
              Text(stats, maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(color: _R.muted, fontSize: 10.4, fontWeight: FontWeight.w800)),
            ]),
          );
        }
        return Container(
      ...
      if (!_heatMode)
        Container(
          height: 30,
          decoration: const BoxDecoration(),
          child: ListView(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 4), children: [
        ...
        final map = ClipRRect(
          ...
          child: hasPoints
              ...
              : CustomPaint(
                  ...
                  child: const Center(child: Text('Нет GPS-точек для карты выбранной сессии', style: TextStyle(color: Colors.white70, fontSize: 10.2, fontWeight: FontWeight.w500))),
                ),
        );
        if (!hasPoints) {
          return Container(
            ...
            child: const Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              SizedBox(width: 8),
              Text('Нет GPS-точек для выбранной сессии', style: TextStyle(color: _R.muted, fontSize: 10.4, fontWeight: FontWeight.w500)),
            ]),
          );
        }
        if (mapHeight != null) {
  ...
  Widget build(BuildContext context) {
    return _NoHoverTap(
      ...
      child: Container(
        height: 36,
        padding: const EdgeInsets.symmetric(horizontal: 7),
        decoration: BoxDecoration(color: active ? _R.softGreen : Colors.transparent, borderRadius: BorderRadius.circular(10)),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
    ...
    return Padding(
      ...
      child: _NoHoverTap(
        ...
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: 7),
          alignment: Alignment.center,
          decoration: BoxDecoration(color: active ? _R.green : const Color(0xFFF2F5F3), borderRadius: BorderRadius.circular(8)),
          child: Text(label, style: TextStyle(color: active ? Colors.white : _R.text, fontSize: 7.8, fontWeight: FontWeight.w900)),
        ),
      ),
    );
  }
    ...
    return Column(children: [
      for (final z in zones) ...[
        Builder(builder: (context) {
          ...
          return Row(children: [
            SizedBox(width: 126, child: Text(z.label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 11.2, fontWeight: FontWeight.w900))),
...
class _ReportPitchPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(RRect.fromRectAndRadius(outer, const Radius.circular(14)), Paint()..color = const Color(0xFF789274));
    ...
    for (var i = 0; i < 11; i++) {
      canvas.drawRect(Rect.fromLTWH(pitch.left + i * stripeW, pitch.top, stripeW, pitch.height), Paint()..color = (i.isEven ? Colors.white : Colors.black).withOpacity(i.isEven ? .07 : .025));
    }
    canvas.restore();
    ...
    if (heatMode) {
      final maxValue = points.fold<double>(1.0, (m, p) => math.max(m, p.value <= 0 ? 1.0 : p.value));
      canvas.save();
      canvas.clipRRect(clip);
      for (final p in points.take(1500)) {
        final o = _point(pitch, p);
        final ratio = ((p.value <= 0 ? 1.0 : p.value) / maxValue).clamp(.05, 1).toDouble();
        final c = ratio > .66 ? const Color(0xFFE11D48) : ratio > .34 ? const Color(0xFFF59E0B) : const Color(0xFF22C55E);
        final r = 7.0 + 13.0 * ratio;
        canvas.drawCircle(o, r, Paint()..shader = RadialGradient(colors: [c.withOpacity(.48 * ratio), c.withOpacity(.16 * ratio), Colors.transparent], stops: const [0, .48, 1]).createShader(Rect.fromCircle(center: o, radius: r)));
      }
      canvas.restore();
      return;
    }
    ...
    if (local.length > 1) {
      for (var i = 1; i < local.length; i++) {
        ...
        canvas.drawLine(a, b, Paint()
          ...
          ..strokeWidth = speed >= sprintKmh ? 3.2 : (speed >= hsrKmh ? 2.7 : 2.0)
          ..strokeCap = StrokeCap.round);
    ...
    for (var i = 0; i < local.length; i++) {
      ...
      if (p.speedKmh > 0 && (i == 0 || i == local.length - 1 || p.speedKmh >= sprintKmh || lastLabel == null || (o - lastLabel).distance > 42)) {
  ...
  void _paintSmallText(Canvas canvas, String text, Offset offset, Color color) {
    final tp = TextPainter(text: TextSpan(text: text, style: TextStyle(color: color, fontSize: 9.6, fontWeight: FontWeight.w900)), textDirection: TextDirection.ltr)..layout(maxWidth: 42);
...
class _ReportSpeedGraphPanel extends StatelessWidget {
  const _ReportSpeedGraphPanel({required this.report, this.constrainedHeight = false});
  final TrackerTrainingReport report;
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 6), child: content);
  }
}
...
class _RadarComparisonPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, c) {
      ...
      return Row(children: [Expanded(child: left), const SizedBox(width: 7), Expanded(child: right)]);
    });
  }
}
    ...
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: const Icon(Icons.info_outline_rounded, color: Color(0xFFB7791F), size: 15),
        ),
        const SizedBox(width: 8),
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget build(BuildContext context) {
    ...
    return _ScrollPage(children: [
      Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Локомоторная работа', style: TextStyle(color: _R.text, fontSize: 15, fontWeight: FontWeight.w900, letterSpacing: -.5)),
          const SizedBox(height: 5),
          Text(report.title, style: const TextStyle(color: _R.muted, fontSize: 12, fontWeight: FontWeight.w700)),
        ]),
      ),
      LayoutBuilder(builder: (context, c) {
      ...
      Padding(
        ...
        child: Row(children: [
          const Expanded(child: Text('Игроки', style: TextStyle(color: _R.text, fontSize: 15, fontWeight: FontWeight.w900, letterSpacing: -.3))),
          Text('${players.length} в отчёте', style: const TextStyle(color: _R.muted, fontSize: 12.5, fontWeight: FontWeight.w800)),
        ]),
      ),
      const SizedBox(height: 8),
    ...
    return Container(
      ...
      child: Row(children: [
        Container(
          width: 28,
          height: 28,
          decoration: BoxDecoration(color: const Color(0xFFEAF8F0), borderRadius: BorderRadius.circular(9)),
          child: Icon(icons[index % icons.length], color: _R.green, size: 15),
        ),
        const SizedBox(width: 8),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
          Text(metric.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 9.2, height: 1.12, fontWeight: FontWeight.w800)),
          const SizedBox(height: 5),
          Text(metric.value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 15.5, height: 1, fontWeight: FontWeight.w900, letterSpacing: -.5)),
          const SizedBox(height: 4),
          Text(metric.subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 8.4, fontWeight: FontWeight.w700)),
        ])),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return _ScrollPage(children: [
      ...
      const Padding(
        ...
        child: Text('Игроки', style: TextStyle(color: _R.text, fontSize: 15, fontWeight: FontWeight.w900, letterSpacing: -.3)),
      ),
      const SizedBox(height: 8),
...
class _MechanicalPlayerCards extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (players.isEmpty) return const _EmptyBlock('Нет данных механической работы по игрокам.');
    return LayoutBuilder(builder: (context, c) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(player.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 12.5, fontWeight: FontWeight.w900)),
            const SizedBox(height: 3),
            Text([if (player.number.isNotEmpty) '#${player.number}', if (player.position.isNotEmpty) player.position].join(' · '), style: const TextStyle(color: _R.muted, fontSize: 9.2, fontWeight: FontWeight.w700)),
          ])),
        ]),
        const SizedBox(height: 8),
...
class _AiAnalysisTab extends StatelessWidget {
  const _AiAnalysisTab({required this.report});
  final TrackerTrainingReport report;
  ...
  Widget build(BuildContext context) {
    return _ScrollPage(children: [
      ...
      _Card(title: 'ЗАГЛУШКА ПОД БУДУЩИЙ ИИ', child: _ReportAiPlaceholder(report: report)),
      const SizedBox(height: 8),
      _Card(title: 'ЧТО ИИ ПОЛУЧИТ ДЛЯ АНАЛИЗА', child: _AiInputMatrix(report: report)),
      const SizedBox(height: 8),
      _Card(title: 'ПЕРСОНАЛЬНЫЕ СИГНАЛЫ', child: _AiPlayerSignals(report: report)),
    ]);
  }
}
...
class _ReportAiPlaceholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final players = _reportPlayers(report);
    final withHr = players.where((p) => p.heartRateSamplesCount > 0).length;
    final withGps = players.where((p) => p.pointsCount > 0 || p.distanceM > 0).length;
    final messages = <String>[
      'ИИ пока работает как заглушка: блок уже встроен в отчёт, дальше сюда подключается серверный анализ.',
      'В анализ будет уходить: GPS-точки, теплокарта, скорость по времени, ускорения/торможения, Polar H10, зоны ЧСС и карточки игроков.',
      'Сейчас готово для передачи: ${players.length} игрок(ов), $withGps с GPS-данными, $withHr с Polar H10, ${report.routePoints.length} точек маршрута.',
    ];
    return Container(
      padding: EdgeInsets.all(compact ? 10 : 14),
      decoration: BoxDecoration(color: const Color(0xFFFAFFFC), borderRadius: BorderRadius.circular(13), border: Border.all(color: _R.greenLine)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        Row(children: [
          Container(width: compact ? 30 : 38, height: compact ? 30 : 38, alignment: Alignment.center, decoration: BoxDecoration(color: _R.softGreen, borderRadius: BorderRadius.circular(12), border: Border.all(color: _R.greenLine)), child: const Icon(Icons.auto_awesome_rounded, color: _R.green, size: 15)),
          const SizedBox(width: 8),
          Expanded(child: Text('AI Coach / Sportoteka', style: TextStyle(color: _R.text, fontSize: compact ? 11.5 : 14, fontWeight: FontWeight.w900))),
          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: _R.soft, borderRadius: BorderRadius.circular(999), border: Border.all(color: _R.border)), child: const Text('stub', style: TextStyle(color: _R.graphite, fontSize: 9, fontWeight: FontWeight.w900))),
        ]),
        SizedBox(height: compact ? 8 : 12),
        for (final m in messages.take(compact ? 2 : 3)) ...[
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Icon(Icons.check_circle_rounded, color: _R.green, size: 15),
            const SizedBox(width: 7),
            Expanded(child: Text(m, style: TextStyle(color: _R.graphite, fontSize: compact ? 9.5 : 10.8, height: 1.32, fontWeight: FontWeight.w800))),
          ]),
          if (m != messages.take(compact ? 2 : 3).last) SizedBox(height: compact ? 5 : 8),
        ],
      ]),
    );
  }
...
class _AiInputMatrix extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final rows = <_Metric>[
      _Metric('Polar H10', '${report.summary.heartRateSamplesCount}', 'HR-точек'),
      _Metric('Игроки', '${_reportPlayers(report).length}', 'персональные блоки'),
      _Metric('Спринты', '${report.summary.sprintCount}', 'скоростные действия'),
      _Metric('Уск/торм', '${report.summary.accelerationCount}/${report.summary.decelerationCount}', 'механика'),
    ];
...
class _AiPlayerSignals extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final players = _reportPlayers(report).toList(growable: false)
      ..sort((a, b) => (b.playerLoad + b.distanceM / 10 + b.heartRateAvgBpm).compareTo(a.playerLoad + a.distanceM / 10 + a.heartRateAvgBpm));
    if (players.isEmpty) return const _EmptyBlock('ИИ-сигналы появятся после появления игроков в отчёте.');
    return Column(children: [
      for (final p in players.take(8)) ...[
        Row(children: [
          ...
          const SizedBox(width: 7),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(p.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 9.5, fontWeight: FontWeight.w900)),
            Text(_aiSignalText(p), maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 9.2, height: 1.25, fontWeight: FontWeight.w800)),
          ])),
        ]),
        if (p != players.take(8).last) const Divider(height: 14, color: _R.border),
      ],
    ]);
  }

  String _aiSignalText(TrackerTrainingPlayerRow p) {
    if (p.heartRateAvgBpm >= 165 && p.distanceM < 500) return 'высокий пульс при небольшом объёме — проверить восстановление и посадку ремня Polar';
    if (p.maxSpeedKmh >= 18 || p.sprintCount > 0) return 'есть скоростная работа: ${p.sprintCount} спринт(ов), max ${p.maxSpeedKmh.toStringAsFixed(1)} км/ч';
    if (p.distanceM <= 1 && p.pointsCount <= 0) return 'нет подтверждённого GPS-движения — не использовать для итоговой оценки нагрузки';
    if (p.heartRateSamplesCount > 0) return 'Polar H10 активен: avg ${p.heartRateAvgBpm.toStringAsFixed(0)} bpm, max ${p.heartRateMaxBpm.toStringAsFixed(0)} bpm';
...
class _MicrocycleTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final avgDistance = points.isEmpty ? 0.0 : points.fold<double>(0, (s, p) => s + p.distanceM) / points.length;
    final avgHsr = points.isEmpty ? 0.0 : points.fold<double>(0, (s, p) => s + p.highSpeedRunningM) / points.length;
    final avgAccDec = points.isEmpty ? 0.0 : points.fold<double>(0, (s, p) => s + p.accDec) / points.length;
    final maxDistance = points.isEmpty ? 0.0 : points.map((p) => p.distanceM).reduce(math.max);
    final metrics = <_Metric>[
    ...
    return _ScrollPage(children: [
      ...
      const Padding(
        padding: EdgeInsets.symmetric(horizontal: 6),
        child: Text('Дни микроцикла', style: TextStyle(color: _R.text, fontSize: 15, fontWeight: FontWeight.w900, letterSpacing: -.3)),
      ),
      const SizedBox(height: 8),
...
class _MicrocycleDayCards extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (points.isEmpty) return const _EmptyBlock('Микроцикл появится после нескольких реальных сессий команды.');
    return LayoutBuilder(builder: (context, c) {
...
class _MicrocycleDayCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(width: 30, height: 30, decoration: BoxDecoration(color: _R.softGreen, borderRadius: BorderRadius.circular(9)), child: const Icon(Icons.calendar_today_rounded, color: _R.green, size: 15)),
          const SizedBox(width: 8),
          Expanded(child: Text(point.label.isEmpty ? 'День ${index + 1}' : point.label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 12.5, fontWeight: FontWeight.w900))),
        ]),
        const SizedBox(height: 8),
        Expanded(child: Row(children: [
          for (var i = 0; i < metrics.length; i++) ...[
...
class _CleanReportHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    padding: const EdgeInsets.fromLTRB(4, 0, 4, 8),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(title, style: const TextStyle(color: _R.text, fontSize: 14, fontWeight: FontWeight.w900, letterSpacing: -.5)),
      const SizedBox(height: 5),
      Text(subtitle, style: const TextStyle(color: _R.muted, fontSize: 9.5, fontWeight: FontWeight.w700)),
    ]),
  );
}
...
class _CleanChartPanel extends StatelessWidget {
  const _CleanChartPanel({required this.title, required this.subtitle, required this.child});
  final String title;
  final String subtitle;
  final Widget child;
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(title, style: const TextStyle(color: _R.text, fontSize: 14, fontWeight: FontWeight.w900)),
      const SizedBox(height: 4),
      Text(subtitle, style: const TextStyle(color: _R.muted, fontSize: 9.5, fontWeight: FontWeight.w700)),
      const SizedBox(height: 8),
      child,
...
class _CleanMiniValue extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 6),
    decoration: BoxDecoration(color: const Color(0xFFF7F9F8), borderRadius: BorderRadius.circular(9)),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
      Text(metric.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 7.8, fontWeight: FontWeight.w800)),
      const SizedBox(height: 5),
      Text(metric.value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 14, fontWeight: FontWeight.w900)),
      const SizedBox(height: 2),
      Text(metric.subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 7.8, fontWeight: FontWeight.w700)),
    ]),
  );
}
...
class _Card extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return Container(
        clipBehavior: Clip.antiAlias,
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(_R.mobileInnerRadius), border: Border.all(color: _R.border)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            height: 30,
            padding: const EdgeInsets.symmetric(horizontal: 10),
...
class _SectionTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Center(child: Text(text, style: const TextStyle(color: _R.text, fontSize: 12.5, fontWeight: FontWeight.w800, letterSpacing: .1))),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, c) {
      ...
      return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
          Text(metric.title.toUpperCase(), maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.muted, fontSize: compact ? 6.8 : 7.4, fontWeight: FontWeight.w900, letterSpacing: .15)),
          SizedBox(height: compact ? 1 : 2),
          FittedBox(alignment: Alignment.centerLeft, fit: BoxFit.scaleDown, child: Text(metric.value, maxLines: 1, style: TextStyle(color: _R.text, fontSize: compact ? 12.5 : 14.0, fontWeight: FontWeight.w900))),
          Text(metric.subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.muted, fontSize: compact ? 7.0 : 7.6, fontWeight: FontWeight.w800)),
        ]),
      );
    });
  }
      ...
      if (compact || !twoColumns) {
        return Column(children: [
          for (var i = 0; i < cards.length; i++) ...[
            _LocomotorPlayerCard(player: cards[i], average: cards[i].name == 'СРЕДНЕЕ', compact: compact),
...
class _LocomotorPlayerCard extends StatelessWidget {
  const _LocomotorPlayerCard({required this.player, required this.average, this.compact = false});
  final TrackerTrainingPlayerRow player;
  final bool average;
  final bool compact;
  ...
  Widget build(BuildContext context) {
    final metrics = <_MiniMetricData>[
      ...
      _MiniMetricData('Интенсивность', '${player.metersPerMin.toStringAsFixed(1)} м/мин'),
      _MiniMetricData('Средняя скорость', '${player.avgSpeedKmh.toStringAsFixed(1)} км/ч'),
      _MiniMetricData('Макс. скорость', '${player.maxSpeedKmh.toStringAsFixed(1)} км/ч'),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          if (!average) ...[_PlayerAvatar(player: player, size: compact ? 38 : 40), const SizedBox(width: 11)],
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(average ? 'Средние показатели' : player.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.text, fontSize: compact ? 12.5 : 13, fontWeight: FontWeight.w900)),
            if (!average) ...[
              const SizedBox(height: 3),
              Text([if (player.number.isNotEmpty) '#${player.number}', if (player.position.isNotEmpty) player.position].join(' · '), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 9.2, fontWeight: FontWeight.w700)),
            ],
          ])),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5),
            decoration: BoxDecoration(color: const Color(0xFFF3F6F4), borderRadius: BorderRadius.circular(999)),
            child: Text(player.duration, style: const TextStyle(color: _R.graphite, fontSize: 9.2, fontWeight: FontWeight.w900)),
          ),
        ]),
...
class _MiniMetric extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 4),
      decoration: BoxDecoration(color: const Color(0xFFF5F8F6), borderRadius: BorderRadius.circular(9)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
        Text(metricLabel, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.muted, fontSize: 7.8, fontWeight: FontWeight.w800)),
        const SizedBox(height: 4),
        FittedBox(alignment: Alignment.centerLeft, fit: BoxFit.scaleDown, child: Text(metricValue, maxLines: 1, style: const TextStyle(color: _R.text, fontSize: 11.8, fontWeight: FontWeight.w900))),
      ]),
    );
  }
}
...
class _LocomotorTable extends StatelessWidget {
  ...
  TableRow _playerRow(TrackerTrainingPlayerRow p, bool avg) {
    return TableRow(
      decoration: BoxDecoration(color: avg ? const Color(0xFFE1E5E2) : Colors.white),
      children: [
        _playerCell(p, avg),
        _cell(p.duration),
...
class _TwoByTwo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constraints) {
      ...
      if (!wide) {
        return Column(children: [a, const SizedBox(height: 8), b, const SizedBox(height: 8), c, const SizedBox(height: 8), d]);
      }
      return Column(children: [
        Row(children: [Expanded(child: a), const SizedBox(width: 7), Expanded(child: b)]),
        const SizedBox(height: 8),
        Row(children: [Expanded(child: c), const SizedBox(width: 7), Expanded(child: d)]),
      ]);
    });
  }
}
...
class _ReportHrTimelineLegend extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final sorted = [...points]..sort((a, b) => _hrPointSortKey(a).compareTo(_hrPointSortKey(b)));
    ...
    final sessionLabel = report.sessionIds.isNotEmpty ? report.sessionIds.map((id) => '#$id').join(', ') : '#${report.sessionId}';
    return Container(
      height: 36,
      padding: const EdgeInsets.symmetric(horizontal: 7),
      decoration: BoxDecoration(color: _R.soft, borderRadius: BorderRadius.circular(10), border: Border.all(color: _R.border)),
      child: Row(children: [
        ...
        const SizedBox(width: 7),
        Expanded(child: Text('$sessionLabel · ${names.isEmpty ? 'команда' : names.take(5).join(', ')}${names.length > 5 ? ' +${names.length - 5}' : ''} · $durationMin мин', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _R.text, fontSize: 10, fontWeight: FontWeight.w900))),
        Text('${points.length} HR', style: const TextStyle(color: _R.muted, fontSize: 9.4, fontWeight: FontWeight.w900)),
      ]),
    );
  }
}
...
class _HrZoneSummaryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(children: [
      ...
      const SizedBox(width: 8),
      SizedBox(width: 74, child: Text('${min}:${sec.toString().padLeft(2, '0')} · ${(ratio * 100).toStringAsFixed(0)}%', textAlign: TextAlign.right, style: const TextStyle(color: _R.muted, fontSize: 10.4, fontWeight: FontWeight.w900))),
    ]);
  }
}
...
int _hrPointSortKey(TrackerHeartRatePoint p) => p.timeMs > 0 ? p.timeMs : p.minute * 60000;
...
List<List<TrackerHeartRatePoint>> _hrPointGroups(List<TrackerHeartRatePoint> points) {
  final map = <String, List<TrackerHeartRatePoint>>{};
  for (final p in points.where((p) => p.bpm > 0)) {
    map.putIfAbsent(_hrPointGroupKey(p), () => <TrackerHeartRatePoint>[]).add(p);
  }
...
class _HeartRateTimelinePainter extends _BaseChartPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final minX = list.map(_hrPointSortKey).fold<int>(_hrPointSortKey(list.first), (a, b) => a < b ? a : b).toDouble();
    var maxX = list.map(_hrPointSortKey).fold<int>(_hrPointSortKey(list.first), (a, b) => a > b ? a : b).toDouble();
    if ((maxX - minX).abs() < 1) maxX = minX + 60000;
    ...
    Offset mapPoint(TrackerHeartRatePoint p, double yValue) {
      final x = rect.left + rect.width * ((_hrPointSortKey(p) - minX) / (maxX - minX)).clamp(0.0, 1.0).toDouble();
    ...
    for (var gi = 0; gi < groups.length; gi++) {
      ...
      canvas.drawPath(loadPath, Paint()..color = const Color(0xFF7F1D1D).withOpacity(groups.length == 1 ? .38 : .16)..style = PaintingStyle.stroke..strokeWidth = 2.0..strokeCap = StrokeCap.round);
      canvas.drawPath(bpmPath, Paint()..color = lineColor..style = PaintingStyle.stroke..strokeWidth = groups.length == 1 ? 3.0 : 2.2..strokeCap = StrokeCap.round..strokeJoin = StrokeJoin.round);
      ...
      for (var i = 0; i < group.length; i += every) {
        ...
        canvas.drawCircle(o, 7.8, Paint()..color = _hrZoneColor(p.zone).withOpacity(.08));
        text(canvas, '${p.bpm}', o + const Offset(-10, -22), size: 9, color: lineColor, weight: FontWeight.w900, maxWidth: 36);
      }
      if (groups.length > 1) {
        final p = group.first;
        final o = mapPoint(p, p.bpm.toDouble());
        text(canvas, _shortReportPlayerName(p.playerName.isEmpty ? 'Игрок' : p.playerName), o + const Offset(5, 7), size: 8, color: lineColor, weight: FontWeight.w900, maxWidth: 80);
      }
    }
...
abstract class _BaseChartPainter extends CustomPainter {
  void drawGrid(Canvas canvas, Rect rect, {int lines = 5}) {
    final paint = Paint()..color = const Color(0xFFE1E5E2)..strokeWidth = 1;
...
TableRow _tableRow(List<String> values, {bool header = false}) {
  return TableRow(
    decoration: BoxDecoration(color: header ? const Color(0xFFE1E5E2) : Colors.white),
...
Widget _cell(String text, {bool bold = false, TextAlign align = TextAlign.center}) {
  return Padding(
    ...
    child: Text(text, textAlign: align, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: _R.text, fontSize: 10.4, fontWeight: bold ? FontWeight.w800 : FontWeight.w600)),
  );
}
...
class _EmptyBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Text(text, textAlign: TextAlign.center, style: const TextStyle(color: _R.muted, fontSize: 11.2, fontWeight: FontWeight.w700)),
  );
}
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Container(
        ...
        padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 6),
        decoration: BoxDecoration(color: const Color(0xFFF7F9F8), borderRadius: BorderRadius.circular(_R.mobileInnerRadius), border: Border.all(color: _R.border)),
...
class _PrintableTrainingReportHtml {
  ...
  static String _build(TrackerTrainingReport report, {List<String>? sections}) {
    final selectedSections = sections == null || sections.isEmpty ? _defaultReportExportSectionIds() : sections.toSet();
...
''');
    ...
    if (show('summary')) b.write('<section class="section"><h2>Сводка тренировки</h2>${_kpis([
      _Kpi('Время', report.durationLabel, 'длительность'),
      _Kpi('Игроки', '${report.playersCount}', 'в отчёте'),
      _Kpi('Дистанция', _m(s.totalDistanceM > 0 ? s.totalDistanceM : s.averageDistanceM), 'команда'),
      ...
      _Kpi('Уск/торм', '${s.accelerationCount}/${s.decelerationCount}', 'кол-во'),
      _Kpi('Нагрузка', s.playerLoad.toStringAsFixed(0), 'усл. ед.'),
      _Kpi('Avg HR', s.heartRateAvgBpm > 0 ? s.heartRateAvgBpm.toStringAsFixed(0) : '—', 'bpm'),
      _Kpi('Max HR', s.heartRateMaxBpm > 0 ? s.heartRateMaxBpm.toStringAsFixed(0) : '—', 'bpm'),
    ])}</section>');

    if (showAny(const ['maps', 'heatmap'])) {
      b.write('<section class="section"><h2>Карты и тепловая карта</h2><div class="grid grid-2">');
    b.write(_card('Карта активности', '${_mapSvg(route, heatMode: false, report: report)}${_mapLegend(report)}'));
    b.write(_card('Тепловая карта', '${_mapSvg(heat, heatMode: true, report: report)}<div class="legend">Плотность перемещений игрока: зелёный — низкая, жёлтый — средняя, красный — высокая</div>'));
    b.write('</div></section>');
    }

    if (show('speed')) {
      b.write('<section class="section"><h2>Скорость и зоны</h2><div class="grid grid-2">');
    b.write(_card('График скорости по времени', '${_speedSvg(report)}${_mapLegend(report)}'));
    ...
    if (showAny(const ['locomotor', 'players'])) {
      ...
      b.write(_card('Локомоторный профиль игроков', _locomotorCardsHtml(players)));
      b.write('</section>');
    }

    if (show('mechanics')) {
      b.write('<section class="section"><h2>Механика</h2><div class="grid grid-2">');
    b.write(_card('Ускорения / торможения', _dualBarSvg(players, (p) => p.accelerations.toDouble(), (p) => p.decelerations.toDouble(), 'ACC', 'DEC')));
    b.write(_card('Взрывные действия / смены направления', _dualBarSvg(players, (p) => p.explosiveActions.toDouble(), (p) => p.highSpeedActions.toDouble(), 'Взрывные', 'Повороты')));
    b.write('</div></section>');
    }

    if (showAny(const ['internal', 'hr'])) {
      b.write('<section class="section"><h2>Внутренняя нагрузка / Polar H10</h2><div class="grid grid-2">');
    b.write(_card('Пульс по времени', _hrSvg(report, hr)));
    b.write(_card('Зоны ЧСС и рекомендации', '${_hrZonesTable(players)}${_coachMessages(report)}'));
    b.write('</div></section>');
    }

    if (show('microcycle')) {
      b.write('<section class="section"><h2>Рейтинг и микроцикл</h2><div class="grid grid-2">');
    b.write(_card('Рейтинг команды / нагрузка', _dualBarSvg(players, (p) => p.distanceM, (p) => p.playerLoad, 'м', 'load')));
    b.write(_card('Динамика микроцикла', _microcycleSvg(report)));
    b.write('</div></section>');
    }

    if (show('player_pages')) {
      b.write('<section class="section page-break"><h2>Карточки игроков: карта + график</h2>');
    ...
    b.write(_card('Что передаётся в AI Coach', _aiBlock(report)));
    b.write('</section>');
    }
  ...
  static String _m(double value) => value >= 1000 ? '${(value / 1000).toStringAsFixed(2)} км' : '${value.toStringAsFixed(0)} м';
  static String _f(double value, [int digits = 1]) => value.isFinite ? value.toStringAsFixed(digits) : '0';

  static String _kpis(List<_Kpi> items) {
    final b = StringBuffer('<div class="kpis">');
    for (final item in items) {
      b.write('<div class="kpi"><div class="v">${_h(item.value)}</div><div class="l">${_h(item.title)} · ${_h(item.note)}</div></div>');
    }
    b.write('</div>');
    return b.toString();
  }

  static String _card(String title, String body) => '<div class="card"><div class="card-title">${_h(title)}</div><div class="card-body">$body</div></div>';

  static String _locomotorCardsHtml(List<TrackerTrainingPlayerRow> players) {
    if (players.isEmpty) return '<div class="muted">Нет игроков в выбранной сессии.</div>';
    final rows = [...players, _avgRow(players)];
    final b = StringBuffer('<div class="loco-grid">');
    for (final p in rows) {
      final average = p.name == 'СРЕДНЕЕ';
      b.write('<article class="loco-card"${average ? ' style="background:#F0FDF4"' : ''}>');
      b.write('<div class="loco-head"><div><div class="loco-name">${_h(p.name)}</div><div class="muted">${_h([if (p.number.isNotEmpty) '#${p.number}', if (p.position.isNotEmpty) p.position].join(' · '))}</div></div><div class="loco-duration">${_h(p.duration)}</div></div>');
      ...
      for (final m in metrics) b.write('<div class="loco-metric"><strong>${_h(m[0])}</strong><span>${_h(m[1])}</span></div>');
      b.write('</div></article>');
    }
    b.write('</div>');
    return b.toString();
  ...
  static String _playersTable(List<TrackerTrainingPlayerRow> players) {
    ...
    const headers = ['Игрок', 'Дист.', 'м/мин', 'Avg', 'Max', 'V3', 'V4', 'V5', 'Спр.', 'Уск.', 'Торм.', 'Load', 'Avg HR', 'Max HR', 'GPS'];
    ...
    for (final p in players) {
      b.write('<tr><td><strong>${_h(p.name)}</strong><div class="muted">${_h([if (p.number.isNotEmpty) '#${p.number}', if (p.position.isNotEmpty) p.position, if (p.deviceName.isNotEmpty) p.deviceName].join(' · '))}</div></td>');
      final values = [
        _m(p.distanceM), _f(p.metersPerMin), _f(p.avgSpeedKmh), _f(p.maxSpeedKmh), _m(p.v3RunM), _m(p.v4HsrM), _m(p.v5SprintM), '${p.sprintCount}', '${p.accelerations}', '${p.decelerations}', _f(p.playerLoad, 0), p.heartRateAvgBpm > 0 ? _f(p.heartRateAvgBpm, 0) : '—', p.heartRateMaxBpm > 0 ? _f(p.heartRateMaxBpm, 0) : '—', '${p.pointsCount}'
      ];
  ...
  static String _speedZonesTable(TrackerTrainingReport report) {
    final zones = report.speedZones;
    if (zones.isEmpty) return '<div class="muted">Скоростные зоны не пришли с сервера. Границы на графиках рассчитаны по методике U13: HIR ${_reportHsrThreshold(report).toStringAsFixed(1)}, Sprint ${_reportSprintThreshold(report).toStringAsFixed(1)} км/ч.</div>';
    final total = zones.fold<double>(0, (a, z) => a + math.max(0.0, z.distanceM));
    final b = StringBuffer('<table><thead><tr><th>Зона</th><th class="num">Скорость</th><th class="num">Дистанция</th><th class="num">Доля</th></tr></thead><tbody>');
    for (final z in zones) {
      final double pct = total > 0 ? (z.distanceM.toDouble() * 100.0 / total) : 0.0;
      final toLabel = z.toKmh > 0 ? _f(z.toKmh) : '∞';
      b.write('<tr><td><span class="pill">${_h(z.label)}</span></td><td class="num">${_f(z.fromKmh)}-$toLabel км/ч</td><td class="num">${_m(z.distanceM)}</td><td class="num">${_f(pct, 0)}%</td></tr>');
    }
    b.write('</tbody></table>');
    return b.toString();
  }
  ...
  static String _hrZonesTable(List<TrackerTrainingPlayerRow> players) {
    final withHr = players.where((p) => p.heartRateSamplesCount > 0).toList(growable: false);
    if (withHr.isEmpty) return '<div class="muted">Нет записей Polar H10 по выбранным игрокам.</div>';
    final totals = <int>[
      withHr.fold<int>(0, (a, p) => a + p.hrZ1Samples),
      withHr.fold<int>(0, (a, p) => a + p.hrZ2Samples),
      withHr.fold<int>(0, (a, p) => a + p.hrZ3Samples),
    ...
    const names = ['Z1 восстановление', 'Z2 лёгкая', 'Z3 рабочая', 'Z4 высокая', 'Z5 пик'];
    final b = StringBuffer('<table><thead><tr><th>Зона</th><th class="num">Время</th><th class="num">Доля</th></tr></thead><tbody>');
    for (var i = 0; i < totals.length; i++) {
      ...
      b.write('<tr><td>${_h(names[i])}</td><td class="num">$min:$secLabel</td><td class="num">$pctLabel%</td></tr>');
    }
    b.write('</tbody></table>');
    return b.toString();
  }
  ...
  static String _mapSvg(List<TrackerReportPoint> source, {required bool heatMode, required TrackerTrainingReport report}) {
    ...
    final hsr = _reportHsrThreshold(report);
    final sprint = _reportSprintThreshold(report);
    String px(TrackerReportPoint p) => (pad + p.x.clamp(0.0, 1.0) * pitchW).toStringAsFixed(1);
    String py(TrackerReportPoint p) => (pad + p.y.clamp(0.0, 1.0) * pitchH).toStringAsFixed(1);
    final b = StringBuffer('<div class="svg-wrap"><svg viewBox="0 0 ${w.toInt()} ${h.toInt()}" xmlns="http://www.w3.org/2000/svg" role="img">');
    b.write('<rect x="0" y="0" width="$w" height="$h" rx="18" fill="#6E8C6F"/>');
    b.write('<rect x="$pad" y="$pad" width="$pitchW" height="$pitchH" rx="14" fill="#78936F"/>');
    for (var i = 0; i < 11; i++) {
      final x = pad + pitchW * i / 11;
      b.write('<rect x="${x.toStringAsFixed(1)}" y="$pad" width="${(pitchW / 11).toStringAsFixed(1)}" height="$pitchH" fill="${i.isEven ? '#FFFFFF' : '#000000'}" opacity="${i.isEven ? '.08' : '.04'}"/>');
    }
    b.write('<rect x="$pad" y="$pad" width="$pitchW" height="$pitchH" rx="14" fill="none" stroke="#fff" stroke-opacity=".86" stroke-width="3"/>');
    b.write('<line x1="${w / 2}" y1="$pad" x2="${w / 2}" y2="${h - pad}" stroke="#fff" stroke-opacity=".82" stroke-width="2"/>');
    b.write('<circle cx="${w / 2}" cy="${h / 2}" r="72" fill="none" stroke="#fff" stroke-opacity=".78" stroke-width="2"/>');
    b.write('<rect x="$pad" y="${h / 2 - pitchH * .26}" width="${pitchW * .18}" height="${pitchH * .52}" fill="none" stroke="#fff" stroke-opacity=".82" stroke-width="2"/>');
    b.write('<rect x="${w - pad - pitchW * .18}" y="${h / 2 - pitchH * .26}" width="${pitchW * .18}" height="${pitchH * .52}" fill="none" stroke="#fff" stroke-opacity=".82" stroke-width="2"/>');
    if (points.isEmpty) {
    ...
    if (heatMode) {
      final maxValue = points.fold<double>(1.0, (m, p) => math.max(m, p.value <= 0 ? 1.0 : p.value));
      for (final p in points) {
        final ratio = ((p.value <= 0 ? 1.0 : p.value) / maxValue).clamp(.08, 1.0).toDouble();
        final c = ratio > .66 ? '#EF4444' : ratio > .34 ? '#F59E0B' : '#22C55E';
        final r = 3.2 + 5.8 * ratio;
        b.write('<circle cx="${px(p)}" cy="${py(p)}" r="${r.toStringAsFixed(1)}" fill="$c" opacity=".16"/>');
      }
    } else {
      for (var i = 1; i < points.length; i++) {
        final a = points[i - 1];
        ...
        b.write('<line x1="${px(a)}" y1="${py(a)}" x2="${px(p)}" y2="${py(p)}" stroke="$c" stroke-opacity=".86" stroke-width="$sw" stroke-linecap="round"/>');
      }
      final step = math.max(1, (points.length / 120).ceil());
      for (var i = 0; i < points.length; i += step) {
        final p = points[i];
        final c = _speedHex(p.speedKmh, hsr, sprint);
        b.write('<circle cx="${px(p)}" cy="${py(p)}" r="1.7" fill="$c" opacity=".94"/><circle cx="${px(p)}" cy="${py(p)}" r="3.2" fill="$c" opacity=".08"/>');
      }
    }
    b.write('</svg></div>');
    return b.toString();
  ...
  static String _speedSvg(TrackerTrainingReport report) {
    final samples = _sample(report.routePoints.map((p) => p.speedKmh).where((v) => v.isFinite && v >= 0).toList(growable: false), 900);
    const w = 980.0, h = 340.0, left = 48.0, top = 22.0, right = 18.0, bottom = 34.0;
    ...
    for (var i = 0; i <= 4; i++) {
      ...
      b.write('<line x1="$left" y1="$y" x2="${w - right}" y2="$y" stroke="#E5E7EB"/><text x="8" y="${y + 4}" fill="#667085" font-size="12" font-weight="800">${(maxSpeed * i / 4).round()}</text>');
    }
    void threshold(double value, String color, String label) {
      if (value <= 0 || value > maxSpeed) return;
      final y = top + rh - rh * value / maxSpeed;
      b.write('<line x1="$left" y1="$y" x2="${w - right}" y2="$y" stroke="$color" stroke-width="2" stroke-opacity=".65"/><text x="${w - 135}" y="${y - 7}" fill="$color" font-size="13" font-weight="950">$label</text>');
    }
    ...
    if (samples.isNotEmpty) {
      ...
      b.write('<text x="50%" y="50%" text-anchor="middle" fill="#667085" font-size="16" font-weight="900">Нет speed_kmh для графика</text>');
    }
    b.write('</svg></div>');
    return b.toString();
  }

  static String _dualBarSvg(List<TrackerTrainingPlayerRow> players, double Function(TrackerTrainingPlayerRow p) first, double Function(TrackerTrainingPlayerRow p) second, String firstLabel, String secondLabel) {
    final list = players.take(14).toList(growable: false);
    const w = 980.0, h = 360.0, left = 128.0, top = 24.0, right = 28.0, bottom = 26.0;
    final rh = h - top - bottom;
    final maxV = math.max(1.0, list.fold<double>(0, (m, p) => math.max(m, math.max(first(p), second(p))))) * 1.10;
    final rowH = list.isEmpty ? 24.0 : rh / list.length;
    final b = StringBuffer('<div class="svg-wrap"><svg viewBox="0 0 ${w.toInt()} ${h.toInt()}" xmlns="http://www.w3.org/2000/svg">');
    b.write('<rect width="$w" height="$h" rx="16" fill="#FAFBFC"/><text x="$left" y="16" fill="#00A750" font-size="12" font-weight="950">${_h(firstLabel)}</text><text x="${left + 90}" y="16" fill="#F59E0B" font-size="12" font-weight="950">${_h(secondLabel)}</text>');
    if (list.isEmpty) {
      b.write('<text x="50%" y="50%" text-anchor="middle" fill="#667085" font-size="16" font-weight="900">Нет игроков для графика</text></svg></div>');
      return b.toString();
    }
    for (var i = 0; i < list.length; i++) {
      final p = list[i];
      ...
      b.write('<text x="8" y="${y + hBar + 3}" fill="#374151" font-size="11" font-weight="900">${_h(_shortReportPlayerName(p.name))}</text>');
      b.write('<rect x="$left" y="$y" width="$fw" height="$hBar" rx="5" fill="#00A750" opacity=".88"/>');
      b.write('<rect x="$left" y="${y + hBar + 4}" width="$sw" height="$hBar" rx="5" fill="#F59E0B" opacity=".86"/>');
    }
    b.write('</svg></div>');
    return b.toString();
  }

  static String _hrSvg(TrackerTrainingReport report, List<TrackerHeartRatePoint> points) {
    final list = _sample(points.where((p) => p.bpm > 0).toList(growable: false)..sort((a, b) => _hrPointSortKey(a).compareTo(_hrPointSortKey(b))), 900);
    const w = 980.0, h = 340.0, left = 48.0, top = 20.0, right = 18.0, bottom = 34.0;
    final rw = w - left - right, rh = h - top - bottom;
    final b = StringBuffer('<div class="svg-wrap"><svg viewBox="0 0 ${w.toInt()} ${h.toInt()}" xmlns="http://www.w3.org/2000/svg">');
    b.write('<rect width="$w" height="$h" rx="16" fill="#FAFBFC"/>');
    for (var i = 0; i <= 5; i++) {
      ...
      b.write('<line x1="$left" y1="$y" x2="${w - right}" y2="$y" stroke="#E5E7EB"/><text x="7" y="${y + 4}" fill="#667085" font-size="12" font-weight="800">$bpm</text>');
    }
    if (list.isNotEmpty) {
      ...
      b.write('<text x="50%" y="50%" text-anchor="middle" fill="#667085" font-size="16" font-weight="900">Нет HR timeline</text>');
    }
    ...
    b.write('</svg></div><div class="legend">${points.length} HR-точек · Avg $avgHrLabel bpm · Max $maxHrLabel bpm</div>');
    return b.toString();
  }

  static String _microcycleSvg(TrackerTrainingReport report) {
    final points = report.microcycle;
    if (points.isEmpty) return '<div class="muted">Микроцикл не пришёл с сервера.</div>';
    const w = 980.0, h = 340.0, left = 54.0, top = 22.0, right = 20.0, bottom = 46.0;
    final rw = w - left - right, rh = h - top - bottom;
    final maxV = math.max(1.0, points.fold<double>(0, (m, p) => math.max(m, math.max(p.distanceM, math.max(p.highSpeedRunningM, p.accDec * 100)))));
    final b = StringBuffer('<div class="svg-wrap"><svg viewBox="0 0 ${w.toInt()} ${h.toInt()}" xmlns="http://www.w3.org/2000/svg">');
    b.write('<rect width="$w" height="$h" rx="16" fill="#FAFBFC"/>');
    ...
    for (var i = 0; i < points.length; i++) {
      final x = left + rw * (points.length == 1 ? 0 : i / (points.length - 1));
      b.write('<text x="$x" y="${h - 18}" text-anchor="middle" fill="#667085" font-size="11" font-weight="850">${_h(points[i].label)}</text>');
    }
    b.write('</svg></div><div class="legend"><span><i class="dot" style="background:#00A750"></i>дистанция</span><span><i class="dot" style="background:#F59E0B"></i>ВСБ</span><span><i class="dot" style="background:#2563EB"></i>уск/торм</span></div>');
    return b.toString();
  }

  static String _playerCards(TrackerTrainingReport report, List<TrackerTrainingPlayerRow> players) {
    if (players.isEmpty) return '<div class="muted">Нет игроков для индивидуальных страниц.</div>';
    final b = StringBuffer('<div class="player-grid">');
    for (final p in players.take(24)) {
      final avatar = p.avatarUrl.trim().isEmpty ? _h(p.name.trim().isEmpty ? '?' : p.name.trim().substring(0, 1).toUpperCase()) : '<img src="${_h(p.avatarUrl)}" alt="">';
      final pts = report.routePoints.where((pt) => _samePlayerPoint(pt, p)).toList(growable: false);
      final pReport = TrackerTrainingReport(
        ...
        heatmapPoints: pts,
        speedZones: report.speedZones,
        heartRateTimeline: report.heartRateTimeline.where((hr) => _sameHrPoint(hr, p)).toList(growable: false),
      );
      b.write('<div class="player-card"><div class="player-head"><div class="avatar">$avatar</div><div><div class="player-name">${_h(p.name)}</div><div class="muted">${_h(p.duration)} · ${_m(p.distanceM)} · max ${_f(p.maxSpeedKmh)} км/ч · HR ${p.heartRateSamplesCount}</div></div></div>');
      b.write('<div class="grid grid-2"><div>${_mapSvg(pts, heatMode: false, report: report)}</div><div>${_speedSvg(pReport)}</div></div></div>');
    }
    b.write('</div>');
    return b.toString();
  }
  ...
  static String _aiBlock(TrackerTrainingReport report) {
    ...
    final withGps = players.where((p) => p.pointsCount > 0 || p.distanceM > 0).length;
    final items = [
      'Маршрут, тепловая карта, скорость и Polar H10 подготовлены для анализа.',
      'Локомоторика: дистанция, скорость, V3/V4/V5, спринты, скоростные зоны.',
      'Механика: ускорения, торможения, взрывные действия, смены направления.',
      'Внутренняя нагрузка: $withHr игрок(ов) с Polar H10, HR timeline и зоны ЧСС.',
      'Игроки: ${players.length} в отчёте, $withGps с GPS-данными.',
    ];
  ...
  static bool _samePlayerPoint(TrackerReportPoint point, TrackerTrainingPlayerRow player) {
    if (point.playerId != null && player.playerId != null && point.playerId == player.playerId) return true;
    return point.playerName.trim().isNotEmpty && point.playerName.trim().toLowerCase() == player.name.trim().toLowerCase();
  }

  static bool _sameHrPoint(TrackerHeartRatePoint point, TrackerTrainingPlayerRow player) {
    if (point.playerId != null && player.playerId != null && point.playerId == player.playerId) return true;
    return point.playerName.trim().isNotEmpty && point.playerName.trim().toLowerCase() == player.name.trim().toLowerCase();
  }
...
class _PerformanceMatrix extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final rows = players.where((p) => p.name.trim().isNotEmpty).toList(growable: false);
    if (rows.isEmpty) return const _EmptyBlock('Нет игроков с данными для матрицы нагрузки.');
    final avgLoad = rows.fold<double>(0, (v, p) => v + p.playerLoad) / rows.length;
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Padding(
      ...
      SingleChildScrollView(
        ...
        child: SizedBox(
          ...
          child: Table(
            ...
            columnWidths: const {
              0: FixedColumnWidth(210), 1: FixedColumnWidth(82), 2: FixedColumnWidth(90),
              3: FixedColumnWidth(78), 4: FixedColumnWidth(82), 5: FixedColumnWidth(82),
              6: FixedColumnWidth(76), 7: FixedColumnWidth(76), 8: FixedColumnWidth(76),
              9: FixedColumnWidth(76), 10: FixedColumnWidth(82), 11: FixedColumnWidth(82),
              12: FixedColumnWidth(82), 13: FixedColumnWidth(78), 14: FixedColumnWidth(82),
              15: FixedColumnWidth(86), 16: FixedColumnWidth(86), 17: FixedColumnWidth(90),
              18: FixedColumnWidth(92), 19: FixedColumnWidth(94),
            },
            children: [
              _matrixGroupRow(),
              _matrixHeaderRow(),
  ...
  TableRow _matrixGroupRow() => TableRow(children: [
    ...
    _groupCell('МЕХАНИЧЕСКАЯ НАГРУЗКА', const Color(0xFFEAF6EC), spanHint: true),
    for (var i = 0; i < 3; i++) _groupCell('', const Color(0xFFEAF6EC)),
    _groupCell('ЛОКОМОТОРНАЯ НАГРУЗКА', const Color(0xFFFFEEEE), spanHint: true),
    for (var i = 0; i < 4; i++) _groupCell('', const Color(0xFFFFEEEE)),
    _groupCell('ВНУТРЕННЯЯ / КОНТРОЛЬ', const Color(0xFFF1EDFF), spanHint: true),
    for (var i = 0; i < 4; i++) _groupCell('', const Color(0xFFF1EDFF)),
  ]);

  TableRow _matrixHeaderRow() => _tableRow([
    'Игрок','Время','Дист. м','м/мин','Макс.','Load',
    'Уск.','Торм.','УСК+ТОР/мин','Взрыв.',
    'V3','V4 HSR','V5 спринт','Спр.','V4+V5',
    'HR ср.','HR max','Интенс.','Баланс','Статус'
  ], header: true);
  ...
  TableRow _matrixPlayerRow(TrackerTrainingPlayerRow p, double avgLoad) {
    ...
    return TableRow(decoration: const BoxDecoration(color: Colors.white), children: [
      _playerCell(p, false), _cell(p.duration),
      ...
      _matrixValue(p.playerLoad.toStringAsFixed(0), avgLoad <= 0 ? 0 : p.playerLoad / avgLoad),
      ...
      _matrixValue(p.highSpeedWorkM.toStringAsFixed(0), p.highSpeedWorkM / 650),
      _matrixValue(p.heartRateAvgBpm > 0 ? p.heartRateAvgBpm.toStringAsFixed(0) : '—', p.heartRateAvgBpm / 170),
      _matrixValue(p.heartRateMaxBpm > 0 ? p.heartRateMaxBpm.toStringAsFixed(0) : '—', p.heartRateMaxBpm / 195),
  ...
  TableRow _matrixAverageRow(List<TrackerTrainingPlayerRow> rows, double avgLoad) {
    double a(double Function(TrackerTrainingPlayerRow) f) => rows.fold<double>(0, (v,p)=>v+f(p))/rows.length;
    int ai(int Function(TrackerTrainingPlayerRow) f) => (rows.fold<int>(0,(v,p)=>v+f(p))/rows.length).round();
    return TableRow(decoration: const BoxDecoration(color: Color(0xFFE8ECE9)), children: [
      _cell('СРЕДНЕЕ', bold: true), _cell('—'),
      ...
      _cell('${ai((p)=>p.accelerations)}', bold: true), _cell('${ai((p)=>p.decelerations)}', bold: true),
      _cell(a((p)=>p.accDecPerMin).toStringAsFixed(1), bold: true), _cell('${ai((p)=>p.explosiveActions)}', bold: true),
      _cell(a((p)=>p.v3RunM).toStringAsFixed(0), bold: true), _cell(a((p)=>p.v4HsrM).toStringAsFixed(0), bold: true),
      _cell(a((p)=>p.v5SprintM).toStringAsFixed(0), bold: true), _cell('${ai((p)=>p.sprintCount)}', bold: true),
      _cell(a((p)=>p.highSpeedWorkM).toStringAsFixed(0), bold: true),
      _cell(a((p)=>p.heartRateAvgBpm).toStringAsFixed(0), bold: true), _cell(a((p)=>p.heartRateMaxBpm).toStringAsFixed(0), bold: true),
      _cell('${(a(_intensity)*100).round()}%', bold: true), _cell('100%', bold: true), _cell('КОМАНДА', bold: true),
    ]);
  }

  double _intensity(TrackerTrainingPlayerRow p) {
    final external = ((p.metersPerMin / 130) * .34 + (p.highSpeedWorkM / 650) * .26 + (p.explosiveActions / 36) * .20 + (p.maxSpeedKmh / 30) * .20);
    final internal = p.heartRateAvgBpm > 0 ? (p.heartRateAvgBpm / 180) : external;
  ...
  Widget _matrixValue(String text, double ratio) {
    ...
    final color = r >= 1.08 ? const Color(0xFFFFD8D8) : r >= .88 ? const Color(0xFFFFF1B8) : r > 0 ? const Color(0xFFDFF4E8) : const Color(0xFFE9EEF3);
    return Container(height: 38, alignment: Alignment.center, color: color, padding: const EdgeInsets.symmetric(horizontal: 4), child: Text(text, maxLines: 1, style: const TextStyle(fontSize: 10.2, fontWeight: FontWeight.w800, color: _R.text)));
  }

  Widget _statusCell(String status) {
    final color = status == 'ВЫСОКО' ? const Color(0xFFFFD8D8) : status == 'ПОВЫШЕНО' ? const Color(0xFFFFF1B8) : status == 'ОПТИМАЛЬНО' ? const Color(0xFFDFF4E8) : const Color(0xFFE9EEF3);
    return Container(height: 38, alignment: Alignment.center, color: color, padding: const EdgeInsets.symmetric(horizontal: 4), child: Text(status, textAlign: TextAlign.center, style: const TextStyle(fontSize: 8.6, fontWeight: FontWeight.w900, color: _R.text)));
  }

  Widget _groupCell(String text, Color color, {bool spanHint=false}) => Container(height: 32, alignment: Alignment.center, color: color, padding: const EdgeInsets.symmetric(horizontal: 4), child: Text(text, maxLines: 2, textAlign: TextAlign.center, style: TextStyle(fontSize: spanHint ? 9.2 : 8, fontWeight: FontWeight.w900, color: _R.text)));
}

class _MatrixLegend extends StatelessWidget {
  const _MatrixLegend({required this.color, required this.label});
  final Color color; final String label;
  @override Widget build(BuildContext context) => Row(mainAxisSize: MainAxisSize.min, children: [Container(width: 12,height: 12,decoration: BoxDecoration(color: color,borderRadius: BorderRadius.circular(3))),const SizedBox(width:5),Text(label,style: const TextStyle(fontSize:10,fontWeight:FontWeight.w700,color:_R.muted))]);
}
