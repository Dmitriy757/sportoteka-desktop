...
class HeartRateSample {
  ...
  Map<String, dynamic> toJson() => <String, dynamic>{
        ...
        if (batteryPercent != null) 'heart_rate_battery_percent': batteryPercent,
        if (sensorContactDetected != null) 'heart_rate_contact': sensorContactDetected,
        if (rrIntervalsMs.isNotEmpty) 'heart_rate_rr_intervals_ms': rrIntervalsMs,
...
class _HeartRateCharacteristics {
  ...
  const _HeartRateCharacteristics({this.measurement, this.battery, this.bodyLocation});
}
...
class HeartRateBleService {
  ...
  final StreamController<HeartRateSample> _sampleController = StreamController<HeartRateSample>.broadcast();
  final StreamController<String> _logController = StreamController<String>.broadcast();

  final Map<String, HeartRateBleDevice> _devices = <String, HeartRateBleDevice>{};
  final Map<String, _HeartRateConnection> _connections = <String, _HeartRateConnection>{};
  final Map<String, HeartRateSample> _lastSamplesByDeviceId = <String, HeartRateSample>{};

  bool _connectingAny = false;

  Stream<List<HeartRateBleDevice>> get devicesStream => _devicesController.stream;
  ...
  List<HeartRateBleDevice> get connectedInfos => _connections.values.where((c) => c.ready).map((c) => c.info).toList(growable: false);
  Map<String, HeartRateSample> get lastSamplesByDeviceId => Map<String, HeartRateSample>.unmodifiable(_lastSamplesByDeviceId);
  int get connectedCount => connectedInfos.length;

  bool get heartRateReady => _connections.values.any((c) => c.ready);
  int? get batteryPercent => connectedInfo == null ? null : _connections[connectedInfo!.id]?.batteryPercent;

  bool isConnected(String deviceId) => _connections[deviceId]?.ready == true;
  HeartRateSample? lastSampleForDevice(String deviceId) => _lastSamplesByDeviceId[deviceId];
  HeartRateBleDevice? connectedDevice(String deviceId) => _connections[deviceId]?.info;
  ...
  Future<void> scan({
    ...
    for (final c in _connections.values) {
      _devices[c.info.id] = c.info.copyWith(rssi: c.info.rssi, serviceHit: true, rawProbe: false);
    }
    _emitDevices();
    ...
    final modeText = showAllBleCandidates ? 'all-ble-diagnostic' : 'heart-rate-only';
    _log('HEART TEAM SCAN START: adapter=$adapterState · mode=$modeText · connected=${connectedInfos.length}/12 · service=Heart Rate 180D');

    await _addSystemHeartDevices(source: 'pre-scan', includeAllBleCandidates: showAllBleCandidates);
    ...
    Future<void> runStage({
      ...
      try {
        ...
        _log('HEART SCAN STAGE[$stage]: timeout=${stageTimeout.inSeconds}s · androidFineLocation=$androidUsesFineLocation');
        sub = FlutterBluePlus.scanResults.listen((results) {
          for (final result in results) {
            ...
            final serviceUuids = result.advertisementData.serviceUuids.map((u) => u.str.toUpperCase()).toList(growable: false);
            final servicesText = serviceUuids.isEmpty ? 'services=нет' : serviceUuids.take(6).join(',');
            rawServices[id] = servicesText;

            final serviceHit = result.advertisementData.serviceUuids.any((u) => _sameUuid(u.str, HeartRateBleProfile.heartRateServiceUuid));
            final charHint = result.advertisementData.serviceUuids.any((u) => _sameUuid(u.str, HeartRateBleProfile.heartRateMeasurementUuid));
            ...
            final shouldShow = serviceHit || charHint || nameHit || (hasName && usableSignal) || (showAllBleCandidates && usableSignal);
            if (!shouldShow) continue;

            final connected = _connections[id];
            final safeName = name.trim().isEmpty
                ? (serviceHit || charHint ? 'Пульсометр ${_shortId(id)}' : 'BLE ${_shortId(id)} · проверить')
                : name.trim();
            final item = connected?.info.copyWith(rssi: result.rssi, serviceHit: true, rawProbe: false) ??
                HeartRateBleDevice(
                  id: id,
                  name: safeName,
                  rssi: result.rssi,
            ...
            if (isNew) {
              added++;
              final marker = serviceHit || charHint || nameHit ? 'HEART SENSOR?' : 'BLE DIAG?';
        ...
        await FlutterBluePlus.startScan(timeout: stageTimeout, androidUsesFineLocation: androidUsesFineLocation);
        await Future<void>.delayed(stageTimeout + const Duration(milliseconds: 250));
      } catch (e) {
      ...
      _log('HEART SCAN STAGE[$stage] END: raw_seen=${rawSeen.length} · shown=${_devices.length} · added=$added · service_hits=$serviceHits · name_hits=$nameHits · connected=${connectedInfos.length}/12');
    }

    final stableTimeout = timeout < const Duration(seconds: 12)
        ? const Duration(seconds: 12)
        : timeout;
    await runStage(stage: 'normal', stageTimeout: stableTimeout, androidUsesFineLocation: true);

    if (_devices.length <= _connections.length || showAllBleCandidates) {
      await runStage(
        stage: showAllBleCandidates ? 'diagnostic-wide' : 'fallback-no-location',
        stageTimeout: showAllBleCandidates ? const Duration(seconds: 15) : const Duration(seconds: 12),
        androidUsesFineLocation: false,
      );
    }

    await _addSystemHeartDevices(source: 'post-scan', includeAllBleCandidates: showAllBleCandidates);
    _emitDevices();

    final topRaw = rawSeen.values.toList()..sort((a, b) => b.rssi.compareTo(a.rssi));
    final topText = topRaw.take(12).map((d) => '${d.name}/${d.id}/RSSI ${d.rssi}${d.serviceHit ? '/180D' : ''}/${rawServices[d.id] ?? 'services=?'}').join(' | ');
    ...
    if (_devices.isEmpty || visibleNew <= 0 && _connections.isEmpty) {
      ...
      _log('POLAR H10 NOT FOUND DETAIL: $rawHint Для команды подключайте датчики по одному: подключили H10 → выбрали игрока → назначили → снова поиск следующего H10.');
    } else {
      ...
      _log('HEART SCAN END: показано ${_devices.length}, подключено ${connectedInfos.length}/12. $probeHint');
    }
  }
  ...
  Future<void> connect(HeartRateBleDevice info) async {
    ...
    if (current != null && current.ready && await _isDeviceConnected(current.device)) {
      connectedInfo = current.info;
      lastKnownInfo = current.info;
      _log('Пульсометр уже подключён: ${current.info.name} · всего ${connectedInfos.length}/12');
      return;
    }
    ...
    _emitDevices();
    _log('Подключение пульсометра: ${info.name} / ${info.id} · команда ${connectedInfos.length}/12...');

    await _connectAndDiscover(conn);
    _log('Пульсометр подключён: ${conn.info.name}. Ожидаю bpm · всего ${connectedInfos.length}/12');
  }
    ...
    try {
      ...
      conn.connectionSub = device.connectionState.listen((state) {
        if (state == BluetoothConnectionState.disconnected) {
          ...
          lastKnownInfo = info;
          _log('Пульсометр отключился: ${info.name}. Осталось подключено ${connectedInfos.length}/12.');
          _emitDevices();
        }
      });
      ...
      if (measurement == null) {
        ...
        final serviceList = services.map((s) {
          final c = s.characteristics.map((x) => '${x.uuid.str.toUpperCase()}(${_charProps(x)})').join(', ');
          return '${s.uuid.str.toUpperCase()} [$c]';
        }).join(' | ');
        throw Exception('Это BLE-устройство не отдаёт Heart Rate 180D/2A37. Services: $serviceList');
      }
      ...
      if (conn.battery != null && conn.battery!.properties.read) {
        try {
          ...
          if (raw.isNotEmpty) conn.batteryPercent = raw.first.clamp(0, 100).toInt();
        } catch (e) {
      ...
      conn.ready = true;
      connectedInfo = conn.info;
      _devices[conn.info.id] = conn.info.copyWith(serviceHit: true, rawProbe: false);
      _emitDevices();

      conn.notifySub = measurement.onValueReceived.listen((bytes) {
        final sample = _parseHeartRateMeasurement(bytes, conn.info, conn.batteryPercent);
        if (sample == null) return;
        lastSample = sample;
        _lastSamplesByDeviceId[sample.deviceId] = sample;
        _sampleController.add(sample);
        final rr = sample.rrIntervalsMs.isEmpty ? '' : ' · RR ${sample.rrIntervalsMs.take(3).join('/')} мс';
        final contact = sample.sensorContactDetected == null ? '' : ' · контакт ${sample.sensorContactDetected! ? 'есть' : 'нет'}';
        final battery = sample.batteryPercent == null ? '' : ' · батарея ${sample.batteryPercent}%';
  ...
  _HeartRateCharacteristics _findHeartRateCharacteristics(List<BluetoothService> services) {
    BluetoothCharacteristic? measurement;
    BluetoothCharacteristic? battery;
    ...
    for (final service in services) {
      ...
      for (final c in service.characteristics) {
        ...
        if (_sameUuid(serviceUuid, HeartRateBleProfile.heartRateServiceUuid) && _sameUuid(uuid, HeartRateBleProfile.bodySensorLocationUuid)) {
          bodyLocation = c;
        }
        if (_sameUuid(serviceUuid, HeartRateBleProfile.batteryServiceUuid) && _sameUuid(uuid, HeartRateBleProfile.batteryLevelUuid)) {
          battery = c;
        }
      }
    }
    ...
    measurement ??= services
        ...
        .where((c) => _sameUuid(c.uuid.str, HeartRateBleProfile.heartRateMeasurementUuid) && (c.properties.notify || c.properties.indicate))
        .cast<BluetoothCharacteristic?>()
        .firstWhere((c) => c != null, orElse: () => null);

    battery ??= services
        .expand((s) => s.characteristics)
        .where((c) => _sameUuid(c.uuid.str, HeartRateBleProfile.batteryLevelUuid))
        .cast<BluetoothCharacteristic?>()
        .firstWhere((c) => c != null, orElse: () => null);

    return _HeartRateCharacteristics(measurement: measurement, battery: battery, bodyLocation: bodyLocation);
  }

  HeartRateSample? _parseHeartRateMeasurement(List<int> bytes, HeartRateBleDevice info, int? batteryPercent) {
  ...
  Future<void> disconnect({String? deviceId, bool clearInfo = true}) async {
    final ids = deviceId == null ? _connections.keys.toList(growable: false) : <String>[deviceId];
    for (final id in ids) {
      ...
      try {
        if (await _isDeviceConnected(conn.device)) await conn.device.disconnect();
      } catch (_) {}
      if (clearInfo && connectedInfo?.id == id) connectedInfo = connectedInfos.isEmpty ? null : connectedInfos.last;
  ...
  Future<void> _addSystemHeartDevices({required String source, bool includeAllBleCandidates = false}) async {
    ...
    for (final device in candidates.values) {
      ...
      if (!_looksLikeHeartRateName(rawName) && !includeAllBleCandidates && !_connections.containsKey(id)) continue;
      final name = rawName.isEmpty ? 'BLE ${_shortId(id)} · system' : rawName;
      final item = _connections[id]?.info ??
          HeartRateBleDevice(id: id, name: name, rssi: 0, serviceHit: _looksLikeHeartRateName(rawName), rawProbe: !_looksLikeHeartRateName(rawName));
      final isNew = !_devices.containsKey(id);
      _devices[id] = item;
      if (isNew) {
        added++;
  ...
  bool _sameUuid(String a, String b) {
    ...
    return (aa.contains('180D') && bb.contains('180D')) || (aa.contains('2A37') && bb.contains('2A37')) || (aa.contains('2A19') && bb.contains('2A19'));
  }
  ...
  void _emitDevices() {
    int priority(HeartRateBleDevice d) {
      ...
      if (name.contains('POLAR') || name.contains('H10') || d.serviceHit) return 2;
