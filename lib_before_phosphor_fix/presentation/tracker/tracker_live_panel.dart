...
bool get _isTeamMultiLive => widget.onStartTeamLive != null && widget.onStopTeamLive != null;
...
final teamConnectionsChanged =
    oldWidget.connectedTeamPlayerIds.length !=
...
if (widget.onStartTeamLive != null) {
  ...
  setState(() {
    _starting = true;
    _lastProblem = 'Запускаю командный Live для ${widget.teamTrackerCount} GPS-трекеров...';
  });
  try {
    ...
    _setLiveRunning(true);
    _paused = false;
    _lastProblem = 'Командный Live активен · GPS ${widget.teamTrackerCount}';
    _log('TEAM MULTI LIVE START: count=${widget.teamTrackerCount} firstSession=$id');
    _startPolling();
  } catch (e) {
...
double _safeLiveDistance(
  ...
  if (durationSec != null && durationSec > 0) {
    final maxPlausibleM =
        (durationSec * (maxAverageSpeedKmh / 3.6)) + 250.0;
...
final local = _safeLiveSpeed(_mainTrack?.speedKmh ?? 0);
if (local > 0) return local;

final server = _sessions.isEmpty ? 0.0 : _safeLiveSpeed(_sessions.first.speedKmh);
return server;
}
...
final online = monitoredPlayers.where((p) => _isPlayerOnlineLive(p.id)).length;
...
for (final device in widget.savedDevices) {
  ...
  final text = '${device.deviceName} ${device.deviceUuid}'.trim().toLowerCase();
  final isHeart =
      text.contains('polar') ||
      text.contains('h10') ||
      text.contains('heart') ||
    ...
    children: [
      ...
      _NoHoverTap(
        ...
        child: Container(
          ...
          child: const Icon(Icons.layers_clear_rounded, color: _OF.muted2, size: 18),
        ),
      ),
    ],
  ),
...
final currentIds = _monitorPlayersForGrid().map((p) => p.id).toList(growable: false);
setState(() {
    ...
    child: AnimatedContainer(
      ...
      child: Column(
        ...
        children: [
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: Text(number, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: focused ? Colors.white : _OF.graphite, fontSize: 9.3, fontWeight: FontWeight.w800)),
                    ),
                  ),
                ],
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 2),
                    Text(
                      online ? 'online · данные Live' : (gpsReady ? 'GPS готов · ждёт данные' : (heartRate != null ? 'Polar online' : 'ожидает подключения')),
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: TextStyle(color: active ? _OF.green : _OF.muted2, fontSize: compact ? 8.1 : 8.7, fontWeight: FontWeight.w700),
                    ),
                  ],
                ),
              ),
              if (_showTeamOnFocusedField)
                ...
                _NoHoverTap(
                  ...
                  child: const SizedBox(
                    width: 24,
                    height: 24,
                    child: Icon(Icons.close_rounded, color: _OF.muted2, size: 14),
                  ),
                ),
            ],
          ),
          const Spacer(),
          Row(children: [
            Expanded(child: _monitorStripMetric('Дист.', '${distance.toStringAsFixed(0)} м', distance > 0 ? _OF.text : _OF.muted2)),
            Expanded(child: _monitorStripMetric('Скор.', speed.toStringAsFixed(1), speed > 0 ? _OF.text : _OF.muted2)),
            Expanded(child: _monitorStripMetric('Пульс', heartRate == null ? '—' : '${heartRate.bpm}', heartRate == null ? _OF.muted2 : _OF.red)),
            Expanded(child: _monitorStripMetric('Нагрузка', load.toStringAsFixed(0), load > 0 ? loadColor : _OF.muted2)),
          ]),
          if (expandedCard) ...[
            const SizedBox(height: 7),
            _NoHoverTap(
...
return Column(
  ...
  crossAxisAlignment: CrossAxisAlignment.start,
  children: [
    Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _OF.muted2, fontSize: 7.7, fontWeight: FontWeight.w700)),
    const SizedBox(height: 1),
    Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: color, fontSize: 9.8, fontWeight: FontWeight.w900, fontFeatures: const [FontFeature.tabularFigures()])),
  ],
);
}
        ...
        child: Column(
          ...
          children: const [
            ...
            Text(
              ...
              style: TextStyle(color: _OF.text, fontSize: 13.5, fontWeight: FontWeight.w800),
            ),
            SizedBox(height: 5),
            Text(
              ...
              textAlign: TextAlign.center,
              style: TextStyle(color: _OF.muted2, fontSize: 11.0, fontWeight: FontWeight.w700, height: 1.3),
            ),
          ],
        ),
      ),
...
if (!_showTeamOnFocusedField) {
  return focused == null ? const <_RuntimeTrack>[] : <_RuntimeTrack>[focused];
}
...
final teamPlayers = teamMode
    ...
    : <TrackerPlayerOption>[player];
final teamPointCount = tracks.fold<int>(0, (sum, track) => sum + track.points.length);
return _ofPanel(
  title: teamMode
      ? (_activity.requiresField ? 'Поле · команда' : 'Маршрут · команда')
      : (_activity.requiresField ? 'Поле · ${_compactPlayerName(player.name)}' : 'Маршрут · ${_compactPlayerName(player.name)}'),
  subtitle: teamMode
      ? '${teamPlayers.length} выбрано · $teamPointCount точек'
      : (hasGps ? '${focusedTrack.points.length} GPS-точек' : (online ? 'online · ждём GPS' : 'ожидает данные')),
  actions: [
            ...
            child: _activity.requiresField
                ...
                : CustomPaint(
                    painter: _OpenFieldTimelinePainter(
                      points: focusedTrack?.points ?? const <_RuntimePoint>[],
                      running: _running,
                      progressRatio: _timelineProgressRatio(),
                    ),
                    child: Center(
                      child: Text(
                        hasGps ? 'Маршрут и интенсивность обновляются в Live' : 'Ожидаем движение игрока',
                        style: const TextStyle(color: _OF.muted2, fontSize: 11, fontWeight: FontWeight.w700),
                      ),
                    ),
                  ),
          ),
            ...
            child: Row(
              ...
              children: [
                Container(
                  width: 7,
                  height: 7,
                  decoration: BoxDecoration(color: online ? _OF.green : _OF.orange, shape: BoxShape.circle),
                ),
                const SizedBox(width: 5),
                Text(
                  teamMode
                  ...
                  style: const TextStyle(color: _OF.text, fontSize: 9.6, fontWeight: FontWeight.w800),
                ),
              ],
            ),
          ),
...
List<_LiveMetricPoint> _focusedLoadTimeline(_RuntimeTrack? track) {
  ...
  for (var i = 1; i < track.points.length; i++) {
    ...
    final acceleration = (((point.speedKmh - previous.speedKmh) / 3.6) / dt)
        .abs();
    final speedPart =
  ...
  return result.length <= 180
      ? result
      : result.sublist(result.length - 180);
}
...
for (var i = 0; i < players.length; i++) {
  ...
  final heart = _heartRateForPlayerId(player.id);
  final heartAge = heart == null ? 999 : now.difference(heart.measuredAt).inSeconds;
  ...
  final currentSpeed = _safeLiveSpeed(
    math.max(session?.speedKmh ?? 0.0, local?.speedKmh ?? 0.0).toDouble(),
  );
...
final currentHeartValues =
    freshHeartByPlayer.values.map((sample) => sample.bpm).toList(growable: false);
...
final collecting = _running && (onlineCount > 0 || gpsCount > 0 || polarCount > 0);
    ...
    children: [
      SizedBox(
        ...
        child: collecting
            ...
            : Icon(Icons.hourglass_top_rounded, color: color, size: dense ? 15 : 17),
      ),
      ...
      Expanded(
        child: Column(
          ...
          children: [
            Text(
              collecting ? 'Командные данные собираются' : 'Ожидание командных данных',
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
...
final bpmValues = history.map((point) => point.bpm).where((bpm) => bpm > 0).toList(growable: false);
final avgHeartRate = bpmValues.isEmpty
    ? 0
    : (bpmValues.fold<int>(0, (sum, bpm) => sum + bpm) / bpmValues.length).round();
...
final speed = _safeLiveSpeed(math.max(session?.speedKmh ?? 0.0, local?.speedKmh ?? 0.0));
final maxSpeed = _safeLiveSpeed(math.max(session?.maxSpeedKmh ?? 0.0, local?.maxSpeedKmh ?? 0.0));
...
return _ofPanel(
  ...
  subtitle: online ? '${_compactPlayerName(player.name)} · online' : '${_compactPlayerName(player.name)} · ждём данные',
  actions: [
    Container(
      height: 22,
      ...
      child: Text(
        ...
        style: TextStyle(color: markColor, fontSize: 9.2, fontWeight: FontWeight.w900),
      ),
    ),
  ],
  child: Padding(
    ...
    child: Column(
      children: [
        Row(
          children: [
            Expanded(child: _focusedHeartMetric('Сейчас', heartRate == null ? '—' : '${heartRate.bpm}', 'bpm', _OF.red, dense: compact)),
            const SizedBox(width: 6),
            Expanded(child: _focusedHeartMetric('Средний', avgHeartRate <= 0 ? '—' : '$avgHeartRate', 'bpm', _OF.orange, dense: compact)),
            const SizedBox(width: 6),
            Expanded(child: _focusedHeartMetric('Максимум', maxHeartRate <= 0 ? '—' : '$maxHeartRate', 'bpm', _OF.graphite, dense: compact)),
          ],
        ),
        SizedBox(height: compact ? 4 : 7),
        Expanded(
            ...
            child: series.isEmpty && loadTimeline.isEmpty
                ? const Center(
                    child: Text(
                      ...
                      style: TextStyle(color: _OF.muted2, fontSize: 10.6, fontWeight: FontWeight.w700),
                    ),
                  )
                : Stack(
                    fit: StackFit.expand,
        ...
        SizedBox(
          ...
          child: Column(
            children: [
              Expanded(
                child: Row(children: [
                  Expanded(child: _focusedLoadMetric('Дистанция', '${distance.toStringAsFixed(0)} м', dense: compact)),
                  const SizedBox(width: 5),
                  Expanded(child: _focusedLoadMetric('Скорость', speed.toStringAsFixed(1), dense: compact)),
                  const SizedBox(width: 5),
                  Expanded(child: _focusedLoadMetric('Макс.', maxSpeed.toStringAsFixed(1), dense: compact)),
                  const SizedBox(width: 5),
                  Expanded(child: _focusedLoadMetric('Нагрузка', load.toStringAsFixed(0), color: markColor, dense: compact)),
                ]),
              ),
              const SizedBox(height: 5),
              Expanded(
                child: Row(children: [
                  Expanded(child: _focusedLoadMetric('HIR', '${hir.toStringAsFixed(0)} м', dense: compact)),
                  const SizedBox(width: 5),
                  Expanded(child: _focusedLoadMetric('Спринты', '${sprint.toStringAsFixed(0)} м', dense: compact)),
                  const SizedBox(width: 5),
                  Expanded(child: _focusedLoadMetric('ACC/DEC', '$accel/$decel', dense: compact)),
                  const SizedBox(width: 5),
                  Expanded(child: _focusedLoadMetric('Время', _formatDuration(Duration(seconds: durationSec)), dense: compact)),
                ]),
              ),
            ],
          ),
        ...
        if (!compact) ...[
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.health_and_safety_rounded, color: markColor, size: 16),
                const SizedBox(width: 7),
                Expanded(
                  child: Text(
                    ...
                    maxLines: 2,
                    overflow: TextOverflow.ellipsis,
                    style: const TextStyle(color: _OF.text, fontSize: 9.7, fontWeight: FontWeight.w700, height: 1.2),
                  ),
                ),
              ],
            ),
...
return Container(
  ...
  child: Column(
    ...
    mainAxisAlignment: MainAxisAlignment.center,
    children: [
      Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _OF.muted2, fontSize: dense ? 8.0 : 8.8, fontWeight: FontWeight.w700)),
      SizedBox(height: dense ? 1 : 2),
      Row(children: [
        Text(value, style: TextStyle(color: color, fontSize: dense ? 13.5 : 15, fontWeight: FontWeight.w900, fontFeatures: const [FontFeature.tabularFigures()])),
        const SizedBox(width: 3),
        Text(unit, style: TextStyle(color: _OF.muted2, fontSize: dense ? 7.8 : 8.5, fontWeight: FontWeight.w700)),
      ]),
    ],
  ),
);
...
const colors = <Color>[_OF.cyan, _OF.green, _OF.orange, Color(0xFFF97316), _OF.red];
return Row(
    ...
    return Expanded(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Text('Z${index + 1}', style: TextStyle(color: colors[index], fontSize: 8.8, fontWeight: FontWeight.w900)),
              const Spacer(),
              Text('${(ratio * 100).round()}%', style: const TextStyle(color: _OF.muted2, fontSize: 8.2, fontWeight: FontWeight.w700)),
            ]),
            const SizedBox(height: 3),
            ClipRRect(
              borderRadius: BorderRadius.circular(999),
...
return Container(
  ...
  child: Column(
    ...
    mainAxisAlignment: MainAxisAlignment.center,
    children: [
      Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: _OF.muted2, fontSize: dense ? 7.2 : 8.0, fontWeight: FontWeight.w700)),
      SizedBox(height: dense ? 1 : 2),
      Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: color, fontSize: dense ? 9.3 : 10.5, fontWeight: FontWeight.w900, fontFeatures: const [FontFeature.tabularFigures()])),
    ],
  ),
);
}
...
final distance = math.max(session?.totalDistanceM ?? 0.0, local?.totalDistanceM ?? 0.0);
final speed = math.max(session?.speedKmh ?? 0.0, local?.speedKmh ?? 0.0);
final maxSpeed = math.max(_safeLiveSpeed(session?.maxSpeedKmh ?? 0.0), _safeLiveSpeed(local?.maxSpeedKmh ?? 0.0));
final load = math.max(session?.loadScore ?? 0.0, local?.loadScore ?? 0.0);
final sprint = math.max(session?.sprintDistanceM ?? 0.0, local?.sprintDistanceM ?? 0.0);
...
return Column(
  ...
  children: [
    ...
    Row(
      children: [
        Expanded(child: _monitorMetricTile('Нагрузка', load.toStringAsFixed(0), active: load > 0, compact: compact)),
        const SizedBox(width: 5),
    ...
    children: [
      ...
      Row(
        children: [
          ...
          const SizedBox(width: 7),
          Expanded(child: _phoneMetricBox('Нагрузка', load.toStringAsFixed(0))),
        ],
      ),
      const SizedBox(height: 8),
      Row(
        children: [
          ...
          const SizedBox(width: 8),
          Expanded(child: _phoneSmallInfoLine(
            Icons.sensors_rounded,
            _teamTrackerReady
                ? (widget.teamTrackerCount > 0 ? 'GPS online · ${widget.teamTrackerCount}' : 'Трекер online')
                : 'Трекеры offline',
          )),
        ],
      ),
...
final selectedTrack = widget.selectedPlayer == null ? null : _displayTrackForPlayer(widget.selectedPlayer!);
final pointCount = widget.selectedPlayer == null ? _displayPoints : (selectedTrack?.points.length ?? 0);
final hasPoints = pointCount > 0;
final selectedName = widget.selectedPlayer == null ? 'команда' : _compactPlayerName(widget.selectedPlayer!.name);
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(isFieldMode ? 'Поле · $selectedName' : 'Активность · $selectedName', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _OF.text, fontSize: 13.4, fontWeight: FontWeight.w700, letterSpacing: -.25)),
                const SizedBox(height: 1),
                Text('$fieldTitle · ${hasPoints ? 'точек $pointCount' : 'ждём GPS'}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _OF.muted2, fontSize: 11.2, fontWeight: FontWeight.w600)),
              ],
            ),
          ),
    ...
    children: [
      ...
      Row(
        children: [
          ...
          const SizedBox(width: 6),
          Expanded(child: _phoneMetricBox('Нагрузка', selectedLoad.toStringAsFixed(0))),
        ],
      ),
    ],
  ),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 6),
              Expanded(child: _phoneAnalysisMetric('Нагрузка', load.toStringAsFixed(0))),
            ],
          ),
        ],
      ),
...
final number = player.number ?? '${index + 1}';
return Material(
  color: selected || focused ? _OF.greenSoft : (online ? _OF.greenSoft : _OF.header),
  borderRadius: BorderRadius.circular(4),
  child: _NoHoverTap(
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(4),
        border: Border.all(color: selected || focused ? _OF.green : (online ? _OF.greenBorder : _OF.line)),
      ),
      child: Row(
        children: [
          Stack(
            ...
            children: [
              CircleAvatar(
                radius: phone ? 12 : 14,
                backgroundColor: selected || focused ? _OF.green : (online ? _OF.green : Colors.white),
          ...
          Expanded(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(_compactPlayerName(player.name), maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: selected || focused ? _OF.green : _OF.text, fontSize: phone ? 9.4 : 10.2, fontWeight: FontWeight.w700)),
                const SizedBox(height: 2),
                Row(
                  children: [
...
Widget primaryLiveButton({required bool phone}) {
  ...
  return SizedBox(
    ...
    child: FilledButton.icon(
      ...
      icon: Icon(_paused ? Icons.play_arrow_rounded : (_running ? Icons.stop_rounded : Icons.play_arrow_rounded), size: 15),
      label: Text(
        text,
        style: _OFText.button(
          color: accent,
...
return Container(
  ...
  child: SingleChildScrollView(
    ...
    child: Row(
      children: [
      _ofStatusDot(_paused ? _OF.orange : (_running ? _OF.green : _OF.muted2)),
      const SizedBox(width: 8),
      Text(
...
return _phonePlayerCard(
  ...
  load: math.max(session?.loadScore ?? 0.0, local?.loadScore ?? 0.0),
  max: math.max(_safeLiveSpeed(session?.maxSpeedKmh ?? 0.0), _safeLiveSpeed(local?.maxSpeedKmh ?? 0.0)),
...
return _phonePlayerCard(
  ...
  max: math.max(_safeLiveSpeed(s.maxSpeedKmh), _safeLiveSpeed(local?.maxSpeedKmh ?? 0.0)),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(connectionText, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: online || gpsReady ? _OF.green : _OF.muted2, fontSize: 9.6, fontWeight: FontWeight.w700)),
                  ],
                ),
              ),
  ...
  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(_activity.requiresField ? 'Точка и траектория на поле' : (_activity == TrackerTrainingActivity.outdoorRun ? 'Бег: маршрут, темп и интервалы' : 'Активность без поля'), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _OF.text, fontSize: 12.5, fontWeight: FontWeight.w700)),
        const SizedBox(height: 2),
        Text(hasTrack ? (_activity == TrackerTrainingActivity.outdoorRun ? 'контроль темпа, дистанции, ходьбы и спринтов в Live' : 'движение игрока отображается отдельно от команды') : 'ожидаем GPS-точки игрока', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _OF.muted, fontSize: 11.2, fontWeight: FontWeight.w700)),
      ])),
      if (_activity.requiresField) ...[
...
final title = widget.teamTrackerCount > 0
    ...
    : (ready ? (device?.name ?? 'Подключён') : (device == null ? 'Не подключён' : '${device.name} · вне зоны'));
return _miniOfCard('Трекер', title, Row(children: [
  ...
  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
    Text(widget.teamTrackerCount > 0 ? 'Командные GPS-каналы' : (device == null ? 'SPORTOTEKA GPS PRO' : device.name), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _OF.text, fontSize: 11.2, fontWeight: FontWeight.w500)),
    const SizedBox(height: 3),
    Row(children: [_ofStatusDot(ready ? _OF.green : (device == null ? _OF.red : _OF.orange)), const SizedBox(width: 5), Expanded(child: Text(widget.teamTrackerCount > 0 ? 'TX/RX готовы: ${widget.teamTrackerCount}' : (ready ? 'TX/RX готов' : (device == null ? 'Офлайн' : 'Вне зоны · автопоиск')), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _OF.text, fontSize: 11.2, fontWeight: FontWeight.w500)))]),
    const SizedBox(height: 3),
...
final metrics = <Widget>[
  ...
  _bottomMetricChip('Макс.', '${_safeLiveSpeed(_mainTrack?.maxSpeedKmh ?? 0).toStringAsFixed(1)}'),
...
class _LiveLineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final heartPoints =
        series.expand((s) => s.points).toList(growable: false);
...
for (final track in tracks) {
  ...
  if (showVectors) _drawVector(canvas, pitch, track, bounds, focused: focused);
...
final paint = Paint()
  ..color = (focused ? _OF.green : _trackAccentColor(track)).withOpacity(.88)
  ..strokeWidth = 2.4
  ..strokeCap = StrokeCap.round;
