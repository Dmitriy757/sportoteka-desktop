...
class _PlayerProfileEditorPanelState extends State<PlayerProfileEditorPanel> {
  ...
  void initState() {
    ...
    height = TextEditingController(text: _s(p['height']));
    weight = TextEditingController(text: _s(p['weight']));
  ...
  Widget _medicalForm() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      TextField(controller: title, decoration: _decoration('Название / диагноз')),
      const SizedBox(height: 12),
      TextField(
        controller: note,
        minLines: 6,
      ...
      Material(
        ...
        child: ListTile(
          ...
          title: Text('Дата записи', style: PpText.body(11, color: PpColors.text, weight: FontWeight.w600)),
          subtitle: Text(DateFormat('dd.MM.yyyy').format(date), style: PpText.body(11)),
          trailing: Icon(_dateCalendarOpen ? Icons.keyboard_arrow_up_rounded : Icons.calendar_month_rounded, color: PpColors.green),
          onTap: _pickDate,
        ),
      ),
      const SizedBox(height: 12),
  ...
  Widget _field(TextEditingController controller, String label) => TextField(controller: controller, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: _decoration(label));
  InputDecoration _decoration(String label) => InputDecoration(labelText: label, filled: true, fillColor: PpColors.soft, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: PpColors.green)));
  ...
  Future<void> _save() async {
    ...
    try {
      if (widget.mode == PlayerProfileEditorMode.metrics) {
        await widget.onSaveMetrics({'height': height.text.trim(), 'weight': weight.text.trim(), 'metrics_date': DateFormat('yyyy-MM-dd').format(DateTime.now())});
      } else {
