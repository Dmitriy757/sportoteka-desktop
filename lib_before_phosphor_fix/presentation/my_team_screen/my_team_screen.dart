...
// ✅ НОВОЕ: экран игрока (меню игрока)
...
class _MyTeamScreenState extends State<MyTeamScreen> {
  ...
  static const String checkTeamClubAccessUrl = "$apiBase/check_team_club_access.php";
  ...
  // ✅ команда игрока (его основная команда)
  static const String getPlayerTeamUrl = "$apiBase/get_player_team.php";

  // =============================
  // STATE
  // =============================
  List<Map<String, dynamic>> teams = []; // "Мои команды" (созданные)
  List<Map<String, dynamic>> assignedTeams = []; // доступ/назначенные команды
  List<Map<String, dynamic>> players = [];

  // ✅ “Моя команда” для игрока (одна основная команда)
  Map<String, dynamic>? playerTeam;
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      await pref.init();
      final role = pref.getUserRole().toString().trim().toLowerCase();
      _isPlayerRole = role == "player" || role == "игрок";
    } catch (_) {
      _isPlayerRole = false;
    }

    // ✅ если пришли с teamId — для игрока сразу открываем PlayerDashboardScreen
    if (passedTeamId != null && passedTeamId > 0) {
      if (_isPlayerRole) {
        _openPlayerDashboard(teamId: passedTeamId!, teamName: passedTeamName.isEmpty ? "Команда #$passedTeamId" : passedTeamName);
        return;
      }
    ...
    // ✅ общий режим: грузим списки + (если игрок) грузим его “Мою команду”
    await _loadAllTeamsAndPlayerTeam();
  }

  Future<void> _loadAllTeamsAndPlayerTeam() async {
    if (!mounted) return;
    setState(() => isLoading = true);

    // 1) “Мои команды” (созданные) — не блокируем для игрока
    await fetchTeams(silent: true);

    // 2) “Команды с доступом” (назначенные)
    await fetchAssignedTeams(silent: true);

    // 3) “Моя команда” для игрока (из get_player_team.php)
    if (_isPlayerRole) {
      await fetchPlayerTeam(silent: true);
    }

    // автосоздание команды — только не игроку
    if (!_isPlayerRole && teams.isEmpty && assignedTeams.isEmpty) {
  ...
  // ✅ PLAYER NAV
  // =============================
 Future<void> _openPlayerDashboard({
  required int teamId,
  ...
  if (uid == null || uid == 0) {
    _snackErr("Не найден user_id. Перезайдите в аккаунт.");
    return;
  }

  Get.to(() => PlayerDashboardScreen(
        teamId: teamId,
        teamName: teamName,
        userId: uid,
        teamLogo: teamLogo,
  ...
  // ✅ PLAYER TEAM
  // =============================
  Future<void> fetchPlayerTeam({bool silent = false}) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => playerTeamLoading = false);
      }
    }
  }

 void _openTeamFromPlayerTeam() {
  final t = playerTeam;
  if (t == null) return;

  final id = _asInt(t["team_id"] ?? t["id"]);
  final name = _asStr(t["team_name"] ?? t["name"]).trim();
  final logo = _asStr(t["team_logo"] ?? t["logo"]).trim();

  if (id <= 0) {
    _snackErr("team_id не найден");
    return;
  }

  if (_isPlayerRole) {
    _openPlayerDashboard(
      teamId: id,
      teamName: name.isEmpty ? "Команда #$id" : name,
      teamLogo: logo,
    );
    return;
  }
  ...
  String _normAccess(String raw) {
    ...
    if (s.isEmpty) return "";
    if (s.contains("admin") || s.contains("owner") || s.contains("root")) return "admin";
    if (s.contains("edit") || s.contains("write") || s.contains("coach")) return "edit";
    if (s.contains("view") || s.contains("read")) return "view";
    if (s.contains("pro") || s.contains("premium") || s.contains("club")) return "edit";
    return s;
  }

  bool get canEditTeam {
    // ✅ игроку нельзя редактировать
    if (_isPlayerRole) return false;
  ...
  Future<void> fetchAssignedTeams({bool silent = false}) async {
    ...
    try {
      ...
      if ((data["success"] == true || data["status"] == "success") && data["teams"] is List) {
 ...
 void _selectTeam(Map<String, dynamic> team, {required bool forceClub}) {
  if (_isPlayerRole) {
    ...
    final logo = _asStr(team["team_logo"] ?? team["logo"]).trim();
  ...
  Future<void> _pickAndUploadTeamLogo() async {
    ...
    try {
      ...
      if (data["success"] == true || data["status"] == "success") {
        ...
        _snackErr(_asStr(data["message"]).isEmpty ? "Не удалось обновить фото" : _asStr(data["message"]));
      }
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> _fetchTeamEvents() async {
    ...
    if ((data["success"] == true || data["status"] == "success") && data["events"] is List) {
  ...
  Future<void> _openAttendancePicker() async {
    ...
    // ✅ игроку это не показываем
    if (_isPlayerRole) {
      Get.snackbar("Доступ ограничен", "Посещаемость доступна тренеру/админу.");
      return;
    }
    ...
    try {
      ...
      if (events.isEmpty) {
        Get.snackbar("Посещаемость", "Событий нет. Добавьте тренировку/игру в календарь.");
        return;
      }
  ...
  Widget _buildDefaultLogo() {
    return Container(
      ...
      child: Center(
        child: Icon(Icons.shield_outlined, size: 30, color: AppColors.primaryGreen),
      ),
    );
  }
                  ...
                  Positioned(
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          color: canEditTeam ? AppColors.primaryGreen : AppColors.textTertiary,
                          shape: BoxShape.circle,
                        ),
                        child: Icon(
                          canEditTeam ? Icons.photo_camera_outlined : Icons.lock_outline,
                          size: 16,
  ...
  // ✅ Список команд
  Widget _teamsList() {
    final bool nothingAll = (teams.isEmpty) && (assignedTeams.isEmpty) && (playerTeam == null);
        ...
        children: [
          ...
          Row(
            children: [
              ...
              if (playerTeamLoading)
                const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2))
              else
                IconButton(
                  tooltip: "Обновить",
                  onPressed: () => fetchPlayerTeam(silent: false),
          ...
          if (playerTeam == null && !playerTeamLoading) ...[
            Container(
              padding: const EdgeInsets.all(14),
              decoration: BoxDecoration(
                color: AppColors.card,
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 6),
                  const Text(
                    "Попросите тренера добавить вас в состав — и здесь появится ваша команда.",
                    style: TextStyle(
                      color: AppColors.textTertiary,
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      label: const Text("Проверить привязку"),
                    ),
                  ),
                ],
              ),
            ...
            _teamListTilePlayerTeam(playerTeam!),
          ],
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () => Get.toNamed(AppRoutes.createTeamScreen)?.then((_) => _loadAllTeamsAndPlayerTeam()),
          ...
          // ✅ Команды клуба (доступ), если есть
          ...
          if (nothingAll) ...[
            const SizedBox(height: 18),
            const Text(
              "Пока здесь пусто. Создайте команду или попросите тренера добавить вас в состав.",
              style: TextStyle(
                color: AppColors.textSecondary,
                fontWeight: FontWeight.w700,
              ),
  ...
  // ✅ отдельный tile для playerTeam
  Widget _teamListTilePlayerTeam(Map<String, dynamic> t) {
    final id = _asInt(t["team_id"] ?? t["id"]);
    final name = _asStr(t["team_name"] ?? t["name"]).trim();
    final logo = _asStr(t["team_logo"] ?? t["logo"]).trim();

    return InkWell(
      onTap: _openTeamFromPlayerTeam,
      borderRadius: BorderRadius.circular(16),
      child: Container(
              ...
              child: ClipRRect(
                ...
                child: logo.isNotEmpty
                    ? Image.network(
                        ...
                        height: 44,
                        fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => _buildListTileLogo(true),
                      )
                    : _buildListTileLogo(true),
              ),
            ),
  ...
  Widget _teamListTile(Map<String, dynamic> t, {required bool isAssigned}) {
    ...
    final name = _asStr(isAssigned ? (t["team_name"] ?? t["name"]) : t["name"]).trim();
    ...
    return InkWell(
      onTap: () {
        // ✅ игрок: открываем PlayerDashboardScreen
       if (_isPlayerRole) {
  if (id <= 0) {
    _snackErr("team_id не найден");
    return;
              ...
              child: ClipRRect(
                ...
                child: logo.isNotEmpty
                    ? Image.network(
                        ...
                        height: 44,
                        fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => _buildListTileLogo(isAssigned),
                      )
  ...
  Widget _buildListTileLogo(bool isAssigned) {
    return Container(
      decoration: BoxDecoration(
        color: isAssigned ? const Color(0xFFE3F2FD) : AppColors.primaryGreen.withOpacity(0.1),
        borderRadius: BorderRadius.circular(22),
      ),
      child: Center(
        child: Icon(
          Icons.shield_outlined,
          color: isAssigned ? const Color(0xFF1565C0) : AppColors.primaryGreen,
          size: 22,
        ),
      ),
    );
  ...
  // ✅ ТЕЛО С ВЫБРАННОЙ КОМАНДОЙ (только НЕ-игрок использует этот режим)
  Widget _teamScrollableBody() {
    final mainItems = <Map<String, dynamic>>[
      ...
      },
      {
        ...
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "Чат подключим следующим этапом"),
      },
      {
        "t": "Визитная карточка",
    ...
    final proItems = <Map<String, dynamic>>[
      {
        ...
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "Редактор подключим следующим этапом"),
      },
      {
        "t": "Планы-конспекты",
      ...
      },
      {
        ...
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "Видеоанализ подключим следующим этапом"),
      },
      {
        "t": "Тепловая карта",
        ...
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "Тепловую карту подключим следующим этапом"),
      },
      {
        "t": "Тестирование",
        ...
        "theme": ModuleThemes.basic,
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "Тестирование подключим следующим этапом"),
      },
      {
        "t": "AI-анализ техники",
        ...
        "theme": ModuleThemes.analytics,
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "AI-технику подключим следующим этапом"),
      },
      {
        "t": "AR-Paint цели",
        "s": "Цели на поле в AR",
        ...
        "theme": ModuleThemes.editor,
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "AR-Paint подключим следующим этапом"),
      },
      {
        ...
        "theme": ModuleThemes.analytics,
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "AI-план подключим следующим этапом"),
      },
      {
        "t": "One Ring",
        ...
        "theme": ModuleThemes.basic,
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "One Ring подключим следующим этапом"),
      },
      {
        "t": "Файлообменник",
        ...
        "theme": ModuleThemes.basic,
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "Файлообмен подключим следующим этапом"),
      },
      {
        "t": "Новости команды",
        ...
        "enabled": true,
        "onTap": () => Get.snackbar("Модуль", "Новости подключим следующим этапом"),
      },
    ];
      ...
      children: [
        ...
        Padding(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _sectionTitle("Основные модули", "всего: ${mainItems.length}"),
              const SizedBox(height: 8),
              _modulesGrid(mainItems),
              const SizedBox(height: 16),
              _sectionTitle("Профессиональные инструменты", "всего: ${proItems.length}"),
              const SizedBox(height: 8),
  ...
  Widget build(BuildContext context) {
    // ✅ игрок не должен видеть режим “showTeam” в MyTeamScreen (он уходит в PlayerDashboard)
    final showTeam = (!_isPlayerRole) && (selectedTeamId != 0);
