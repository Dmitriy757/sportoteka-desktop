...
class _CmrCalendarPanelState extends State<CmrCalendarPanel> {
  ...
  TeamEvent? _eventForRightPane(List<TeamEvent> selectedList) {
    ...
    if (_workPanel == _CalendarWorkPanel.details && current != null) {
  ...
  Widget build(BuildContext context) {
    ...
    if (error != null) {
      return _CmrEmptyState(
        ...
        text: error ?? 'Ошибка загрузки календаря',
          ...
          : Column(
              ...
              children: [
                for (var index = 0; index < selectedList.length; index++) ...[
                  _CmrEventTile(
                    event: selectedList[index],
                    canEdit: canEdit,
                    onDetails: () => _openDetails(selectedList[index]),
                    onEdit: () => _openEdit(selectedList[index]),
                    onDelete: () => _delete(selectedList[index]),
                    onRating: () => _openTrainingRatings(selectedList[index]),
                  ),
                  if (index != selectedList.length - 1) const SizedBox(height: 6),
                ],
              ],
            ),
    );
      ...
      children: [
        ...
        LayoutBuilder(
          builder: (context, gridBox) {
            ...
            final limitHeight = maxHeight;
            final maxGridHeight = limitHeight != null && limitHeight.isFinite ? math.max(180.0, limitHeight - 20.0) : null;
...
class _C {
  ...
  static const Color surface = Color(0xFFFAFCFD);
  static const Color soft = Color(0xFFF3F5F7);
  static const Color soft2 = Color(0xFFF5F7FB);
  static const Color glass = Color(0xFFF3F5F7);
  ...
  static const Color accentSoft = Color(0xFFF1FBF6);
  ...
  Widget build(BuildContext context) {
    ...
    final header = Row(
      children: [
        ...
        if (trailingWidget != null) ...[SizedBox(width: dense ? 6 : 8), trailingWidget],
      ],
    );
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              if (canEdit) ...[
                const SizedBox(width: 6),
                if (isTrainingLike && ratingTap != null) ...[
                  _MiniIconButton(icon: Icons.star_rate_rounded, onTap: ratingTap),
                  const SizedBox(width: 5),
                ],
            ...
            children: [
              ...
              if (actionText != null && onAction != null) ...[
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(actionText ?? 'Повторить'),
