...
class AdvancedVideoAnalysisPlaybackController extends ChangeNotifier {
  ...
  bool get isReady => _state?._isVideoReady ?? false;
  bool get isPlaying => !(_state?._isVideoPaused ?? true);
  bool get isLoading => _state?._isLoading ?? false;
  String get status => _state?._cleanStatus(_state?._connectionStatus ?? '') ?? '';
...
class AdvancedVideoAnalysisScreen extends StatefulWidget {
  ...
  /// false — использовать этот виджет как обычный синхронный видеоплеер без WebSocket/AI.
  /// Нужен для левого блока «Прямая трансляция», чтобы нижняя общая кнопка Play
  /// управляла видео без запуска второго AI-потока.
  final bool connectAi;

  /// false — не рисовать AI-квадраты поверх видео.
...
class _AdvancedVideoAnalysisScreenState extends State<AdvancedVideoAnalysisScreen> {
  ...
  void initState() {
    ...
    if (widget.connectAi) {
      ...
      _connectionStatus = 'Загрузка видео...';
      _isLoading = true;
    }
...
<body>
  ...
  <script>
    ...
    let isMuted = false;
    let lastStateEmit = 0;
    let hostWantedPlay = false;

    function tryHostPlay() {
      if (!video) return;
      if (video.readyState >= 2) {
        video.play().catch(function(e){
          console.log('Host play failed', e);
        });
      } else {
    ...
    video.addEventListener('loadeddata', function() {
      emitVideoState('loadeddata');
      if (hostWantedPlay) tryHostPlay();
    });
    ...
    video.addEventListener('waiting', function() {
      emitVideoState('waiting');
    });

    video.addEventListener('error', function() {
      emitVideoState('video_error');
    });
    ...
    window.sportotekaHostPlay = function() {
      hostWantedPlay = true;
      tryHostPlay();
      emitVideoState('host_play_requested');
    };
    window.sportotekaHostPause = function() {
      hostWantedPlay = false;
      if (video) video.pause();
      emitVideoState('host_pause_requested');
    };
...
'''.replaceAll('__SPORTOTEKA_CONTROLS_DISPLAY__', widget.hideControls ? 'none' : 'flex');
  ...
  void _connectWebSocket({bool forceRestart = false}) {
    ...
    if (!_wsListenersReady) {
      ...
      _analysisSub = _wsService.analysisStream.listen(
        (result) {
          if (!mounted || !widget.connectAi) return;
  ...
  void _handleVideoBridgeMessage(JavaScriptMessage message) {
    try {
      ...
      final nextDurationMs = _asDouble(map['duration_ms']);
      final readyState = _asDouble(map['ready_state']);

      _videoStateReady = true;
      _currentVideoTimeMs = nextTimeMs;
      if (nextDurationMs > 0) _videoDurationMs = nextDurationMs;
      if (readyState >= 1 || reason == 'loadedmetadata' || reason == 'loadeddata' || reason == 'canplay') {
        _isVideoReady = true;
        if (!widget.connectAi && mounted) {
          setState(() {
            _connectionStatus = 'Видео готово';
            _isLoading = false;
          });
        }
      }
      if (reason == 'waiting' && mounted && !widget.connectAi) {
        setState(() {
          _connectionStatus = 'Загрузка видео...';
          _isLoading = true;
        });
      }
      if (reason == 'video_error' && mounted && !widget.connectAi) {
        setState(() {
          _connectionStatus = 'Видео не открылось во встроенном плеере';
          _isLoading = false;
        });
      }
    ...
    return LayoutBuilder(
      ...
      child: Stack(
        children: [
          ...
          if ((_isLoading || !_isVideoReady) && (widget.connectAi ? _players.isEmpty : true))
            Center(
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      widget.connectAi
                          ? (_isVideoReady ? 'Подключение к AI-анализу...' : 'Загрузка видео...')
                          : 'Загрузка видео...',
                      textAlign: TextAlign.center,
                      style: _AvaText.title(13),
                    ),
                    const SizedBox(height: 7),
                    if (!widget.embedded)
                      Text(
                        widget.connectAi ? 'Идёт подключение и подготовка анализа' : 'Подготовка встроенного плеера',
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        textAlign: TextAlign.center,
