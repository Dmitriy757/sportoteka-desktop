...
class AdvancedVideoAnalysisPlaybackController extends ChangeNotifier {
  final List<_AdvancedVideoAnalysisScreenState> _states = [];
  ...
  void _attach(_AdvancedVideoAnalysisScreenState state) {
    ...
    _states.add(state);
    notifyListeners();
    Future.microtask(_flushPendingCommands);
  }
  ...
  Future<void> _flushPendingCommands() async {
    ...
    final pendingSpeed = _pendingSpeed;
...
class AdvancedVideoAnalysisScreen extends StatefulWidget {
  ...
  /// false — использовать как обычный видеоплеер без WebSocket/AI-подключения.
  final bool connectAi;

  /// false — не рисовать AI overlay/квадраты поверх видео.
  ...
  static Future<T?> show<T>(
    ...
    AdvancedVideoAnalysisPlaybackController? externalPlaybackController,
    bool hideControls = false,
    bool connectAi = true,
    bool showAiOverlay = true,
  }) {
    return showAdvancedVideoAnalysisWindow<T>(
      context,
      params,
...
<body class="__SPORTOTEKA_BODY_CLASS__">
  ...
  <script>
    ...
    if (!HOST_CONTROLLED) {
      playBtn.addEventListener('click', toggleLocalPlayback);
...
unawaited(_allowAndroidInlineVideoPlayback());
  ...
  void _handleVideoBridgeMessage(JavaScriptMessage message) {
    try {
      ...
      if (nextDurationMs > 0) {
        _videoDurationMs = nextDurationMs;
      }
      ...
      if (reason == 'canplay' || reason == 'loadedmetadata') {
        if (_userWantsPlayback && nextPaused) {
          Future.delayed(const Duration(milliseconds: 60), () {
            if (mounted && _userWantsPlayback) _hostPlay();
          });
        }
      }
      ...
      if (widget.connectAi && widget.showAiOverlay &&
          (nextPaused || reason == 'pause' || reason == 'seeked' || reason == 'loadedmetadata')) {
        _applyOverlayForVideoTime(force: true);
      }
    } catch (e) {
  ...
  Future<void> _hostPlay() async {
    ...
    try {
      ...
      (function(){
        if (window.sportotekaHostPlay) {
          window.sportotekaHostPlay();
        } else {
          window.__sportotekaPendingHostPlay = true;
        }
      })();
    """);
      ...
      debugPrint('⚠️ AI host play pending: $e');
    }
  }
  ...
  Future<void> _hostPause() async {
    ...
    """);
      ...
      debugPrint('⚠️ AI host pause ignored: $e');
    }
  }
