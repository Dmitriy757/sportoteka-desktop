...
class _TrackerMatchWorkspaceScreenState extends State<TrackerMatchWorkspaceScreen> {
  ...
  Future<void> _logRemote(
    ...
    try {
      ...
      await _liveApi.sendDebugLog(
        ...
        appVersion: 'tracker_63_tablet_all_touch_layout' ,
  ...
  Future<void> _connectGpsAndAssignToSelectedPlayer(ActionTrackerDevice device) async {
    ...
    try {
      ...
      await _loadServerData();
      _toast('GPS-трекер', '${device.name} → ${player.name}');
    } catch (e) {
  ...
  Widget _devices() {
    ...
    TrackerDeviceModel? savedGpsForUuid(String uuid) {
      for (final d in _mergedSavedDevices) {
        if (d.deviceUuid == uuid && !_isHeartRateDeviceModel(d)) return d;
      }
      return null;
    }
                ...
                child: Row(children: [
                  connectChip(
                    ...
                    onTap: () {
                      ...
                      _scan(clean: true, universalMode: true);
                    },
                  ),
                  const SizedBox(width: 8),
                  connectChip(
    ...
    Widget gpsDeviceRow(ActionTrackerDevice d) {
      ...
      final saved = savedGpsForUuid(d.id);
      ...
      return Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Row(children: [
            ...
            _DarkActionButton(icon: Icons.refresh_rounded, label: 'Найти GPS', onTap: _scanning ? null : () => _scan(clean: true, universalMode: true)),
          ]),
          const SizedBox(height: 10),
          Expanded(
            child: StreamBuilder<List<ActionTrackerDevice>>(
            ...
            children: [
              ...
              if (gpsDevice == null)
                ...
                assignedDeviceCard(
                  ...
                  connected: _ble.commandChannelReady && _ble.connectedInfo?.id == gpsDevice.deviceUuid,
                  battery: gpsDevice.batteryPercent ?? (_ble.connectedInfo?.id == gpsDevice.deviceUuid ? _batteryPercent : null),
                  rssi: null,
                  onRefresh: () {
                    if (_ble.commandChannelReady && _ble.connectedInfo?.id == gpsDevice.deviceUuid) {
                      _requestGpsRecordsFromTracker();
                    } else {
                      _scan(clean: true, universalMode: true);
                    }
                  },
                  ...
                  mobileCard(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                      ...
                      Row(children: [
                        ...
                        const SizedBox(width: 8),
                        compactMetric(Icons.fitness_center_rounded, '${summary.playerLoad.toStringAsFixed(0)}', 'Load'),
                      ]),
                    ]),
                  ),
                if (_heartRate)
              ...
              return ListView(
                ...
                children: [
                  ...
                  if (_zones || _speedChart || _mechanics || _internal) Row(children: [
                    ...
                    if (_mechanics || _internal) Expanded(child: previewCard(title: 'Игровая нагрузка', height: 170, child: Row(children: [_previewMetric('${(report?.summary.playerLoad ?? 0).toStringAsFixed(0)}', 'Load'), _previewMetric('${(report?.summary.accelerationCount ?? s.accelCount)}', 'Уск.'), _previewMetric('${(report?.summary.decelerationCount ?? s.decelCount)}', 'Торм.')]))),
                  ]),
                ],
              );
            }
