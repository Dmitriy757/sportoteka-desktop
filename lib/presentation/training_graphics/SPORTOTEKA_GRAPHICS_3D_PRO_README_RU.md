# Sportoteka Training Graphics — 3D PRO

Версия модуля после ревизии `training_graphics(3).zip`.

## Что изменено

- Добавлен **Sportoteka 3D PRO** (`flutter_3d_pro_screen.dart`). Unity, WebView, `three_dart` и отдельный 3D runtime не требуются.
...
- Старые `field_3d` и `unity` экраны оставлены как совместимые точки входа, но теперь тоже используют 3D PRO и больше не импортируют Unity/three_dart.
...
4. Для 3D PRO не нужны Unity, `flutter_embed_unity`, `three_dart` и `three_dart_jsm` именно этому модулю. Если они больше нигде в приложении не используются, зависимости можно удалить из `pubspec.yaml` основного проекта.
