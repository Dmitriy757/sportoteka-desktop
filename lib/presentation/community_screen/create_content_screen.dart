...
/// Куда должен попасть именно ПОСТ. Reels остаётся отдельным типом контента.
enum CreatePostDestination { community, profile }

class CreateContentScreen extends StatefulWidget {
  final CreateContentType initialType;
  final String sportName;
  final CreatePostDestination postDestination;
  /// В Community создаём только публикации ленты; в профиле можно переключаться Post/Reels.
  final bool allowReels;
  ...
  const CreateContentScreen({
    ...
    this.postDestination = CreatePostDestination.community,
    this.allowReels = true,
    this.authorLabel = '',
    this.teamId = 0,
    this.clubId = 0,
    this.teamName = '',
...
class _CreateContentScreenState extends State<CreateContentScreen> {
  ...
  String get _postVisibility =>
      widget.postDestination == CreatePostDestination.profile
          ? 'profile'
          : 'feed';

  String get _postDestinationLabel =>
      widget.postDestination == CreatePostDestination.profile
          ? 'вашем профиле'
          : 'ленте';
  ...
  void initState() {
    super.initState();
    _type = widget.allowReels ? widget.initialType : CreateContentType.post;
  ...
  void _selectType(CreateContentType type) {
    if (!widget.allowReels && type == CreateContentType.reel) return;
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                widget.allowReels ? 'Создать' : 'Новая публикация',
                maxLines: 1,
  ...
  Widget _typeSwitcher() {
    if (!widget.allowReels) return const SizedBox.shrink();
  ...
  Widget _destinationBar() {
    if (_type != CreateContentType.post) return const SizedBox.shrink();
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(
            'Публикация появится в ',
            style: _text(9.2, color: const Color(0xFF7A828D)),
          ),
          Text(
            _postDestinationLabel,
            style: _text(9.2,
                weight: FontWeight.w700, color: const Color(0xFF067A46)),
          ),
        ],
      ),
    );
  ...
  Widget _composerBody() {
    final postComposer = CreatePostEditorScreen(
    ...
    if (!widget.allowReels) return postComposer;
