...
String _translatePosition(String key) {
  ...
  void _openAiReviewCmrWindow() {
    ...
    if (overlay == null) {
      if (_tabController.index != 5) {
        _tabController.animateTo(5, duration: const Duration(milliseconds: 160), curve: Curves.easeOutCubic);
      }
      if (mounted) setState(() {});
      return;
    }
    ...
    late OverlayEntry entry;
    Offset position = const Offset(72, 46);
    bool minimized = false;
    bool maximized = false;
    bool closed = false;
    ...
    entry = OverlayEntry(
      builder: (overlayContext) {
        ...
        final double height = maximized
            ? screen.height - media.padding.top - media.padding.bottom - 28
            : min(760.0, max(520.0, screen.height * .76));
        final double left = maximized
            ? 14.0
            : position.dx.clamp(14.0, max(14.0, screen.width - width - 14.0)).toDouble();
        final double top = maximized
            ? media.padding.top + 14.0
            : position.dy.clamp(media.padding.top + 14.0, max(media.padding.top + 14.0, screen.height - height - 14.0)).toDouble();
              ...
              child: InkWell(
                ...
                child: Container(
                  ...
                  child: const Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(Icons.psychology_alt_rounded, color: Color(0xFF28A86B), size: 16),
                      SizedBox(width: 8),
                      Text(
                        'ИИ-разбор',
                        style: TextStyle(
            ...
            child: Container(
              ...
              child: Column(
                children: [
                      ...
                      child: Row(
                        children: [
                          ...
                          Container(
                            ...
                            child: const Icon(Icons.psychology_alt_rounded, color: Color(0xFF1F2937), size: 16),
                          ),
                          const SizedBox(width: 10),
                          Expanded(
                            child: Text(
                  ...
                  Expanded(
                    child: ClipRect(
                      child: _buildAiVideoAnalysisTab(),
                    ),
                  ),
                ],
              ),
  ...
  Future<void> _deleteVideo(Map<String, dynamic> video) async {
    ...
    try {
      ...
      // Некоторые старые PHP-скрипты читают только $_POST, поэтому даём второй
      // безопасный вариант обычной form-urlencoded отправкой.
        ...
        children: [
          FutureBuilder<void>(
            ...
            builder: (context, snapshot) {
              final initError = _analysisVideoInitErrors[key];
                    ...
                    Center(
                      child: Padding(
                        ...
                        child: Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            Icon(Icons.error_outline_rounded, color: _mcOnDark, size: 34),
                            const SizedBox(height: 10),
                            Text(
                              'Видео не открылось во встроенном плеере',
                              textAlign: TextAlign.center,
                              style: TextStyle(
                                color: _mcOnDark,
                                fontSize: 13,
