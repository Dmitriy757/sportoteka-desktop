...
class _CmrClubAiAssistantPanelState extends State<CmrClubAiAssistantPanel> {
  ...
  static const String _askUrl = '$_apiBase/club_ai_search.php';
  static const String _feedbackUrl = '$_apiBase/club_ai_feedback.php';
  ...
  void initState() {
    ...
    _messages.add(_AiMessage.assistant(
      text:
          'Я локальный ИИ клуба без OpenAI. Работаю на вашем сервере: ищу отчеты, делаю тренерский разбор по GPS/Polar, объясняю причины спринтов и рисков, строю схемы прямо в чате и запоминаю правила тренера. Напишите: «почему такой спринт у Берёзкина», «сделай анализ тренировки», «нарисуй схему прессинга 4-3-3».',
      ...
      final res = await http
          .post(
            ...
            body: <String, String>{
              ...
              'q': q,
              if (widget.initialPayload != null) 'context_json': json.encode(widget.initialPayload),
            },
          )
    ...
    return Column(
      children: [
        _AiHeader(
          ...
          compact: true,
          onExample: () => _ask('Найди последний отчет по игроку'),
        ),
          ...
          child: Column(
            children: [
              _AiHeader(
                ...
                teamName: widget.teamName,
                compact: false,
                onExample: () => _ask('Покажи последнюю тренировку и отчет команды'),
              ),
...
class _AiHeader extends StatelessWidget {
  ...
  const _AiHeader({required this.clubName, required this.teamName, required this.compact, required this.onExample});
