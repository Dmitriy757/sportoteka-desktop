...
class _ClubWorkspaceScreenState extends State<ClubWorkspaceScreen>
  ...
  Future<void> _loadAll({bool initial = false}) async {
    ...
    if (trainerAssignedMode) {
      // В режиме тренера подписка принадлежит клубу.
      // Сначала получаем назначенные команды, восстанавливаем clubId,
      // затем профиль и тариф именно этого клуба.
      await _safeLoad(_loadTeams);
      await _safeLoad(_loadClubProfile);
      await _safeLoad(_loadSubscriptionPlan);
    } else {
      // Для владельца клуба clubId == currentUserId, поэтому всё можно
  ...
  Future<void> _loadSubscriptionPlan() async {
    // Подписка принадлежит клубу.
    // Тренер, которому клуб выдал доступ к команде, наследует тариф клуба.
    final subscriptionOwnerId = trainerAssignedMode ? clubId : currentUserId;

    if (subscriptionOwnerId <= 0) return;
    try {
      final response = await http.post(
        Uri.parse(getMySubscriptionUrl),
        body: {'user_id': '$subscriptionOwnerId'},
  ...
  Future<void> _loadAssignedTeamsForTrainer() async {
    ...
    try {
      ...
      debugPrint('Staff Access team filter error: $error');
    }
    ...
    if (!_trainerStaffManaged && teams.isNotEmpty) {
      ...
      clubSourceTeam ??= teams.first;

      final resolvedClubId = _asInt(
        clubSourceTeam['club_id'] ?? clubSourceTeam['clubId'],
      );
  ...
  Future<void> _selectTeam(Map<String, dynamic> team,
    ...
    final teamClubId = _asInt(team['club_id'] ?? team['clubId']);
    final trainerClubChanged =
        trainerAssignedMode && teamClubId > 0 && teamClubId != clubId;

    if (trainerAssignedMode && teamClubId > 0) {
      clubId = teamClubId;
    }
  ...
  void _openGameModule(ClubSection section) {
    // Меню «Ещё» передаёт сюда и обычные модули.
  ...
  void _selectWorkspaceSection(ClubSection section) {
    // Редактор схем всегда открываем сразу полноценным экраном.
    if (section == ClubSection.graphics) {
      if (!_canOpenWorkspaceSection(section)) return;
      _openFullGraphics();
      return;
  ...
  void _activateInlineSection(ClubSection section) {
    // При ширине окна меньше desktop-wide Workspace работает inline.
    // Для редактора схем промежуточную панель всё равно пропускаем.
    if (section == ClubSection.graphics) {
      if (!_canOpenWorkspaceSection(section)) return;
      _openFullGraphics();
      return;
  ...
  void _openModuleWindow(ClubSection section) {
    // На широком macOS/Windows тоже сразу открываем настоящий редактор.
    if (section == ClubSection.graphics) {
      if (!_canOpenWorkspaceSection(section)) return;
      _openFullGraphics();
      return;
  ...
  void _handleAiNavigate(
    ...
    switch (normalized) {
      ...
      case 'graphics':
      case 'tactical_board':
        _selectWorkspaceSection(ClubSection.graphics);
        return;
        ...
        return _TeamModulePanel(
          ...
          quickActions: [
            _ModuleQuickAction('Планы', Icons.folder_copy_rounded,
                () => setState(() => selectedSection = ClubSection.plans)),
            _ModuleQuickAction(
                'Графика', Icons.draw_rounded, _openFullGraphics),
            _ModuleQuickAction('Состав', Icons.groups_2_rounded,
                () => setState(() => selectedSection = ClubSection.roster)),
          ],
        );
