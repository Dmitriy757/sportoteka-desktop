...
import 'package:sportoteka/presentation/club_workspace/cmr_player_account_access_panel.dart';
...
class _PlayerDetailPanelState extends State<_PlayerDetailPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    final content = ListView(
      controller: scrollController,
      padding: EdgeInsets.fromLTRB(14, onClose == null ? 14 : 10, 14, 20),
      children: [
        if (onClose != null) ...[
          const _ModalGrabber(),
        ...
        CmrPlayerAccountAccessPanel(
          key: ValueKey(
            'player-account-${_first(p, const ['user_id', 'userId', 'player_id', 'id'])}',
          ),
          player: p,
          clubId: clubId,
          teamId: teamId,
        ...
        if (onOpenFullProfile != null) ...[
          const SizedBox(height: 12),
          _PlayerOpenWorkProfileButton(onTap: onOpenFullProfile!),
        ],
        ...
        _InspectorActionGroup(
          actions: [
            _InspectorAction(
              ...
              subtitle: 'Данные, амплуа и параметры игрока',
            ...
            _InspectorAction(
              ...
              subtitle: 'Нагрузка, метрики и динамика',
              onTap: onOpenFullProfile,
            ),
          ],
        ),
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Text(
                    'Игрок команды',
                    style: _CmrRosterText.muted(11).copyWith(
              ...
              Text(
                meta,
                style: _CmrRosterText.muted(11.8),
                maxLines: 2,
                overflow: TextOverflow.ellipsis,
              ),
...
class _PlayerOpenWorkProfileButton extends StatelessWidget {
  final VoidCallback onTap;

  const _PlayerOpenWorkProfileButton({required this.onTap});
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      ...
                      style: _CmrRosterText.value(11.8).copyWith(
                        color: _CmrRosterColors.greenDark,
                      ),
                    ),
                    const SizedBox(height: 2),
                    Text(
                      'Карточка · Дневник · Готовность · Матчи · Тестирование',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: _CmrRosterText.muted(9.3),
                    ),
                  ],
                ),
              ),
...
class _RosterManageMenuButton extends StatelessWidget {
  final VoidCallback onOpenUnassigned;
  final VoidCallback onOpenArchive;

  const _RosterManageMenuButton({
    required this.onOpenUnassigned,
    required this.onOpenArchive,
  });
        ...
        PopupMenuItem<String>(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 9),
              Text('Без команды', style: _CmrRosterText.action()),
            ],
          ),
        ),
        PopupMenuItem<String>(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 9),
              Text('Архив игроков', style: _CmrRosterText.action()),
            ],
          ),
        ),
      ],
          ...
          child: Column(
            children: [
              for (final action in actions)
                _InspectorActionRow(action: action),
            ],
          ),
        ),
      ],
...
class _InspectorActionRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final accentColor = action.accent
        ? _CmrRosterColors.green
        : _CmrRosterColors.muted2;
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        action.title,
                        style: _CmrRosterText.value(12.4),
                      ),
                      const SizedBox(height: 3),
                      Text(
                        action.subtitle,
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                        style: _CmrRosterText.muted(10.6),
                      ),
                    ],
                  ),
                ),
