...
class _ClubDashboardScreenState extends State<ClubDashboardScreen> 
  ...
  Future<void> _reloadAll() async {
    ...
    try {
      ...
      if (selectedTeamId == null && teams.isNotEmpty) {
        final t0 = teams.first as Map<String, dynamic>;
      ...
      if (mounted) {
        setState(() {
          if (data["success"] == true && data["club"] is Map) {
            ...
            clubName = _asString(clubData["club_name"])?.trim() ?? "Название клуба";
            final normalized = MediaUtils.normalizeUrl(_asString(clubData["photo"]));
    ...
    try {
      ...
      if (data["success"] == true) {
        ...
        setState(() {
          ...
          final clubMap = (data["club"] is Map) ? Map<String, dynamic>.from(data["club"]) : null;
        ...
        Get.snackbar("Ошибка", _asString(data["message"]) ?? "Не удалось обновить профиль");
      }
    } catch (e) {
                  ...
                  children: [
                    TextButton(
                      ...
                      child: const Text(
                        "Отмена",
                        style:
                            TextStyle(color: ClubDashboardPalette.textMuted),
                      ),
                    ),
  ...
  Future<List<Map<String, dynamic>>> _searchTrainerByEmail(String email,
                          ...
                          itemBuilder: (context, i) {
                            ...
                            return ListTile(
                              leading: CircleAvatar(
                                ...
                                child: Icon(Icons.person_outline, color: ClubDashboardPalette.primaryGreen),
                              ),
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              return ListTile(
                ...
                leading: CircleAvatar(
                  backgroundColor: ClubDashboardPalette.lightGreen,
                  child: Icon(Icons.group_outlined, color: ClubDashboardPalette.primaryGreen),
                ),
                title: Text(
                  name,
                ...
                trailing: isSelected
                    ? Icon(Icons.check_circle, color: ClubDashboardPalette.primaryGreen)
                      ...
                      onTap: () async {
                        ...
                        if (picked != null) {
                          setSB(() {
                            ...
                            selectedCoachName = _asString(picked["name"]) ?? "Тренер #$selectedCoachId";
                          });
                        }
                      },
                      borderRadius: BorderRadius.circular(12),
                        ...
                        child: Row(
                          children: [
                            CircleAvatar(
                              ...
                              backgroundColor: ClubDashboardPalette.lightGreen,
                              backgroundImage:
                                  pickedLogo != null ? FileImage(File(pickedLogo!.path)) : null,
                              child: pickedLogo == null 
                                ? Icon(Icons.image_outlined, color: ClubDashboardPalette.primaryGreen) 
                                : null,
                            ),
                            const SizedBox(width: 12),
                            Expanded(
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () async {
                          ...
                          if (selectedCoachId == null || selectedCoachId == 0) {
                            Get.snackbar("Ошибка", "Выберите тренера (coach)");
                            return;
                          }
  ...
  Future<void> _createTeam({
    ...
    try {
      ...
      if (data["success"] == true) {
        ...
        Get.snackbar(
            "Ошибка", _asString(data["message"]) ?? "Не удалось создать команду");
      }
    } catch (e) {
      Get.snackbar("Сеть", "Ошибка соединения: $e");
    }
  ...
  Future<void> _linkEventPlan({
    ...
    try {
      final resp = await http.post(
        ...
        body: {"event_id": eventId.toString(), "plan_id": planId.toString()},
      );
      final data = _decode(resp);
      if (data["success"] == true) {
        ...
        Get.snackbar(
            "Ошибка", _asString(data["message"]) ?? "Не удалось привязать план");
      }
    } catch (_) {
  ...
  Future<Map<String, dynamic>> _loadAllClubBundle() async {
    ...
    try {
      if (localTeams.isEmpty) {
        final resp = await http.post(Uri.parse(getClubTeamsUrl),
            body: {"club_id": clubId.toString()});
        final data = _decode(resp);
        if (data["success"] == true) {
      ...
      if (localTrainers.isEmpty) {
        final resp = await http.post(Uri.parse(getClubTrainersUrl),
            body: {"club_id": clubId.toString()});
        final data = _decode(resp);
        if (data["success"] == true) {
      ...
      if (localEvents.isEmpty) {
        final resp = await http.post(Uri.parse(getClubEventsUrl),
            body: {"club_id": clubId.toString()});
        final data = _decode(resp);
        if (data["success"] == true) {
  ...
  void _openModule(ClubModuleType type) {
  ...
  // АДАПТИВНЫЙ БАННЕР ДЛЯ ПЛАНШЕТА - более компактный и эстетичный
  Widget _buildModuleBanner(Map<String, dynamic> item) {
    return LayoutBuilder(
      builder: (context, constraints) {
        // Определяем, планшет ли это (ширина больше 600)
        ...
        return Material(
          ...
          child: InkWell(
            borderRadius: BorderRadius.circular(isTablet ? 12 : 16),
            onTap: () => _openModule(item['type'] as ClubModuleType),
            child: Container(
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(isTablet ? 12 : 16),
                boxShadow: [
        ...
        return SafeArea(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              final title = _asString(p["title"]) ?? _asString(p["name"]) ?? "План #$id";
              return ListTile(
                leading: CircleAvatar(
                  backgroundColor: ClubDashboardPalette.lightGreen,
                  child: Icon(Icons.menu_book, color: ClubDashboardPalette.primaryGreen),
                ),
                title: Text(title),
                ...
                : RefreshIndicator(
                    ...
                    child: CustomScrollView(
                      slivers: [
                        ...
                        SliverPadding(
                          padding: EdgeInsets.fromLTRB(isTablet ? 24 : 16, 0, isTablet ? 24 : 16, 12),
                        ...
                        SliverPadding(
                          padding: EdgeInsets.fromLTRB(isTablet ? 24 : 16, 0, isTablet ? 24 : 16, 16),
                          sliver: SliverToBoxAdapter(
                        ...
                        SliverPadding(
                          padding: EdgeInsets.fromLTRB(isTablet ? 24 : 16, 0, isTablet ? 24 : 16, 8),
                          sliver: SliverToBoxAdapter(
                            child: _SectionTitle(
                              title: "Команды клуба",
                              right: "всего: ${teams.length}",
                            ),
                          ),
                        ),
                        ...
                        if (teams.isEmpty)
                          ...
                          SliverPadding(
                            padding: EdgeInsets.fromLTRB(isTablet ? 24 : 16, 0, isTablet ? 24 : 16, 24),
                            sliver: SliverList(
                              delegate: SliverChildBuilderDelegate(
                                (context, index) => _TeamTile(
                                  team: teams[index] as Map<String, dynamic>,
                                  selectedId: selectedTeamId,
                                  onTap: () {
                                    final t = teams[index] as Map<String, dynamic>;
                                    setState(() {
                                      selectedTeamId = _asInt(t["id"]);
                                      selectedTeamName = _asString(t["name"]) ?? "Команда";
                                    });
                                    _openSelectedTeam();
                                  },
                                ),
                        ...
                        // АДАПТИВНАЯ СЕТКА ДЛЯ ПЛАНШЕТА
                        SliverPadding(
                          padding: EdgeInsets.fromLTRB(isTablet ? 24 : 16, 0, isTablet ? 24 : 16, 32),
                          sliver: SliverGrid(
                            gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                              crossAxisCount: isTablet ? 3 : 2, // На планшете 3 колонки
                              crossAxisSpacing: isTablet ? 12 : 12,
                              mainAxisSpacing: isTablet ? 12 : 12,
                              childAspectRatio: isTablet ? 1.2 : 1.4, // Более квадратные на планшете
                            ),
                            delegate: SliverChildBuilderDelegate(
                              (context, index) => _buildModuleBanner(_clubModuleBanners()[index]),
                              childCount: _clubModuleBanners().length,
                            ),
                          ),
                        ),
                      ],
      ...
      body: FadeTransition(
        ...
        child: loading
            ...
            : error != null
                ...
                : _ClubModuleBody(type: widget.type, bundle: bundle ?? const {}),
      ),
    );
  }
}
...
class _ClubModuleBody extends StatelessWidget {
  ...
  const _ClubModuleBody({required this.type, required this.bundle});
  ...
  Widget build(BuildContext context) {
    final isTablet = MediaQuery.of(context).size.width > 600;
    final club = bundle["club"] is Map ? Map<String, dynamic>.from(bundle["club"]) : null;
    ...
    Widget header = Container(
      ...
      margin: EdgeInsets.fromLTRB(isTablet ? 24 : 16, 12, isTablet ? 24 : 16, 12),
      decoration: BoxDecoration(
        color: ClubDashboardPalette.white,
        borderRadius: BorderRadius.circular(16),
      ),
      child: Row(
        children: [
          CircleAvatar(
            backgroundColor: ClubDashboardPalette.lightGreen,
            child: Icon(Icons.shield_outlined, color: ClubDashboardPalette.primaryGreen),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style:
                      const TextStyle(color: ClubDashboardPalette.textMuted, fontWeight: FontWeight.w600),
                ),
              ],
            ),
          ),
      ...
      children: [
        _whiteCard(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 6),
              if (desc.isNotEmpty)
                Text(desc, style: const TextStyle(color: ClubDashboardPalette.text, height: 1.25))
              else
                const Text(
                  "Описание клуба пока недоступно",
                  style: TextStyle(color: ClubDashboardPalette.textMuted, fontWeight: FontWeight.w600),
                ),
              if (address.isNotEmpty) ...[
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 6),
                    Expanded(
                      child: Text(address, style: const TextStyle(fontWeight: FontWeight.w700)),
                    ),
                  ],
                ),
              ],
        ...
        _whiteCard(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text("Состав клуба (команды)", style: TextStyle(fontWeight: FontWeight.w900)),
              const SizedBox(height: 10),
              if (teams.isEmpty)
                const Text("Команд нет.", style: TextStyle(color: ClubDashboardPalette.textMuted))
              else
                ...teams.take(20).map((e) {
                  ...
                  return Padding(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.group_outlined, size: 18, color: ClubDashboardPalette.primaryGreen),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Text(
                        ...
                        if (cat.isNotEmpty)
                          Text(cat, style: const TextStyle(color: ClubDashboardPalette.textMuted)),
                      ],
                    ),
                  );
                }),
  ...
  Widget _trainersView(List<dynamic> trainers) {
    return _whiteCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Тренеры", style: TextStyle(fontWeight: FontWeight.w900)),
          const SizedBox(height: 10),
          if (trainers.isEmpty)
            const Text("Тренеры не найдены.", style: TextStyle(color: ClubDashboardPalette.textMuted))
          else
            ...trainers.map((e) {
              final t = e as Map<String, dynamic>;
              ...
              return ListTile(
                ...
                title: Text(name.isEmpty ? "Тренер" : name,
                    style: const TextStyle(fontWeight: FontWeight.w800)),
  ...
  Widget _calendarView(List<dynamic> events) {
    return _whiteCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("События всех команд", style: TextStyle(fontWeight: FontWeight.w900)),
          const SizedBox(height: 10),
          if (events.isEmpty)
            const Text("Событий нет.", style: TextStyle(color: ClubDashboardPalette.textMuted))
          else
            ...events.take(30).map((e) {
              final ev = e as Map<String, dynamic>;
              final title =
                  _asStr(ev["title"]).isNotEmpty ? _asStr(ev["title"]) : _asStr(ev["name"]);
              ...
              return Padding(
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Icon(Icons.calendar_month_outlined, size: 18, color: ClubDashboardPalette.primaryGreen),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(title.isEmpty ? "Событие" : title,
                              style: const TextStyle(fontWeight: FontWeight.w800)),
                          ...
                          Text(
                            ...
                            style: const TextStyle(
                                color: ClubDashboardPalette.textMuted, fontWeight: FontWeight.w600),
                          ),
                        ],
                      ),
                    ),
    ...
    return _whiteCard(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text(text, style: const TextStyle(color: ClubDashboardPalette.text, height: 1.25)),
          const SizedBox(height: 12),
          const Text(
            "",
            style: TextStyle(color: ClubDashboardPalette.textMuted, fontWeight: FontWeight.w600),
        ...
        children: [
          Row(
            children: [
              CircleAvatar(
                backgroundColor: ClubDashboardPalette.lightGreen,
                child: Icon(Icons.sports_soccer_outlined, color: ClubDashboardPalette.primaryGreen),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label:
                  const Text("Открыть панель команды", style: TextStyle(color: Colors.white)),
              style: ElevatedButton.styleFrom(
                backgroundColor: ClubDashboardPalette.primaryGreen,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
                elevation: 0,
