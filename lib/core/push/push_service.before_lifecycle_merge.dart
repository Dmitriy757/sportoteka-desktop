...
class PushService with WidgetsBindingObserver {
  ...
  Future<void> _consumePendingCallkitAccept() async {
    // При Accept с заблокированного iPhone Flutter может ещё
    // только поднимать GetMaterialApp / Navigator.
    for (var attempt = 0; attempt < 40; attempt++) {
      ...
      if (currentUserId <= 0 || Get.context == null) {
      ...
      if (callId <= 0) {
        _log(
          'CALLKIT ACCEPT HANDLE: cannot determine call_id '
          'pending=$pending',
        );
    ...
    _log('CALLKIT ACCEPT HANDLE: navigator did not become ready');
  }

  @override
  void didChangeAppLifecycleState(AppLifecycleState state) {
    _log('SPORTOTEKA lifecycle: $state');
  ...
  Future<void> _recoverAcceptedCallOnResume() async {
    ...
    // После нажатия системной кнопки Accept CallKit и Flutter
    // пробуждаются не строго одновременно. Даём plugin несколько попыток.
    for (var attempt = 1; attempt <= 12; attempt++) {
      if (_callActionInProgress) {
        _log(
          'CALLKIT foreground recovery: call action already running',
        );
        return;
      }
      ...
      if (Get.context == null) {
        _log(
          ...
          'navigator not ready attempt=$attempt',
        );
      ...
      try {
        ...
        for (final params in calls) {
          ...
          _log(
            'CALLKIT active call '
            'callId=$callId '
            'accepted=${params.isAccepted} '
            'extra=${params.extra}',
          );

          if (!params.isAccepted || callId <= 0) {
            continue;
          }
          ...
          // Не даём старому FCM/dialog state заблокировать уже
          // принятый системный CallKit-вызов.
          _visibleCallId = null;

          _log(
            'CALLKIT foreground recovery OPEN '
            'callId=$callId '
            'callerId=$callerId '
            'calleeId=$calleeId',
          );
    ...
    _log('CALLKIT foreground recovery: no accepted call found');
  }
