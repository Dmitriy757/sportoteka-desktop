...
Future<void> sportotekaFirebaseMessagingBackgroundHandler(
  ...
  final callId =
      _sportotekaPushInt(data['call_id']);

  final userId =
      _sportotekaPushInt(data['callee_id']);
...
class PushService with WidgetsBindingObserver {
  PushService._() {
    if (Platform.isIOS) {
      IosNativeCallBridge.onAccepted =
          _handleIosNativeAcceptedCall;
    }
  }
  ...
  Future<void> _handleIosNativeAcceptedCall(
    ...
    final userId =
        _sportotekaPushInt(data['user_id']);

    final callerId =
        _sportotekaPushInt(data['caller_id']);
    ...
    await _openAcceptedCallScreen(
      ...
      callerId: callerId,
      callerName:
          callerName.isEmpty ? null : callerName,
    );
  }
  ...
  Future<void> _acceptIncomingCall({
    ...
    final nativeCallUuid = suppliedUuid.isNotEmpty
        ? suppliedUuid
        : _sportotekaCallUuid(callId);
  ...
  Future<void> _handleCallKitEvent(CallEvent event) async {
    ...
    if (eventName.contains('accept')) {
      ...
      // flutter_callkit_incoming Android оставляет incoming ringtone /
      // incoming notification активными после ACTION_CALL_ACCEPT.
      // Убираем их ДО запуска LiveKit, иначе STREAM_RING может
      // продолжать держать audio focus и мешать голосовому звуку.
      if (Platform.isAndroid && params != null) {
        try {
          await FlutterCallkitIncoming.hideCallkitIncoming(params);
          _log(
            'CALLKIT Android incoming ringtone stopped '
            'callId=$callId',
          );
        } catch (e) {
          _log(
            'CALLKIT Android hide incoming error '
            'callId=$callId error=$e',
          );
        }
      }
